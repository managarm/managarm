...
struct Device final : drvcore::ClassDevice {
	...
	std::optional<std::string> getClassPath() override { return "net"; };
...
async::detached run() {
	...
	auto filter = mbus_ng::Conjunction{{mbus_ng::EqualsFilter{"unix.subsystem", "net"}}};
	...
	while (true) {
		...
		for (auto &event : events) {
			...
			if (event.properties.contains("usb.parent-interface"))
				parent_dev =
				    co_await usb_subsystem::getInterfaceDevice(parent_dev, event.properties);
			...
			auto net =
			    std::make_shared<Device>(sysfsSubsystem, ifname, std::stoi(ifindex), parent_dev);
