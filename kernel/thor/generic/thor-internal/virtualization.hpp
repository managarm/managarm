...
struct VirtualizedPageSpace : VirtualSpace {
	...
	struct Operations final : VirtualOperations {
		Operations(VirtualizedPageSpace *space) : space_{space} {}

		void retire(RetireNode *node) override { return space_->retire(node); }

		bool submitShootdown(ShootNode *node) override { return space_->submitShootdown(node); }

		void mapSingle4k(VirtualAddr pointer, PhysicalAddr physical, uint32_t flags, CachingMode)
		    override {
			space_->map(pointer, physical, flags);
		}

		PageStatus unmapSingle4k(VirtualAddr pointer) override { return space_->unmap(pointer); }
		...
		bool isMapped(VirtualAddr pointer) override { return space_->isMapped(pointer); }
