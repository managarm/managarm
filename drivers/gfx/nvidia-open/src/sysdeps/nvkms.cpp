...
void nvkms_call_rm(void *ops) {
    rm_kernel_rmapi_op(nullptr, ops);
}
...
int    nvkms_copyin     (void *kptr [[maybe_unused]],
                         NvU64 uaddr [[maybe_unused]],
                         size_t n [[maybe_unused]]) STUBBED;
int    nvkms_copyout    (NvU64 uaddr [[maybe_unused]],
                         const void *kptr [[maybe_unused]],
                         size_t n [[maybe_unused]]) STUBBED;
...
NvBool nvkms_syncpt_op  (enum NvKmsSyncPtOp op [[maybe_unused]],
                         NvKmsSyncPtOpParams *params [[maybe_unused]]) STUBBED;
...
void *handleTimer(void *arg) {
    ...
    if(timer->isRefPtr) {
        timer->proc(nvkms_dec_ref(reinterpret_cast<nvkms_ref_ptr *>(timer->dataPtr)), timer->dataU32);
    } else {
...
nvkms_timer_handle_t *nvkms_alloc_timer(nvkms_timer_proc_t *proc,
	    void *dataPtr, NvU32 dataU32, NvU64 usec) {
    uint64_t now;
    HEL_CHECK(helGetClock(&now));

    auto timer = new nvkmsTimer{proc, dataPtr, dataU32,
        now + (usec * 1'000)};
...
NvBool nvkms_alloc_timer_with_ref_ptr(nvkms_timer_proc_t *proc, struct nvkms_ref_ptr *ref_ptr,
        NvU32 dataU32, NvU64 usec) {
    uint64_t now;
    HEL_CHECK(helGetClock(&now));

    nvkms_inc_ref(ref_ptr);
    auto timer = new nvkmsTimer{proc, ref_ptr, dataU32,
        now + (usec * 1'000), true};
...
void nvkms_read_lock_pm_lock() {
    pthread_rwlock_rdlock(&pmRwLock);
}

void nvkms_read_unlock_pm_lock() {
    pthread_rwlock_unlock(&pmRwLock);
}

bool nvkms_read_trylock_pm_lock() {
    return pthread_rwlock_tryrdlock(&pmRwLock) != 0;
}

struct nvkms_per_open *nvkms_open_common(enum NvKmsClientType type, struct NvKmsKapiDevice *device, int *status) {
...
NvBool nvkms_ioctl_from_kapi(struct nvkms_per_open *popen [[maybe_unused]], NvU32 cmd [[maybe_unused]], void *params_address [[maybe_unused]], const size_t param_size [[maybe_unused]]) {
    nvkms_read_lock_pm_lock();
...
NvBool nvkms_ioctl_from_kapi_try_pmlock(struct nvkms_per_open *popen, NvU32 cmd, void *params_address, const size_t param_size) {
...
struct nvkms_backlight_device *nvkms_register_backlight(NvU32 gpu_id [[maybe_unused]],
    NvU32 display_id [[maybe_unused]], void *drv_priv [[maybe_unused]],
    NvU32 current_brightness [[maybe_unused]]) STUBBED;
