...
struct Mapping {
	...
	Mapping() : _window{nullptr}, _offset{0}, _size{0} {}
	...
	Mapping(Mapping &&other) : Mapping() { swap(*this, other); }
	...
	helix::BorrowedDescriptor memory() { return _memory; }
	ptrdiff_t offset() { return _offset; }

	void *get() { return reinterpret_cast<char *>(_window) + (_offset & (pageSize - 1)); }
...
struct LegacyPciTransport : Transport {
	...
	LegacyPciTransport(
	    protocols::hw::Device hw_device, arch::io_space legacy_space, helix::UniqueDescriptor irq
	);

	protocols::hw::Device &hwDevice() override { return _hwDevice; }
...
bool LegacyPciTransport::checkDeviceFeature(unsigned int feature) {
	if (feature >= 32) {
		std::cout << "core-virtio: Feature index " << feature
		          << " cannot be queried"
		             " on legacy device"
		          << std::endl;
		return false;
...
void LegacyPciTransport::claimQueues(unsigned int max_index) { _queues.resize(max_index); }
...
Queue *LegacyPciTransport::setupQueue(unsigned int queue_index) {
	...
	auto available_offset = (queue_size * sizeof(spec::Descriptor) + (available_align - 1)) &
	...
	auto region_size =
	    used_offset + queue_size * sizeof(spec::UsedRing::Element) + sizeof(spec::UsedExtra);
	...
	HEL_CHECK(helMapMemory(
	    memory, kHelNullHandle, nullptr, 0, 0x4000, kHelMapProtRead | kHelMapProtWrite, &window
	));
	...
	_queues[queue_index] =
	    std::make_unique<LegacyPciQueue>(this, queue_index, queue_size, table, available, used);
...
void StandardPciTransport::claimQueues(unsigned int max_index) { _queues.resize(max_index); }
...
discover(protocols::hw::Device hw_device, DiscoverMode mode) {
	...
	if (mode == DiscoverMode::transitional || mode == DiscoverMode::modernOnly) {
		...
		if (common_mapping && notify_mapping && isr_mapping && device_mapping) {
			...
			common_space.store(
			    PCI_DEVICE_STATUS, common_space.load(PCI_DEVICE_STATUS) | ACKNOWLEDGE
			);
			common_space.store(PCI_DEVICE_STATUS, common_space.load(PCI_DEVICE_STATUS) | DRIVER);
	...
	if (mode == DiscoverMode::legacyOnly || mode == DiscoverMode::transitional) {
		...
		if (info.barInfo[0].ioType == protocols::hw::IoType::kIoTypePort) {
			...
			// The specification says this should be done in two steps
			legacy_space.store(
			    PCI_L_DEVICE_STATUS, legacy_space.load(PCI_L_DEVICE_STATUS) | ACKNOWLEDGE
			);
			legacy_space.store(
			    PCI_L_DEVICE_STATUS, legacy_space.load(PCI_L_DEVICE_STATUS) | DRIVER
			);

			std::cout << "virtio: Using legacy PCI transport" << std::endl;
			co_return std::make_unique<LegacyPciTransport>(
			    std::move(hw_device), legacy_space, std::move(irq)
			);
		}
#else
...
Handle::Handle(Queue *queue, size_t table_index) : _queue{queue}, _tableIndex{table_index} {}
...
void Queue::postDescriptor(Handle handle, Request *request, void (*complete)(Request *)) {
