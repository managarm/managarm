...
async::result<std::shared_ptr<nic::Link>> makeShared(
	...
	auto config =
	    (co_await hw_device.useConfiguration(info.configuration_index, *info.chosen_configuration))
	        .unwrap();
	auto ctrl_intf = (co_await config.useInterface(*info.control_if, 0)).unwrap();
	auto ctrl_ep =
	    (co_await ctrl_intf.getEndpoint(protocols::usb::PipeType::in, *info.int_endp_number))
	        .value();

	auto data_intf = (co_await config.useInterface(*info.data_if, 1)).unwrap();
	auto data_in =
	    (co_await data_intf.getEndpoint(protocols::usb::PipeType::in, info.in_endp_number.value()))
	        .unwrap();
	auto data_out =
	    (co_await data_intf.getEndpoint(protocols::usb::PipeType::out, info.out_endp_number.value())
	    )
	        .unwrap();

	if (info.subclass == protocols::usb::cdc_subclass::ncm) {
		auto nic = std::make_shared<nic::usb_ncm::UsbNcmNic>(
		    entity,
		    std::move(hw_device),
		    mac,
		    std::move(ctrl_intf),
		...
		auto nic = std::make_shared<nic::usb_ecm::UsbEcmNic>(
		    entity,
		    std::move(hw_device),
		    mac,
		    std::move(ctrl_intf),
