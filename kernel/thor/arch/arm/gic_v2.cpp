...
GicDistributorV2::GicDistributorV2(uintptr_t addr) : base_{addr}, space_{}, irqPins_{*kernelAlloc} {
	auto register_ptr = KernelVirtualMemory::global().allocate(0x1000);
	KernelPageSpace::global().mapSingle4k(
	    VirtualAddr(register_ptr), addr, page_access::write, CachingMode::mmio
	);
...
void GicDistributorV2::init() {
	...
	infoLogger() << "GIC Distributor has " << noLines << " IRQs, " << noCpuIface
	             << " CPU interfaces and " << (securityExtensions ? "supports" : "doesn't support")
...
void GicDistributorV2::sendIpi(uint8_t ifaceNo, uint8_t id) {
	space_.store_relaxed(
	    dist_reg::sgi,
	    dist_sgi::sgiNo(id) | dist_sgi::cpuTargetList(1 << ifaceNo) | dist_sgi::targetListFilter(0)
	);
}
...
void GicDistributorV2::Pin::mask() {
	...
	arch::scalar_store_relaxed<uint32_t>(
	    parent_->space_, dist_reg::irqClearEnableBase + regOff, (1 << bitOff)
	);
}
...
void GicDistributorV2::Pin::unmask() {
	...
	arch::scalar_store_relaxed<uint32_t>(
	    parent_->space_, dist_reg::irqSetEnableBase + regOff, (1 << bitOff)
	);
}

void GicDistributorV2::Pin::sendEoi() { getCpuData()->gicCpuInterfaceV2->eoi(0, irq_); }

void GicDistributorV2::Pin::setAffinity_(uint8_t ifaceNo) {
	size_t regOff = (irq_ / 4) * 4;
	size_t bitOff = (irq_ & 3) * 8;

	auto v = arch::scalar_load_relaxed<uint32_t>(parent_->space_, dist_reg::irqTargetBase + regOff);

	v &= ~(0xFF << bitOff);
	v |= (1 << ifaceNo) << bitOff;

	arch::scalar_store_relaxed<uint32_t>(parent_->space_, dist_reg::irqTargetBase + regOff, v);
}
...
void GicDistributorV2::Pin::setPriority_(uint8_t prio) {
	...
	arch::scalar_store_relaxed<uint32_t>(parent_->space_, dist_reg::irqPriorityBase + regOff, v);
}
...
bool GicDistributorV2::Pin::setMode(TriggerMode trigger, Polarity polarity) {
	...
	auto v = arch::scalar_load_relaxed<uint32_t>(parent_->space_, dist_reg::irqConfigBase + i * 4);

	v &= ~(3 << j);
	v |= (trigger == TriggerMode::edge ? 2 : 0) << j;

	arch::scalar_store_relaxed<uint32_t>(parent_->space_, dist_reg::irqConfigBase + i * 4, v);
...
void GicDistributorV2::dumpPendingSgis() {
	for (int i = 0; i < 16; i++) {
		...
		auto regv =
		    arch::scalar_load_relaxed<uint32_t>(space_, dist_reg::sgiSetPendingBase + reg * 4);

		auto sgiv = (regv >> off) & 0xFF;

		for (int j = 0; j < 8; j++) {
			if (sgiv & (1 << j)) {
				infoLogger() << "thor: on CPU " << getCpuData()->cpuIndex << ", SGI " << i
...
uint8_t GicDistributorV2::getCurrentCpuIfaceNo_() {
	for (size_t i = 0; i < 8; i++) {
		...
		auto mask = ((v >> 24) & 0xFF) | ((v >> 16) & 0xFF) | ((v >> 8) & 0xFF) | (v & 0xFF);
...
void initGicOnThisCpuV2() {
	...
	cpuData->gicCpuInterfaceV2 = frg::construct<GicCpuInterfaceV2>(
	    *kernelAlloc, dist.get(), cpuInterfaceAddr, cpuInterfaceSize
	);
...
void GicV2::sendIpiToOthers(uint8_t id) { dist->sendIpiToOthers(id); }
...
void GicV2::eoi(uint32_t cpuId, uint32_t id) { getCpuData()->gicCpuInterfaceV2->eoi(cpuId, id); }

Gic::Pin *GicV2::setupIrq(uint32_t irq, TriggerMode trigger) {
	return dist->setupIrq(irq, trigger);
}

Gic::Pin *GicV2::getPin(uint32_t irq) { return dist->getPin(irq); }
