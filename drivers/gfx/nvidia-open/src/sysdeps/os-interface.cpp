...
void * NV_API_CALL os_map_kernel_space(NvU64 start, NvU64 size_bytes, NvU32 mode) {
	...
	if(mode == NV_MEMORY_DEFAULT || mode == NV_MEMORY_WRITECOMBINED)
		caching |= kHelMapCacheWriteCombine;

	void *window;
	HEL_CHECK(helMapMemory(memory.getHandle(), kHelNullHandle, nullptr,
		alignedOffset, alignedSize, kHelMapProtRead | kHelMapProtWrite | caching,
		&window));
	assert(window);
...
void *NV_API_CALL nv_alloc_kernel_mapping(nv_state_t *, void *pAllocPrivate,
	...
	HEL_CHECK(helMapMemory(info->handle, kHelNullHandle,
		...
		kHelMapProtRead | kHelMapProtWrite | info->caching, &ptr));
...
NV_STATUS NV_API_CALL nv_alloc_pages(nv_state_t *, NvU32 page_count, NvU64 page_size [[maybe_unused]],
		NvBool contiguous, NvU32 cache_type, NvBool zeroed, NvBool unencrypted [[maybe_unused]],
	...
	auto info = new AllocInfo{handle, page_count, 0, caching};
