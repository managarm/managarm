...
void PrecisionTimerEngine::installTimer(PrecisionTimerNode *timer) {
	...
	if (logTimers) {
		...
		infoLogger() << "thor: Setting timer at " << timer->_deadline << " (counter is " << current
		             << ")" << frg::endlog;
	}
...
ClockSource *systemClockSource() { return globalClockSource; }

PrecisionTimerEngine *generalTimerEngine() { return globalTimerEngine; }
