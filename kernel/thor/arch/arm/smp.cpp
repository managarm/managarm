...
bool initPsciFromAcpi() {
	uacpi_table fadtTbl;
	if (uacpi_table_find_by_signature("FACP", &fadtTbl) != UACPI_STATUS_OK) {
		infoLogger() << "thor: ACPI FADT not found, trying GICC parking protocol"
		             << frg::endlog;
		return false;
	}
	frg::scope_exit finish{[&] { uacpi_table_unref(&fadtTbl); }};

	auto *fadt = reinterpret_cast<acpi_fadt *>(fadtTbl.ptr);
	constexpr size_t requiredFadtLength = offsetof(acpi_fadt, arm_boot_arch)
	                                      + sizeof(fadt->arm_boot_arch);
	if (fadtTbl.hdr->length < requiredFadtLength) {
...
#define KANKER infoLogger() << "FUCK" << __LINE__ << frg::endlog;
