...
struct OpenFile : File {
  ...
  public:
	...
	async::result<size_t> waitForEvents(
	    struct epoll_event *events, size_t max_events, async::cancellation_token cancellation
	) {
		assert(max_events);
		if (logEpoll) {
			std::cout << "posix.epoll \e[1;34m" << structName()
			          << "\e[0m: Entering wait."
			             " There are "
			          << _pendingQueue.size() << " pending items; cancellation is "
			...
			while (!_pendingQueue.empty()) {
				...
				if (!(item->state & stateActive)) {
					if (logEpoll)
						std::cout << "posix.epoll \e[1;34m" << structName()
						          << "\e[0m: Discarding"
						             " inactive item \e[1;34m"
						          << item->file->structName() << "\e[0m" << std::endl;
					item->state &= ~statePending;
					continue;
				}
				...
				if (logEpoll)
					std::cout << "posix.epoll \e[1;34m" << structName()
					          << "\e[0m:"
					             " Item \e[1;34m"
					          << item->file->structName()
					          << "\e[0m"
					             " mask is "
					          << item->eventMask << ", while " << std::get<1>(result)
					          << " is active" << std::endl;
  ...
  public:
	...
	OpenFile()
	    : File{StructName::get("epoll"), nullptr, SpecialLink::makeSpecialLink(VfsType::regular, 0777), File::defaultPipeLikeSeek},
	      _currentSeq{0} {}
  ...
  private:
	...
	struct KeyHash {
		size_t operator()(const Key &k) const {
			return 13 * reinterpret_cast<uintptr_t>(k.first) + static_cast<size_t>(k.second);
		}
	};
