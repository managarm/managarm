...
read(void *object, const char *, void *buffer, size_t length) {
	...
	/*
	    ...
	    auto &&submit =
	   helix::submitLockMemoryView(helix::BorrowedDescriptor(self->inode->frontalMemory),
	...
	*/
	...
	auto readMemory = co_await helix_ng::readMemory(
	    helix::BorrowedDescriptor(self->inode->frontalMemory), chunk_offset, chunkSize, buffer
	);
...
async::result<protocols::fs::ReadResult>
pread(void *object, int64_t offset, const char *, void *buffer, size_t length) {
	assert(length);
	...
	memcpy(
	    buffer, reinterpret_cast<char *>(file_map.get()) + (chunk_offset - map_offset), chunk_size
	);
...
async::result<frg::expected<protocols::fs::Error, size_t>>
pwrite(void *object, int64_t offset, const char *credentials, const void *buffer, size_t length) {
	(void)credentials;
...
async::detached serve(
	...
	co_await protocols::fs::servePassthrough(std::move(local_pt), file, &fileOperations, cancel_pt);
}
...
async::result<protocols::fs::OpenResult> open(std::shared_ptr<void> object, bool append) {
	auto self = std::static_pointer_cast<ext2fs::Inode>(object);
	...
	auto syncInode = co_await helix_ng::synchronizeSpace(
	    helix::BorrowedDescriptor{kHelNullHandle}, self->diskMapping.get(), self->fs.inodeSize
	);
...
async::result<std::string> readSymlink(std::shared_ptr<void> object) {
	...
	if (self->fileSize() <= 60) {
		co_return std::string{
		    self->diskInode()->data.embedded, self->diskInode()->data.embedded + self->fileSize()
		};
	} else {
		std::string result;
		result.resize(self->fileSize());
		co_await helix_ng::readMemory(
		    helix::BorrowedDescriptor(self->frontalMemory), 0, self->fileSize(), result.data()
		);
		co_return result;
	}
}

async::result<protocols::fs::MkdirResult> mkdir(std::shared_ptr<void> object, std::string name) {
	auto self = std::static_pointer_cast<ext2fs::Inode>(object);
...
rawRead(void *object, const char *, void *buffer, size_t length) {
	...
	auto readMemory = co_await helix_ng::readMemory(
	    helix::BorrowedDescriptor(self->rawFs->frontalMemory), chunk_offset, chunkSize, buffer
	);
	HEL_CHECK(readMemory.error());
...
async::detached servePartition(
    helix::UniqueLane lane, gpt::Partition *partition, std::unique_ptr<raw::RawFs> rawFs
) {
	...
	while (true) {
		...
		if (req.req_type() == managarm::fs::CntReqType::DEV_MOUNT) {
			...
			helix::UniqueLane local_lane, remote_lane;
			std::tie(local_lane, remote_lane) = helix::createStream();
			protocols::fs::serveNode(std::move(local_lane), inode, &nodeOperations);
			...
			if (old_file) {
				...
				if (!result) {
					if (result.error() == protocols::fs::Error::fileNotFound) {
						...
						// Handle error
						std::cout << "libblockfs: rename: unhandled error: " << (int)result.error()
						          << std::endl;
						assert(
						    result.error() == protocols::fs::Error::fileNotFound ||
						    result.error() == protocols::fs::Error::directoryNotEmpty ||
				...
				co_await newInode->link(
				    req->new_name(), old_file.value().inode, old_file.value().fileType
				);
			} else {
			...
			async::detach(
			    protocols::fs::servePassthrough(std::move(local_lane), file, &rawOperations)
			);
			...
			if (req->command() == BLKGETSIZE64) {
				...
				std::cout << "\e[31m"
				             "libblockfs: Unknown ioctl() message with ID "
...
async::detached runDevice(BlockDevice *device) {
	...
	int64_t diskId = 0;
	{
		...
		auto entity =
		    (co_await mbus_ng::Instance::global().createEntity("disk", descriptor)).unwrap();
		diskId = entity.id();
		...
		[](mbus_ng::EntityManager entity) -> async::detached {
			while (true) {
				...
				std::cout << "\e[31mlibblockfs: Disks don't currently serve requests\e[39m"
				          << std::endl;
			}
		}(std::move(entity));
	}
	...
	for (size_t i = 0; i < table->numPartitions(); ++i) {
		...
		mbus_ng::Properties descriptor{
		    ...
		    {"drvcore.mbus-parent", mbus_ng::StringItem{std::to_string(device->parentId)}},
		    {"unix.is-managarm-root",
		     mbus_ng::StringItem{std::to_string(type == gpt::type_guids::managarmRootPartition)}}
		};

		auto entity =
		    (co_await mbus_ng::Instance::global().createEntity("partition", descriptor)).unwrap();

		[](mbus_ng::EntityManager entity,
		   gpt::Partition *partition,
		   std::unique_ptr<raw::RawFs> rawFs) -> async::detached {
			while (true) {
				auto [localLane, remoteLane] = helix::createStream();
