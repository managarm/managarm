...
EcamPcieConfigIo::EcamPcieConfigIo(
    uintptr_t mmioBase, uint16_t seg, uint8_t busStart, uint8_t busEnd
)
    : mmioBase_{mmioBase},
...
arch::mem_space EcamPcieConfigIo::spaceForBus_(uint32_t bus) {
	...
	for (size_t i = 0; i < size; i += 0x1000) {
		KernelPageSpace::global().mapSingle4k(
		    VirtualAddr(ptr) + i, mmioBase_ + i + offset, page_access::write, CachingMode::mmio
		);
	}
...
uintptr_t EcamPcieConfigIo::calculateOffset_(uint32_t slot, uint32_t function, uint16_t offset) {
