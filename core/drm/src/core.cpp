...
void drm_core::File::setBlocking(bool blocking) { _isBlocking = blocking; }
...
uint32_t drm_core::File::createHandle(std::shared_ptr<BufferObject> bo) {
	...
	if (logDrmRequests)
		std::cout << "core/drm: createHandle for BufferObject " << bo.get() << " -> handle "
		          << handle << std::endl;

	auto [boMemory, boOffset] = bo->getMemory();
	HEL_CHECK(helAlterMemoryIndirection(
	    _memory.getHandle(), bo->getMapping() >> 32, boMemory.getHandle(), boOffset, bo->getSize()
	));
...
drm_core::File::read(void *object, const char *, void *buffer, size_t length) {
...
async::result<helix::BorrowedDescriptor> drm_core::File::accessMemory(void *object) {
	auto self = static_cast<drm_core::File *>(object);
	co_return self->_memory;
}

async::result<frg::expected<protocols::fs::Error, protocols::fs::PollWaitResult>>
drm_core::File::pollWait(void *object, uint64_t sequence, int mask, async::cancellation_token) {
	(void)mask;
	...
	co_return protocols::fs::PollWaitResult{
	    self->_eventSequence, self->_eventSequence > 0 ? EPOLLIN : 0
	};
}
...
drm_core::PrimeFile::PrimeFile(helix::BorrowedDescriptor handle, size_t size) : size(size) {
	_memory = std::move(handle);
};

async::result<helix::BorrowedDescriptor> drm_core::PrimeFile::accessMemory(void *object) {
...
async::detached serveDrmDevice(std::shared_ptr<drm_core::Device> device, helix::UniqueLane lane) {
	while (true) {
		...
		if (req.req_type() == managarm::fs::CntReqType::DEV_OPEN) {
			...
			async::detach(
			    protocols::fs::servePassthrough(std::move(local_lane), file, &defaultFileOperations)
			);

			managarm::fs::SvrResponse resp;
			resp.set_error(managarm::fs::Errors::SUCCESS);
			resp.set_caps(
			    managarm::fs::FileCaps::FC_STATUS_PAGE | managarm::fs::FileCaps::FC_POSIX_LANE
			);
...
void drm_core::addDmtModes(
    std::vector<drm_mode_modeinfo> &supported_modes, unsigned int max_width, unsigned max_height
) {
