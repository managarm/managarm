...
enum class Condition { execute, write, io, readwrite };
enum class Size { size1, size2, size8, size4 };

template <typename T> struct BreakOnWrite;
...
template <typename T> void breakOnWrite(const T *p) { _debug::BreakOnWrite<T>::invoke(p); }
