...
struct PllParams {
	...
	void dump(int refclock) {
		std::cout << "n: " << n << ", m1: " << m1 << ", m2: " << m2 << ", p1: " << p1
		          << ", p2: " << p2 << std::endl;
		std::cout << "m: " << computeM() << ", p: " << computeP() << std::endl;
		std::cout << "dot: " << computeDot(refclock) << ", vco: " << computeVco(refclock)
		          << std::endl;
	}
...
struct Timings {
	...
	int blankingStart() { return active; }
	int blankingEnd() { return total; }
