...
#error Signal register loading code is missing for architecture
	...
	memcpy(&sf.ucontext.uc_sigmask, &rawHandlerMask, sizeof(rawHandlerMask));

	sigset_t handlerMask = {};
	sigemptyset(&handlerMask);
	memcpy(&handlerMask, &rawHandlerMask, sizeof(rawHandlerMask));

	if (!(handling.handler.flags & signalReentrant)) {
		sigaddset(&handlerMask, item->signalNumber);
		memcpy(&rawHandlerMask, &handlerMask, sizeof(rawHandlerMask));
	}
