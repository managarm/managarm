...
async::result<void> drm_core::File::ioctl(
	...
	if (id == managarm::fs::GenericIoctlRequest::message_id) {
		...
		if (req->command() == DRM_IOCTL_VERSION) {
			...
			if (!max_width || !max_height) {
				std::cout << "\e[33mcore/drm: driver-supplied max width/height is empty, "
				             "defaulting to 16384x16384\e[39m"
				          << std::endl;
				max_width = 16384;
				max_height = 16384;
			}
			...
			for (auto ass : assignments) {
				...
				if (logDrmRequests) {
					std::cout << "\tproperty " << ass.property->id() << " '" << ass.property->name()
					          << "' = "
					          << resp.drm_obj_property_values(
					                 resp.drm_obj_property_values_size() - 1
					             )
					          << std::endl;
				}
			}
			...
			if (logDrmRequests)
				std::cout << "core/drm: CREATE_DUMB(" << req->drm_width() << "x"
				          << req->drm_height() << ") -> <" << resp.drm_handle() << ">" << std::endl;
			...
			if (logDrmRequests)
				std::cout << "core/drm: ADDFB(" << req->drm_width() << "x" << req->drm_height()
				          << ", pitch " << req->drm_pitch() << ")";
			...
			auto fb = self->_device->createFrameBuffer(
			    buffer, req->drm_width(), req->drm_height(), fourcc, req->drm_pitch()
			);
			...
			if (logDrmRequests)
				std::cout << "core/drm: ADDFB2(" << req->drm_width() << "x" << req->drm_height()
				          << ", pitch " << req->drm_pitch() << ")";
			...
			auto fb = self->_device->createFrameBuffer(
			    buffer, req->drm_width(), req->drm_height(), req->drm_fourcc(), req->drm_pitch()
			);
			self->attachFrameBuffer(fb);
			resp.set_drm_fb_id(fb->id());
			resp.set_error(managarm::fs::Errors::SUCCESS);
			...
			if (req->drm_mode_valid()) {
				...
				assignments.push_back(Assignment::withInt(
				    crtc->sharedModeObject(), self->_device->activeProperty(), true
				));
				assignments.push_back(Assignment::withBlob(
				    crtc->sharedModeObject(), self->_device->modeIdProperty(), mode_blob
				));
				assignments.push_back(Assignment::withModeObj(
				    crtc->primaryPlane()->sharedModeObject(), self->_device->fbIdProperty(), fb
				));
			} else {
				assignments.push_back(Assignment::withInt(
				    crtc->sharedModeObject(), self->_device->activeProperty(), false
				));
				assignments.push_back(Assignment::withBlob(
				    crtc->sharedModeObject(), self->_device->modeIdProperty(), nullptr
				));
			}
			...
			if (req->drm_flags() == DRM_MODE_CURSOR_BO) {
				...
				assignments.push_back(Assignment::withInt(
				    cursor_plane->sharedModeObject(), self->_device->srcWProperty(), width << 16
				));
				assignments.push_back(Assignment::withInt(
				    cursor_plane->sharedModeObject(), self->_device->srcHProperty(), height << 16
				));

				if (bo) {
					auto fb = self->_device->createFrameBuffer(
					    bo->sharedBufferObject(), width, height, DRM_FORMAT_ARGB8888, width * 4
					);
					assert(fb);
					assignments.push_back(Assignment::withModeObj(
					    crtc->cursorPlane()->sharedModeObject(), self->_device->fbIdProperty(), fb
					));
				} else {
					assignments.push_back(Assignment::withModeObj(
				...
				assignments.push_back(Assignment::withInt(
				    cursor_plane->sharedModeObject(), self->_device->crtcXProperty(), x
				));
				assignments.push_back(Assignment::withInt(
				    cursor_plane->sharedModeObject(), self->_device->crtcYProperty(), y
				));
			} else {
				printf(
				    "\e[35mcore/drm: invalid request whilst handling DRM_IOCTL_MODE_CURSOR\e[39m\n"
				);
				resp.set_error(managarm::fs::Errors::ILLEGAL_ARGUMENT);
			}
			...
			if (req->drm_capability() == DRM_CLIENT_CAP_STEREO_3D) {
				std::cout << "\e[31mcore/drm: DRM client cap for stereo 3D unsupported\e[39m"
				          << std::endl;
				resp.set_error(managarm::fs::Errors::ILLEGAL_ARGUMENT);
				...
				std::cout << "\e[31mcore/drm: Attempt to set unknown client capability "
				          << req->drm_capability() << "\e[39m" << std::endl;
				resp.set_error(managarm::fs::Errors::ILLEGAL_ARGUMENT);
			}
			...
			for (auto ass : obj->getAssignments(self->_device)) {
				...
				if (std::holds_alternative<IntPropertyType>(ass.property->propertyType())) {
					...
					if (logDrmRequests)
						std::cout << "\t" << ass.property->name() << " -> int " << ass.intValue
						          << std::endl;
				} else if (std::holds_alternative<EnumPropertyType>(ass.property->propertyType())) {
					resp.add_drm_obj_property_values(ass.intValue);
					...
					if (logDrmRequests) {
						...
						std::cout << "\t" << ass.property->name() << " -> enum " << enum_name
						          << " (" << ass.intValue << ")" << std::endl;
					}
				} else if (std::holds_alternative<BlobPropertyType>(ass.property->propertyType())) {
					if (ass.blobValue) {
						resp.add_drm_obj_property_values(ass.blobValue->id());
						if (logDrmRequests)
							std::cout << "\t" << ass.property->name() << " -> blob ["
							          << ass.blobValue->id() << "]" << std::endl;
					} else {
						resp.add_drm_obj_property_values(0);
					}
				} else if (std::holds_alternative<ObjectPropertyType>(ass.property->propertyType()
			...
			if (!resp.drm_obj_property_ids_size()) {
				std::cout << "\e[31mcore/drm: No properties found for object [" << req->drm_obj_id()
				          << "]\e[39m" << std::endl;
			}
			...
			if (logDrmRequests) {
				...
				std::cout << "core/drm: GETPROPERTY(" << prop_name << " [" << prop_id << "])"
				          << std::endl;
			}
			...
			if (prop) {
				if (std::holds_alternative<IntPropertyType>(prop->propertyType())) {
					...
					default: {
						std::cout << "core/drm: int property type " << type
						          << " is unhandled by DRM_IOCTL_MODE_GETPROPERTY" << std::endl;
						break;
					}
					}
			...
			if (logDrmRequests)
				std::cout << "core/drm: GETPROPBLOB([" << req->drm_blob_id()
				          << ((!blob) ? "] [invalid]" : "]") << ")" << std::endl;
			...
			if (logDrmRequests)
				std::cout << "core/drm: CREATEPROPBLOB() -> [" << resp.drm_blob_id() << "]"
				          << std::endl;
			...
			if (logDrmRequests)
				std::cout << "core/drm: DESTROYPROPBLOB([" << req->drm_blob_id() << "])"
				          << std::endl;
			...
			if (!self->atomic || req->drm_flags() & ~DRM_MODE_ATOMIC_FLAGS ||
			    ((req->drm_flags() & DRM_MODE_ATOMIC_TEST_ONLY) &&
			     (req->drm_flags() & DRM_MODE_PAGE_FLIP_EVENT))) {
				resp.set_error(managarm::fs::Errors::ILLEGAL_ARGUMENT);
				goto send;
			}
			...
			for (size_t i = 0; i < req->drm_obj_ids_size(); i++) {
				...
				for (size_t j = 0; j < req->drm_prop_counts(i); j++) {
					...
					if (std::holds_alternative<IntPropertyType>(prop_type)) {
						assignments.push_back(Assignment::withInt(mode_obj, prop.get(), value));
						if (logDrmRequests)
							std::cout << "\t\t" << prop->name() << " = " << value << " (int)"
							          << std::endl;
					} else if (std::holds_alternative<EnumPropertyType>(prop_type)) {
						assignments.push_back(Assignment::withInt(mode_obj, prop.get(), value));
						if (logDrmRequests)
							std::cout << "\t\t" << prop->name() << " = " << value << " "
							          << prop->enumInfo().at(value) << " (enum)" << std::endl;
					} else if (std::holds_alternative<BlobPropertyType>(prop_type)) {
						auto blob = self->_device->findBlob(value);
						...
						if (logDrmRequests)
							std::cout << "\t\t" << prop->name() << " = "
							          << (blob ? std::to_string(blob->id()) : "<none>") << " (blob)"
							          << std::endl;
					} else if (std::holds_alternative<ObjectPropertyType>(prop_type)) {
						auto obj = self->_device->findObject(value);
		...
		send:
			...
			if (logDrmRequests)
				std::cout << "core/drm: PRIME_HANDLE_TO_FD(<" << req->drm_prime_handle() << ">)"
				          << std::endl;

			// Extract the credentials of the calling thread in order to locate it in POSIX for
			// attaching the file
			auto [proc_creds] =
			...
			auto file =
			    smarter::make_shared<drm_core::PrimeFile>(bo->getMemory().first, bo->getSize());
			...
			// Request POSIX to register our file as a passthrough file, while giving out a fd we
			// can pass back to our client
			...
			if (logDrmRequests)
				std::cout << "core/drm: PRIME_FD_TO_HANDLE({can't resolve credentials yet})"
				          << std::endl;

			// extract the credentials of the lane that served the PRIME fd, as this is keying our
			// maps that keep track of it
			auto [creds] =
			    co_await helix_ng::exchangeMsgs(conversation, helix_ng::extractCredentials());
			HEL_CHECK(creds.error());

			// 'import' the BufferObject while returning or creating the DRM handle that references
			// it
			...
			std::cout << "\e[31m"
			             "core/drm: Unknown ioctl() with ID "
		...
		std::cout << "\e[31m"
		             "core/drm: Unknown ioctl() message with ID "
		          << id << "\e[39m" << std::endl;
