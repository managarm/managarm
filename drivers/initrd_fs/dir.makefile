...
$c_TARGETS := all-$c clean-$c $($c_BINDIR)/initrd_fs $($c_BINDIR)

.PHONY: all-$c clean-$c

all-$c: $($c_BINDIR)/initrd_fs

clean-$c:
	rm -f $($d_BINDIR)/initrd_fs $($d_OBJECT_PATHS) $($d_OBJECT_PATHS:%.o=%.d)
...
$($c_BINDIR)/initrd_fs: $($c_OBJECT_PATHS) | $($c_BINDIR)
