...
write(void *object, const char *credentials, const void *buffer, size_t length) {
	...
	co_await self->nic->writeCommand(
	    arch::dma_buffer_view{nullptr, const_cast<void *>(buffer), length}
	);
...
read(void *object, const char *credentials, void *buffer, size_t length) {
	(void)credentials;
...
ioctl(void *object, uint32_t id, helix_ng::RecvInlineResult msg, helix::UniqueLane conversation) {
	...
	if (id == managarm::fs::GenericIoctlRequest::message_id) {
		...
		default: {
			std::cout << std::format(
			    "drivers/usb-mbim: unexpected ioctl request 0x{:x}", req->command()
			);
...
async::detached serveDevice(helix::UniqueLane lane, smarter::shared_ptr<CdcWdmDevice> cdc_wdm) {
	while (true) {
		...
		if (req.req_type() == managarm::fs::CntReqType::DEV_OPEN) {
			...
			async::detach(
			    protocols::fs::servePassthrough(std::move(local_lane), cdc_wdm, &fileOperations)
			);
...
async::result<void> UsbMbimNic::initialize() {
	...
	protocols::usb::walkConfiguration(
	    raw_descs,
	    [&](int type, size_t, void *descriptor, const auto &) {
	...
	auto wwan_entity =
	    (co_await mbus_ng::Instance::global().createEntity("wwan", descriptor)).unwrap();

	auto cdc_wdm = smarter::make_shared<CdcWdmDevice>(this);

	[](smarter::shared_ptr<CdcWdmDevice> cdc_wdm,
	...
	auto netClassEntity =
	    (co_await mbus_ng::Instance::global().createEntity("net", netProperties)).unwrap();
	...
	auto usbmiscClassEntity =
	    (co_await mbus_ng::Instance::global().createEntity("usbmisc", usbmiscProperties)).unwrap();
...
async::detached UsbMbimNic::receiveEncapsulated() {
	while (true) {
		...
		ctrl_msg->type = protocols::usb::setup_type::byClass | protocols::usb::setup_type::toHost |
		                 protocols::usb::setup_type::targetInterface;
		...
		auto control = protocols::usb::ControlTransfer{protocols::usb::kXferToHost, ctrl_msg, data};
...
async::detached UsbMbimNic::listenForNotifications() {
	...
	while (true) {
		...
		case Notification::CONNECTION_SPEED_CHANGE: {
			auto change = reinterpret_cast<protocols::usb::CdcConnectionSpeedChange *>(
			    report.subview(sizeof(protocols::usb::CdcNotificationHeader)).data()
			);
...
async::result<size_t> UsbMbimNic::receive(arch::dma_buffer_view frame) {
	...
	auto res =
	    co_await data_in_.transfer(protocols::usb::BulkTransfer{protocols::usb::kXferToHost, buf});
	assert(res);

	if (res.value() != 0) {
		auto ncmHeader = reinterpret_cast<usb_ncm::NcmTransferHeader *>(buf.data());
		auto ndp =
		    reinterpret_cast<usb_ncm::NcmDatagramPointer *>(buf.subview(ncmHeader->wNdpIndex).data()
		    );
...
async::result<void> UsbMbimNic::send(const arch::dma_buffer_view payload) {
	arch::dma_buffer buf{
	    &dmaPool_,
	    sizeof(usb_ncm::NcmTransferHeader) + sizeof(usb_ncm::NcmDatagramPointer) + payload.size()
	};
	auto ncmHeader = reinterpret_cast<usb_ncm::NcmTransferHeader *>(buf.data());
	...
	auto res =
	    co_await data_out_.transfer(protocols::usb::BulkTransfer{protocols::usb::kXferToDevice, buf}
	    );
	assert(res);
	co_return;
}
...
async::result<void> UsbMbimNic::writeCommand(const arch::dma_buffer_view request) {
	arch::dma_object<protocols::usb::SetupPacket> ctrl_msg{&dmaPool_};
	ctrl_msg->type = protocols::usb::setup_type::byClass | protocols::usb::setup_type::toDevice |
	                 protocols::usb::setup_type::targetInterface;
	...
	auto res = co_await device_.transfer(
	    protocols::usb::ControlTransfer{protocols::usb::kXferToDevice, ctrl_msg, request}
	);
	assert(res);
