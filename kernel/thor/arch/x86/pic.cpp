...
enum { kModelLegacy = 1, kModelApic = 2 };
...
LocalApicContext *localApicContext() { return &getCpuData()->apicContext; }
} // namespace

GlobalApicContext *globalApicContext() { return &globalApicContextInstance.get(); }
...
LocalApicContext::LocalApicContext() : _preemptionDeadline{0}, _globalDeadline{0} {}
...
bool LocalApicContext::checkPreemption() { return localApicContext()->_preemptionDeadline != 0; }

void LocalApicContext::handleTimerIrq() {
	assert(localApicContext()->timersAreCalibrated);

	if (debugTimer)
		infoLogger() << "thor [CPU " << getLocalApicId() << "]: Timer IRQ triggered" << frg::endlog;
...
void LocalApicContext::_updateLocalTimer() {
	...
	if (localApicContext()->useTscMode) {
		...
		if (deadline < now) {
			if (debugTimer)
				infoLogger() << "thor [CPU " << getLocalApicId() << "]: Setting single tick timer"
				             << frg::endlog;
			ticks = 1;
		} else {
			if (debugTimer)
			...
			auto of = __builtin_mul_overflow(
			    deadline - now, localApicContext()->localTicksPerMilli, &ticks
			);
			assert(!of);
			ticks /= 1'000'000;
			if (!ticks)
...
void disarmPreemption() { LocalApicContext::setPreemption(0); }

bool preemptionIsArmed() { return LocalApicContext::checkPreemption(); }
...
void initLocalApicPerCpu() {
	...
	auto dumpLocalInt = [&](int index) {
		...
		infoLogger() << "thor: CPU #" << getLocalApicId() << " LINT " << index << " mode is "
		             << (lvt & apicLvtMode) << ", it is "
		             << ((lvt & apicLvtMask) ? "masked" : "not masked") << frg::endlog;
	};

	// Enable the local APIC.
	uint32_t spurious_vector = 0x81;
	picBase.store(lApicSpurious, apicSpuriousVector(spurious_vector) | apicSpuriousSwEnable(true));

	dumpLocalInt(0);
	dumpLocalInt(1);

	if (getGlobalCpuFeatures()->haveInvariantTsc && getGlobalCpuFeatures()->haveTscDeadline)
...
void calibrateApicTimer() {
	...
	if (!localApicContext()->useTscMode) {
		...
		uint32_t elapsed = 0xFFFFFFFF - picBase.load(lApicCurCount);
		picBase.store(lApicInitCount, 0);

		localApicContext()->localTicksPerMilli = elapsed / millis;
		infoLogger() << "thor: Local APIC ticks/ms: " << localApicContext()->localTicksPerMilli
	...
	infoLogger() << "thor: TSC ticks/ms: " << localApicContext()->tscTicksPerMilli << " on CPU #"
...
static initgraph::Task assessTimersTask{
    ...
    [] {
	    ...
	    globalTimerEngine = frg::construct<PrecisionTimerEngine>(
	        *kernelAlloc, globalClockSource, globalApicContext()->globalAlarm()
	    );
	    //			globalClockSource, hpetAlarmTracker);
    }
};

void acknowledgeIpi() { picBase.store(lApicEoi, 0); }
...
void raiseInitDeassertIpi(uint32_t dest_apic_id) {
	if (picBase.isUsingX2apic()) {
		...
		picBase.store(lApicIcrLow, apicIcrLowDelivMode(5) | apicIcrLowTriggerMode(true));
		while (picBase.load(lApicIcrLow) & apicIcrLowDelivStatus) {
			// Wait for IPI delivery.
		}
	}
...
void raiseStartupIpi(uint32_t dest_apic_id, uint32_t page) {
	...
	if (picBase.isUsingX2apic()) {
		...
		picBase.store(lApicIcrHigh, apicIcrHighDestField(dest_apic_id));
		// DM:startup = 6
		picBase.store(lApicIcrLow, apicIcrLowVector(vector) | apicIcrLowDelivMode(6));
		while (picBase.load(lApicIcrLow) & apicIcrLowDelivStatus) {
			// Wait for IPI delivery.
		}
	}
...
struct ApicMsiPin final : MsiPin {
	...
	void sendEoi() override { acknowledgeIrq(0); }

	uint64_t getMessageAddress() override { return 0xFEE00000; }

	uint32_t getMessageData() override { return vector_; }
...
MsiPin *allocateApicMsi(frg::string<KernelAlloc> name) {
	...
	auto pin = frg::construct<ApicMsiPin>(*kernelAlloc, std::move(name), 64 + slotIndex);
...
IoApic::Pin::Pin(IoApic *chip, unsigned int index)
    : IrqPin{buildName(chip->_apicId, index)},
      _chip{chip},
      _index{index} {}
...
void IoApic::Pin::dumpHardwareState() {
	...
	if (_levelTriggered != (word1 & pin_word1::levelTriggered))
		urgentLogger() << "thor: Trigger mode does not match software state!" << frg::endlog;
...
IrqStrategy IoApic::Pin::program(TriggerMode mode, Polarity polarity) {
	...
	if (_vector == -1) {
		...
		for (int i = 0; i < 64; i++) {
			if (!globalIrqSlots[i]->isAvailable())
				continue;
			infoLogger() << "thor: Allocating IRQ slot " << i << " to " << name() << frg::endlog;
	...
	if (_vector == -1)
		panicLogger() << "thor: Could not allocate interrupt vector for " << name() << frg::endlog;

	_chip->_storeRegister(
	    kIoApicInts + _index * 2 + 1, static_cast<uint32_t>(pin_word2::destination(0))
	);
	_chip->_storeRegister(
...
void IoApic::Pin::sendEoi() { acknowledgeIrq(0); }
...
IoApic::IoApic(int apic_id, arch::mem_space space) : _apicId(apic_id), _space{std::move(space)} {
	...
	for (size_t i = 0; i < _numPins; i++) {
		...
		if (!(current & pin_word1::masked))
			infoLogger() << "    Pin " << i << " was not masked by BIOS." << frg::endlog;
...
size_t IoApic::pinCount() { return _numPins; }

IrqPin *IoApic::accessPin(size_t n) { return _pins[n]; }
} // namespace
