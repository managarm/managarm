...
coroutine<frg::expected<Error>> AcpiObject::handleRequest(LaneHandle lane) {
	...
	if (preamble.id() == bragi::message_id<managarm::hw::AcpiGetResourcesRequest>) {
		...
		auto ret = uacpi_for_each_device_resource(
		    node,
		    "_CRS",
		    [](void *ctx, uacpi_resource *res) {
			    ...
			    default:
				    warningLogger()
				        << "thor: unhandled uACPI resource type " << res->type << frg::endlog;
...
static initgraph::Task initAcpiWorkqueueTask{
    &globalInitEngine,
    "acpi.init-acpi-workqueue",
