...
serveRequests(std::shared_ptr<Process> self, std::shared_ptr<Generation> generation) {
	...
	async::cancellation_callback cancel_callback{
	    cancellation, [&] { HEL_CHECK(helShutdownLane(self->posixLane().getHandle())); }
	};

	while (true) {
		auto [accept, recv_head] = co_await helix_ng::exchangeMsgs(
		    self->posixLane(), helix_ng::accept(helix_ng::recvInline())
		);
		...
		auto sendErrorResponse = [&conversation]<typename Message = managarm::posix::SvrResponse>(
		...
		if (preamble.id() == bragi::message_id<managarm::posix::GetPidRequest>) {
			...
			if (req->flags() & WSTOPPED)
				std::cout << "\e[31mposix: WAIT_ID flag WSTOPPED is silently ignored\e[39m"
				          << std::endl;

			if (req->flags() & WCONTINUED)
				std::cout << "\e[31mposix: WAIT_ID flag WCONTINUED is silently ignored\e[39m"
				          << std::endl;

			if (req->flags() & WNOWAIT)
				std::cout << "\e[31mposix: WAIT_ID flag WNOWAIT is silently ignored\e[39m"
				          << std::endl;
			...
			if (req->idtype() == P_PID) {
				...
				std::cout << "\e[31mposix: WAIT_ID idtype other than P_PID and P_ALL are not "
				             "implemented\e[39m"
				          << std::endl;
				co_await sendErrorResponse(managarm::posix::Errors::ILLEGAL_ARGUMENTS);
				continue;
			...
			if (req.flags() & WUNTRACED)
				std::cout << "\e[31mposix: WAIT flag WUNTRACED is silently ignored\e[39m"
				          << std::endl;

			if (req.flags() & WCONTINUED)
				std::cout << "\e[31mposix: WAIT flag WCONTINUED is silently ignored\e[39m"
				          << std::endl;
			...
			if (req->flags() & MAP_ANONYMOUS) {
				...
				if (copyOnWrite) {
					result = co_await self->vmContext()->mapFile(
					    hint, {}, nullptr, 0, req->size(), true, nativeFlags
					);
				} else {
					HelHandle handle;
			...
			if (logRequests)
				std::cout << "posix: MOUNT " << req->fs_type() << " on " << req->path() << " to "
			...
			if (req->flags()) {
				if (req->flags() & AT_SYMLINK_NOFOLLOW) {
					std::cout
					    << "posix: ACCESSAT flag handling AT_SYMLINK_NOFOLLOW is unimplemented"
					    << std::endl;
				} else if (req->flags() & AT_EACCESS) {
					std::cout << "posix: ACCESSAT flag handling AT_EACCESS is unimplemented"
					          << std::endl;
				} else {
					std::cout << "posix: ACCESSAT unknown flag is unimplemented: " << req->flags()
					          << std::endl;
					co_await sendErrorResponse(managarm::posix::Errors::ILLEGAL_ARGUMENTS);
					continue;
				}
			...
			auto pathResult = co_await resolve(
			    self->fsContext()->getRoot(), relative_to, req->path(), self.get()
			);
			if (!pathResult) {
				if (pathResult.error() == protocols::fs::Error::fileNotFound) {
					co_await sendErrorResponse(managarm::posix::Errors::FILE_NOT_FOUND);
			...
			resolver.setup(self->fsContext()->getRoot(), relative_to, req->path(), self.get());
			...
			new_resolver.setup(
			    self->fsContext()->getRoot(), relative_to, req->target_path(), self.get()
			);
			...
			PathResolver resolver;
			resolver.setup(self->fsContext()->getRoot(), relativeTo, req->path(), self.get());
			auto resolveResult = co_await resolver.resolve(resolvePrefix | resolveNoTrailingSlash);
			if (!resolveResult) {
				if (resolveResult.error() == protocols::fs::Error::fileNotFound) {
					co_await sendErrorResponse(managarm::posix::Errors::FILE_NOT_FOUND);
			...
			if (logRequests || logPaths)
				std::cout << "posix: RENAMEAT " << req->path() << " to " << req->target_path()
				          << std::endl;
			...
			auto result = co_await superblock->rename(
			    resolver.currentLink().get(), directory.get(), new_resolver.nextComponent()
			);
			if (!result) {
			...
			// This catches cases where associatedLink is called on a file, but the file doesn't
			// implement that. Instead of blowing up, return ENOENT.
			...
			if (!req->path().size()) {
				...
				if (req->flags() & AT_SYMLINK_NOFOLLOW) {
					std::cout << "posix: AT_SYMLINK_FOLLOW is unimplemented for utimensat"
					          << std::endl;
				}
			...
			co_await target->utimensat(
			    req->atimeSec(), req->atimeNsec(), req->mtimeSec(), req->mtimeNsec()
			);
			...
			auto result =
			    co_await path.second->getTarget()->readSymlink(path.second.get(), self.get());
			if (auto error = std::get_if<Error>(&result); error) {
				assert(*error == Error::illegalOperationTarget);
			...
			if ((req->flags() &
			     ~(managarm::posix::OpenFlags::OF_CREATE |
			       ...
			       managarm::posix::OpenFlags::OF_CLOEXEC | managarm::posix::OpenFlags::OF_TRUNC |
			       managarm::posix::OpenFlags::OF_RDONLY | managarm::posix::OpenFlags::OF_WRONLY |
			       managarm::posix::OpenFlags::OF_RDWR | managarm::posix::OpenFlags::OF_PATH |
			       managarm::posix::OpenFlags::OF_NOCTTY | managarm::posix::OpenFlags::OF_APPEND)
			    )) {
				...
				if (tail) {
					if (req->flags() & managarm::posix::OpenFlags::OF_EXCLUSIVE) {
						...
						auto fileResult = co_await tail->getTarget()->open(
						    resolver.currentView(), std::move(tail), semantic_flags
						);
						assert(fileResult);
					...
					if (chmodResult != Error::success) {
						std::cout << "posix: chmod failed when creating file for OpenAtRequest!"
						          << std::endl;
					...
					auto fileResult = co_await node->open(
					    resolver.currentView(), std::move(link), semantic_flags
					);
					assert(fileResult);
					file = fileResult.value();
					assert(file);
			...
			if (file->isTerminal() && !(req->flags() & managarm::posix::OpenFlags::OF_NOCTTY) &&
				...
				if (!cts) {
					std::cout << "posix: Unable to get controlling terminal (" << (int)cts.error()
					          << ")" << std::endl;
				} else {
			...
			int fd = self->fileContext()->attachFile(
			    file, req->flags() & managarm::posix::OpenFlags::OF_CLOEXEC
			);
			...
			if (req->flags()) {
				if (req->flags() & AT_REMOVEDIR) {
					std::cout << "posix: UNLINKAT flag AT_REMOVEDIR handling unimplemented"
					          << std::endl;
				} else {
					std::cout << "posix: UNLINKAT flag handling unimplemented with unknown flag: "
					          << req->flags() << std::endl;
					co_await sendErrorResponse(managarm::posix::Errors::ILLEGAL_ARGUMENTS);
				}
			}
			...
			if (!resolveResult) {
				if (resolveResult.error() == protocols::fs::Error::isDirectory) {
					// TODO: Only when AT_REMOVEDIR is not specified, fix this when flag handling is
					// implemented.
					co_await sendErrorResponse(managarm::posix::Errors::IS_DIRECTORY);
					continue;
				} else if (resolveResult.error() == protocols::fs::Error::fileNotFound) {
			...
			if (req.flags() & ~(SA_ONSTACK | SA_SIGINFO | SA_RESETHAND | SA_NODEFER | SA_RESTART |
			                    SA_NOCLDSTOP)) {
				std::cout << "\e[31mposix: Unknown SIG_ACTION flags: 0x" << std::hex << req.flags()
			...
			auto r_fd = self->fileContext()->attachFile(std::get<0>(pair), req.flags() & O_CLOEXEC);
			auto w_fd = self->fileContext()->attachFile(std::get<1>(pair), req.flags() & O_CLOEXEC);
			...
			if (req->domain() == AF_UNIX) {
				...
				if (req->protocol() == NETLINK_ROUTE)
					...
					file = netlink::nl_socket::createSocketFile(
					    req->protocol(), req->flags() & SOCK_NONBLOCK
					);
				else {
					std::cout << std::format(
					                 "posix: unhandled netlink protocol 0x{:X}", req->protocol()
					             )
					          << std::endl;
					co_await sendErrorResponse(managarm::posix::Errors::ILLEGAL_ARGUMENTS);
					continue;
				...
				std::cout << "posix: SOCKET: Handle unknown protocols families, this is: "
				          << req->domain() << std::endl;
				co_await sendErrorResponse(managarm::posix::Errors::ILLEGAL_ARGUMENTS);
				continue;
			}

			auto fd = self->fileContext()->attachFile(file, req->flags() & SOCK_CLOEXEC);
			...
			auto fd0 =
			    self->fileContext()->attachFile(std::get<0>(pair), req->flags() & SOCK_CLOEXEC);
			auto fd1 =
			    self->fileContext()->attachFile(std::get<1>(pair), req->flags() & SOCK_CLOEXEC);
			...
			for (size_t i = 0; i < req.fds_size(); i++) {
				...
				unsigned int mask = 0;
				if (req.events(i) & POLLIN)
					mask |= EPOLLIN;
				if (req.events(i) & POLLOUT)
					mask |= EPOLLOUT;
				if (req.events(i) & POLLWRNORM)
					mask |= EPOLLOUT;
				if (req.events(i) & POLLPRI)
					mask |= EPOLLPRI;
				if (req.events(i) & POLLRDHUP)
					mask |= EPOLLRDHUP;
				if (req.events(i) & POLLERR)
					mask |= EPOLLERR;
				if (req.events(i) & POLLHUP)
					mask |= EPOLLHUP;

				// addItem() can fail with EEXIST but we check for duplicate FDs above
				// so that cannot happen here.
				Error ret = epoll::addItem(
				    epfile.get(), self.get(), std::move(locked), req.fds(i), mask, req.fds(i)
				);
			...
			size_t k;
			if (req.timeout() < 0) {
				k = co_await epoll::wait(epfile.get(), events, std::min(req.size(), uint32_t(16)));
			} else if (!req.timeout()) {
				// Do not bother to set up a timer for zero timeouts.
				async::cancellation_event cancel_wait;
				cancel_wait.cancel();
				k = co_await epoll::wait(
				    epfile.get(), events, std::min(req.size(), uint32_t(16)), cancel_wait
				);
			} else {
				assert(req.timeout() > 0);
				async::cancellation_event cancel_wait;
			...
			auto file = createSignalFile(
			    req.sigset(), req.flags() & managarm::posix::OpenFlags::OF_NONBLOCK
			);
			auto fd = self->fileContext()->attachFile(
			    file, req.flags() & managarm::posix::OpenFlags::OF_CLOEXEC
			);
			...
			auto wd =
			    inotify::addWatch(ifile.get(), resolver.currentLink()->getTarget(), req->flags());
			...
			if (req->flags() &
			    ~(managarm::posix::EventFdFlags::CLOEXEC | managarm::posix::EventFdFlags::NONBLOCK |
				...
				auto fd = self->fileContext()->attachFile(
				    file, req->flags() & managarm::posix::EventFdFlags::CLOEXEC
				);
			...
			if (logRequests || logPaths)
				std::cout << "posix: MKNODAT for path " << req->path() << " with mode "
				          << req->mode() << " and device " << req->device() << std::endl;
			...
			if (type == VfsType::charDevice || type == VfsType::blockDevice) {
				...
				std::cout
				    << "\e[31mposix: Creating regular files with mknod is not supported.\e[39m"
				    << std::endl;
				co_await sendErrorResponse(managarm::posix::Errors::ILLEGAL_ARGUMENTS);
				continue;
			}
			...
			// If pgid is 0, we're going to set it to the calling process's pid, use the target
			// group.
			if (req->pgid()) {
			...
			if (group) {
				...
				// Not found, making it if pgid and pid match, or if pgid is 0, indicating that we
				// should make one
			...
			auto memFile = smarter::make_shared<MemoryFile>(
			    nullptr, link, (req->flags() & MFD_ALLOW_SEALING) == true
			);
			...
			if (e == kHelErrIllegalArgs) {
				...
				std::cout << "posix: SET_AFFINITY hel call returned unexpected error: " << e
				          << std::endl;
				co_await sendErrorResponse(managarm::posix::Errors::INTERNAL_ERROR);
				continue;
			}
			...
			if (e == kHelErrBufferTooSmall) {
				...
				std::cout << "posix: GET_AFFINITY hel call returned unexpected error: " << e
				          << std::endl;
				co_await sendErrorResponse(managarm::posix::Errors::INTERNAL_ERROR);
				continue;
			}
			...
			auto req =
			    bragi::parse_head_only<managarm::posix::GetMemoryInformationRequest>(recv_head);
			...
			auto kernResp =
			    bragi::parse_head_only<managarm::kerncfg::GetMemoryInformationResponse>(kerncfgResp
			    );
			...
			if (req->num() == _SC_NPROCESSORS_CONF || req->num() == _SC_NPROCESSORS_ONLN) {
				...
				auto kernResp =
				    bragi::parse_head_only<managarm::kerncfg::GetNumCpuResponse>(kerncfgResp);
