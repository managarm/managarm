...
async::detached Controller::handleIrqs_() {
	...
	while (true) {
		...
		if (intStatus) {
			...
			if (useMsis_) {
				// In theory, we shouldn't get here, but refer to xhci handleMsis() comment for
				// details.
				HEL_CHECK(helAcknowledgeIrq(irq_.getHandle(), kHelAckAcknowledge, irqSequence_));
			} else {
				for (auto &port : activePorts_) {
					...
					port->dumpState();
					port->checkErrors();
					std::cout << "\tglobal IS: " << regs_.load(regs::interruptStatus)
					          << ", masked GIS " << (regs_.load(regs::interruptStatus) & portsImpl_)
					          << ", read GIS " << intStatus << "\n";
				}
...
async::result<bool> Controller::initPorts_(size_t numCommandSlots, bool ss) {
	for (int i = 0; i < maxPorts_; i++) {
		if (portsImpl_ & (1 << i)) {
			...
			auto port =
			    std::make_unique<Port>(parentId_, i, numCommandSlots, ss, regs_.subspace(offset));
