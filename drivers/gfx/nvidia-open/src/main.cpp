...
uint8_t GfxDevice::getNvidiaBarIndex(uint8_t nv_bar_index) {
	...
	std::println("gfx/nvidia-open: attempted to resolve invalid nv_bar_index {}",  nv_bar_index);
	assert(!"invalid nv_bar_index");
}
...
async::result<void> GfxDevice::handleIrqs() {
	...
	while(true) {
		...
		[[maybe_unused]] bool rm_handled = rm_isr(nullptr, &nv_, &need_to_run_bottom_half_gpu_lock_held);
...
void GfxDevice::setupCrtcAndPlanes(NvKmsKapiDeviceResourcesInfo &resInfo) {
	...
	for(size_t i = 0; i <= 5; i++)
		modifiers.push_back(DRM_FORMAT_MOD_NVIDIA_BLOCK_LINEAR_2D(0, 1, gen, resInfo.caps.genericPageKind, 5 - i));
	...
	for(size_t i = 0; i < resInfo.numHeads; i++) {
		...
		auto primaryPlane = std::make_shared<Plane>(this, drm_core::Plane::PlaneType::PRIMARY, NVKMS_KAPI_LAYER_PRIMARY_IDX);
		...
		std::println("gfx/nvidia-open: layer {} supported rotations bitmap 0x{:x}", i, supported_rotations);
		...
		if(formats.size() && modifiers.size()) {
			...
			size_t inFormatBlobSize = sizeof(drm_format_modifier_blob) + formatsSize + modifiersSize;
			...
			drm_format_modifier_blob blobHead{
				...
				.modifiers_offset = static_cast<uint32_t>(sizeof(drm_format_modifier_blob) + formatsSize),
			};
			...
			for(size_t i = 0; i < modifiers.size(); i++) {
				...
				memcpy(inFormat.data() + sizeof(blobHead) + formatsSize + (sizeof(drm_format_modifier) * i),
					&mod, sizeof(mod));
			}
...
void GfxDevice::setupConnectorsAndEncoders() {
	...
	auto hDisplays = reinterpret_cast<NvKmsKapiDisplay *>(
		calloc(nDisplays, sizeof(NvKmsKapiDisplay)));
	...
	for(size_t i = 0; i < nDisplays; i++) {
		...
		auto connectorInfo = reinterpret_cast<NvKmsKapiConnectorInfo *>(
			calloc(1, sizeof(NvKmsKapiConnectorInfo)));
		...
		auto connector = Connector::find(this, connectorInfo->physicalIndex,
			connectorInfo->type, displayInfo->internal, displayInfo->dpAddress);
...
async::result<void> GfxDevice::initialize() {
	...
	for(size_t i = 0; i < 6; i++) {\
		...
		if(info_.barInfo[i].hostType == protocols::hw::kIoTypeMemory) {
			...
			mmioRanges.insert({{info_.barInfo[i].address, info_.barInfo[i].length}, co_await hwDevice_.accessBar(i)});
		}
	}
...
async::result<void> bindController(mbus_ng::Entity hwEntity, uint32_t seg, uint32_t bus, uint32_t dev, uint32_t func) {
	...
	auto gfxEntity = (co_await mbus_ng::Instance::global().createEntity(
		"gfx_nvidia_open", descriptor)).unwrap();
...
async::result<protocols::svrctl::Error> bindDevice(int64_t base_id) {
	...
	auto pciSegment = std::stoul(std::get<mbus_ng::StringItem>(properties["pci-segment"]).value, nullptr, 16);
	auto pciBus = std::stoul(std::get<mbus_ng::StringItem>(properties["pci-bus"]).value, nullptr, 16);
	auto pciSlot = std::stoul(std::get<mbus_ng::StringItem>(properties["pci-slot"]).value, nullptr, 16);
	auto pciFunction = std::stoul(std::get<mbus_ng::StringItem>(properties["pci-function"]).value, nullptr, 16);
