...
void * NV_API_CALL os_pci_init_handle(NvU32 domain [[maybe_unused]], NvU8 bus [[maybe_unused]], NvU8 slot [[maybe_unused]], NvU8 func [[maybe_unused]], NvU16 *vendor [[maybe_unused]], NvU16 *dev [[maybe_unused]]) {
    return nullptr;
}
...
void * NV_API_CALL os_map_kernel_space(NvU64 start, NvU64 size_bytes, NvU32 mode [[maybe_unused]]) {
    assert(size_bytes);
	...
	HEL_CHECK(helMapMemory(memory.getHandle(), kHelNullHandle, nullptr,
        alignedOffset, alignedSize, kHelMapProtRead | kHelMapProtWrite,
        &window));
    assert(window);
...
NvBool NV_API_CALL os_check_access(RsAccessRight) {
    return NV_FALSE;
}
...
NV_STATUS   NV_API_CALL  os_find_ns_pid(void *pid_info [[maybe_unused]], NvU32 *ns_pid [[maybe_unused]]) STUBBED;
...
void *NV_API_CALL nv_alloc_kernel_mapping(nv_state_t *, void *pAllocPrivate,
        NvU64 pageIndex, NvU32 pageOffset, NvU64 size, void **pPrivate) {
    auto info = reinterpret_cast<page_info *>(pAllocPrivate);

    assert(size);
    size_t pages = (size + 0xFFF) / 0x1000;

    void *ptr = nullptr;
    HEL_CHECK(helMapMemory(info->handle, kHelNullHandle,
            nullptr, pageIndex << 12, pages << 12,
            kHelMapProtRead | kHelMapProtWrite, &ptr));
...
NV_STATUS NV_API_CALL nv_free_kernel_mapping(nv_state_t *, void *pAllocPrivate, void *address, void *pPrivate) {
    auto info [[maybe_unused]] = reinterpret_cast<page_info *>(pAllocPrivate);
    ...
    HEL_CHECK(helUnmapMemory(kHelNullHandle, reinterpret_cast<void *>(alignedStart), alignedEnd - alignedStart));

    return NV_OK;
}

NV_STATUS  NV_API_CALL  nv_alloc_user_mapping    (nv_state_t *, void *, NvU64, NvU32, NvU64, NvU32, NvU64 *, void **) STUBBED;
NV_STATUS  NV_API_CALL  nv_free_user_mapping     (nv_state_t *, void *, NvU64, void *) STUBBED;
NV_STATUS  NV_API_CALL  nv_add_mapping_context_to_file (nv_state_t *, nv_usermap_access_params_t*, NvU32, void *, NvU64, NvU32) STUBBED;
...
NV_STATUS  NV_API_CALL  nv_alias_pages           (nv_state_t *, NvU32, NvU64, NvU32, NvU32, NvU64, NvU64 *, NvBool, void **) STUBBED;

NV_STATUS NV_API_CALL nv_alloc_pages(nv_state_t *, NvU32 page_count, NvU64 page_size,
        NvBool contiguous, NvU32 cache_type [[maybe_unused]], NvBool zeroed, NvBool unencrypted [[maybe_unused]],
        NvS32 node_id, NvU64 *pte_array, void **priv_data) {
    assert(node_id == -1);
    ...
    auto info = new page_info{handle, page_count};
    *reinterpret_cast<page_info **>(priv_data) = info;
...
NV_STATUS NV_API_CALL nv_free_pages(nv_state_t *, NvU32 page_count, NvBool contiguous, NvU32 cache_type, void *priv_data) {
    auto info = reinterpret_cast<page_info *>(priv_data);
    (void) contiguous;
    (void) cache_type;
...
NV_STATUS  NV_API_CALL  nv_register_user_pages   (nv_state_t *, NvU64, NvU64 *, void *, void **, NvBool) STUBBED;
...
NV_STATUS NV_API_CALL   nv_register_sgt          (nv_state_t *, NvU64 *, NvU64, NvU32, void **, struct sg_table *, void *) STUBBED;
...
NV_STATUS  NV_API_CALL  nv_dma_map_peer          (nv_dma_device_t *, nv_dma_device_t *, NvU8, NvU64, NvU64 *) STUBBED;
...
void      NV_API_CALL nv_get_updated_emu_seg(NvU32 *start [[maybe_unused]], NvU32 *end [[maybe_unused]]) STUBBED;

void NV_API_CALL nv_get_screen_info(nv_state_t *nv [[maybe_unused]], NvU64 *pPhysicalAddress,
        NvU32 *pFbWidth, NvU32 *pFbHeight, NvU32 *pFbDepth, NvU32 *pFbPitch, NvU64 *pFbSize) {
    *pPhysicalAddress = 0;
    *pFbWidth = 0;
    *pFbHeight = 0;
    *pFbDepth = 0;
...
NV_STATUS NV_API_CALL nv_dma_import_sgt  (nv_dma_device_t *, struct sg_table *, struct drm_gem_object *) STUBBED;
void NV_API_CALL nv_dma_release_sgt(struct sg_table *, struct drm_gem_object *) STUBBED;
NV_STATUS NV_API_CALL nv_dma_import_dma_buf      (nv_dma_device_t *, struct dma_buf *, NvU32 *, struct sg_table **, nv_dma_buf_t **) STUBBED;
NV_STATUS NV_API_CALL nv_dma_import_from_fd      (nv_dma_device_t *, NvS32, NvU32 *, struct sg_table **, nv_dma_buf_t **) STUBBED;
...
void NV_API_CALL nv_get_disp_smmu_stream_ids(nv_state_t *, NvU32 *dispIsoStreamId, NvU32 *dispNisoStreamId) {
...
void       NV_API_CALL  nv_flush_coherent_cpu_cache_range(nv_state_t *nv [[maybe_unused]], NvU64 cpu_virtual [[maybe_unused]], NvU64 size [[maybe_unused]]) STUBBED;
...
void        NV_API_CALL nv_create_nano_timer(nv_state_t *, void *pTmrEvent [[maybe_unused]], nv_nano_timer_t **) STUBBED;
void        NV_API_CALL nv_start_nano_timer(nv_state_t *nv [[maybe_unused]], nv_nano_timer_t *, NvU64 timens [[maybe_unused]]) STUBBED;
...
NV_STATUS nv_get_syncpoint_aperture(NvU32 syncpointId [[maybe_unused]], NvU64 *physAddr [[maybe_unused]], NvU64 *limit [[maybe_unused]], NvU32 *offset [[maybe_unused]]) {
    return NV_ERR_NOT_SUPPORTED;
}
