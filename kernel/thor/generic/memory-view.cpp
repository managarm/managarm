		...
		KernelFiber::run([=, this] {
			while (true) {
				if (logUncaching) {
					auto irqLock = frg::guard(&irqMutex());
					auto lock = frg::guard(&_mutex);
					infoLogger() << "thor: " << (_cachedSize / 1024) << " KiB of cached pages"
					             << frg::endlog;
				}
...
void
MemoryView::fork(async::any_receiver<frg::tuple<Error, smarter::shared_ptr<MemoryView>>> receiver) {
...
coroutine<frg::expected<Error>> MemoryView::copyTo(
    uintptr_t offset, const void *pointer, size_t size, smarter::shared_ptr<WorkQueue> wq
) {
	struct Node {
		MemoryView *view;
		uintptr_t offset;
...
coroutine<frg::expected<Error>> MemoryView::copyFrom(
    uintptr_t offset, void *pointer, size_t size, smarter::shared_ptr<WorkQueue> wq
) {
	struct Node {
		MemoryView *view;
		uintptr_t offset;
...
bool MemoryView::asyncLockRange(
    uintptr_t offset, size_t size, smarter::shared_ptr<WorkQueue>, LockRangeNode *node
) {
...
coroutine<frg::expected<Error>> MemoryView::touchRange(
    uintptr_t offset, size_t size, FetchFlags flags, smarter::shared_ptr<WorkQueue> wq
) {
	size_t progress = 0;
	while (progress < size) {
...
Error MemoryView::updateRange(ManageRequest, size_t, size_t) { return Error::illegalObject; }

void MemoryView::submitManage(ManageNode *) {
	panicLogger() << "MemoryView does not support management!" << frg::endlog;
}

Error MemoryView::setIndirection(size_t, smarter::shared_ptr<MemoryView>, uintptr_t, size_t) {
...
struct ZeroMemory final : MemoryView, GlobalFutexSpace {
	...
	Error lockRange(uintptr_t, size_t) override { return Error::success; }
	...
	takeGlobalFutex(uintptr_t, smarter::shared_ptr<WorkQueue>) override {
...
Error ImmediateMemory::lockRange(uintptr_t, size_t) { return Error::success; }
...
coroutine<frg::expected<Error, PhysicalAddr>>
ImmediateMemory::takeGlobalFutex(uintptr_t offset, smarter::shared_ptr<WorkQueue>) {
	auto index = offset >> kPageShift;
	if (index >= _physicalPages.size())
		co_return Error::fault;
...
size_t HardwareMemory::getLength() { return _length; }
...
AllocatedMemory::AllocatedMemory(
    size_t desiredLngth, int addressBits, size_t desiredChunkSize, size_t chunkAlign
)
	...
	if (length != desiredLngth)
		urgentLogger() << "Memory length " << (void *)desiredLngth << " rounded up to chunk size "
		               << (void *)_chunkSize << frg::endlog;
...
AllocatedMemory::~AllocatedMemory() {
	...
	if (logUsage)
		infoLogger() << "thor:     (" << (physicalAllocator->numUsedPages() * 4) << " KiB in use)"
		             << frg::endlog;
}
...
frg::tuple<PhysicalAddr, CachingMode> AllocatedMemory::peekRange(uintptr_t offset) {
	...
	return frg::tuple<PhysicalAddr, CachingMode>{_physicalChunks[index] + disp, CachingMode::null};
}
...
coroutine<frg::expected<Error, PhysicalAddr>>
AllocatedMemory::takeGlobalFutex(uintptr_t offset, smarter::shared_ptr<WorkQueue> wq) {
...
void ManagedSpace::_progressManagement(ManageList &pending) {
	...
	while (!_writebackList.empty() && !_managementQueue.empty()) {
		...
		node->setup(
		    Error::success, ManageRequest::writeback, index << kPageShift, count << kPageShift
		);
	...
	while (!_initializationList.empty() && !_managementQueue.empty()) {
		...
		auto node = _managementQueue.pop_front();
		node->setup(
		    Error::success, ManageRequest::initialize, index << kPageShift, count << kPageShift
		);
		pending.push_back(node);
	}
}
...
void ManagedSpace::_progressMonitors(MonitorList &pending) {
	...
	auto progressNode = [&](MonitorNode *node) -> bool {
		while (node->progress < node->length) {
			...
			auto pit = pages.find(index);
			assert(pit);
			if (pit->loadState == kStateMissing || pit->loadState == kStateWantInitialization ||
			    pit->loadState == kStateInitialization)
				return false;

			assert(
			    pit->loadState == kStatePresent || pit->loadState == kStateWantWriteback ||
			    pit->loadState == kStateWriteback || pit->loadState == kStateAnotherWriteback ||
			    pit->loadState == kStateEvicting
			);
	    ...
	    [](BackingMemory *self, size_t newPages, async::any_receiver<void> receiver
		    ...
		    if (newPages > self->_managed->numPages) {
			    ...
			    co_await self->_managed->_evictQueue.evictRange(
			        newPages << kPageShift, oldPages << kPageShift
			    );
		    }
...
void BackingMemory::submitManage(ManageNode *node) { _managed->submitManagement(node); }
...
Error BackingMemory::updateRange(ManageRequest type, size_t offset, size_t length) {
	...
	MonitorList pending;
	{
		...
		/*	assert(length == kPageSize);
		    auto inspect = (unsigned char *)physicalToVirtual(_managed->physicalPages[offset /
		   kPageSize]); auto log = infoLogger() << "dump"; for(size_t b = 0; b < kPageSize; b += 16)
		   { log << frg::hex_fmt(offset + b) << "   "; for(size_t i = 0; i < 16; i++) log << " " <<
		   frg::hex_fmt(inspect[b + i]); log << "\n";
		    }
		    log << frg::endlog;*/
		...
		if (_managed->readahead)
			for (size_t i = 1; i < 4; ++i) {
				...
				auto [pit, wasInserted] =
				    _managed->pages.find_or_insert(index + i, _managed.get(), index + i);
				assert(pit);
				if (pit->loadState == ManagedSpace::kStateMissing) {
					pit->loadState = ManagedSpace::kStateWantInitialization;
					_managed->_initializationList.push_back(&pit->cachePage);
...
Error IndirectMemory::lockRange(uintptr_t offset, size_t size) {
	...
	if (inSlotOffset + size > indirections_[slot]->size)
		return Error::fault;
	return indirections_[slot]->memory->lockRange(indirections_[slot]->offset + inSlotOffset, size);
}
...
void IndirectMemory::unlockRange(uintptr_t offset, size_t size) {
	...
	return indirections_[slot]->memory->unlockRange(
	    indirections_[slot]->offset + inSlotOffset, size
	);
}
...
frg::tuple<PhysicalAddr, CachingMode> IndirectMemory::peekRange(uintptr_t offset) {
	...
	assert(slot < indirections_.size()); // TODO: Return Error::fault.
	assert(indirections_[slot]);         // TODO: Return Error::fault.
	return indirections_[slot]->memory->peekRange(indirections_[slot]->offset + inSlotOffset);
}
...
IndirectMemory::fetchRange(uintptr_t offset, FetchFlags flags, smarter::shared_ptr<WorkQueue> wq) {
	...
	return indirections_[slot]->memory->fetchRange(
	    indirections_[slot]->offset + inSlotOffset, flags, std::move(wq)
	);
}
...
void IndirectMemory::markDirty(uintptr_t offset, size_t size) {
	...
	assert(indirections_[slot]);                              // TODO: Return Error::fault.
	assert(inSlotOffset + size <= indirections_[slot]->size); // TODO: Return Error::fault.
	indirections_[slot]->memory->markDirty(indirections_[slot]->offset + inSlotOffset, size);
}

size_t IndirectMemory::getLength() { return indirections_.size() << 32; }
...
size_t CopyOnWriteMemory::getLength() { return _length; }
	       ...
	       async::any_receiver<frg::tuple<Error, smarter::shared_ptr<MemoryView>>> receiver
		    ...
		    auto doCopyOnePage = [&](size_t pg, CowPage *page) {
			    ...
			    if (page->lockCount /*|| disableCow */) {
				    ...
				    auto newIt =
				        newChain->_pages.insert(pageOffset >> kPageShift, PhysicalAddr(-1));
...

		    {
			    ...
			    forked = smarter::allocate_shared<CopyOnWriteMemory>(
			        *kernelAlloc, self->_view, self->_viewOffset, self->_length, newChain
			    );
...
Error CopyOnWriteMemory::lockRange(uintptr_t, size_t) {
	panicLogger() << "CopyOnWriteMemory does not support synchronous lockRange()" << frg::endlog;
	__builtin_unreachable();
}
		    ...
		    while (progress < size) {
			    ...
			    if (!chain) {
				    ...
				    auto copyOutcome = co_await view->copyFrom(
				        pageOffset & ~(kPageSize - 1), accessor.get(), kPageSize, wq
				    );
				    assert(copyOutcome);
			    }
...
CopyOnWriteMemory::fetchRange(uintptr_t offset, FetchFlags, smarter::shared_ptr<WorkQueue> wq) {
	...
	if (!chain) {
		FRG_CO_TRY(
		    co_await view->copyFrom(pageOffset & ~(kPageSize - 1), accessor.get(), kPageSize, wq)
		);
	}
...
FutexRealm *getGlobalFutexRealm() { return &globalFutexRealm.get(); }
