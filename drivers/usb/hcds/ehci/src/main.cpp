...
arch::dma_pool *DeviceState::setupPool() { return &schedulePool; }

arch::dma_pool *DeviceState::bufferPool() { return &schedulePool; }
...
DeviceState::configurationDescriptor(uint8_t configuration) {
...
DeviceState::useConfiguration(uint8_t index, uint8_t value) {
	FRG_CO_TRY(co_await _controller->useConfiguration(_device, value));
	co_return proto::Configuration{
	    std::make_shared<ConfigurationState>(_controller, _device, index, value)
	};
}

async::result<frg::expected<proto::UsbError, size_t>>
DeviceState::transfer(proto::ControlTransfer info) {
...
InterfaceState::InterfaceState(std::shared_ptr<Controller> controller, int device, int interface)
...
InterfaceState::getEndpoint(proto::PipeType type, int number) {
	co_return proto::Endpoint{std::make_shared<EndpointState>(_controller, _device, type, number)};
}
		...
		if (sc & portsc::connectChange) {
			...
			if (sc & portsc::connectStatus) {
				if ((sc & portsc::lineStatus) == 1) { // K-state: Low-speed device
					...
					// release the ownership of the port to the companion controller, as required by
					// spec see EHCI spec rev 1.0, p. 28
...
Controller::enumerateDevice(std::shared_ptr<proto::Hub> hub, int port, proto::DeviceSpeed speed) {
	...
	(co_await _directTransfer(
	     proto::ControlTransfer{proto::kXferToDevice, set_address, arch::dma_buffer_view{}},
	     queue,
	     0
	 ))
	    .unwrap();
	...
	get_header->type =
	    proto::setup_type::targetDevice | proto::setup_type::byStandard | proto::setup_type::toHost;
	...
	(co_await _directTransfer(
	     proto::ControlTransfer{
	         proto::kXferToHost, get_header, descriptor.view_buffer().subview(0, 8)
	     },
	     queue,
	     8
	 ))
	...
	(co_await transfer(
	     address,
	     0,
	     proto::ControlTransfer{proto::kXferToHost, get_descriptor, descriptor.view_buffer()}
	 ))
	    .unwrap();
	...
	auto usbEntity = (co_await mbus_ng::Instance::global().createEntity(
	                      "usb-ehci-dev-" + std::string{name}, mbusDescriptor
	                  ))
	                     .unwrap();
...
async::result<frg::expected<proto::UsbError, std::string>> Controller::deviceDescriptor(int address
) {
	arch::dma_object<proto::SetupPacket> get_header{&schedulePool};
	get_header->type =
	    proto::setup_type::targetDevice | proto::setup_type::byStandard | proto::setup_type::toHost;
...
Controller::configurationDescriptor(int address, uint8_t configuration) {
	...
	FRG_CO_TRY(co_await transfer(
	    address, 0, proto::ControlTransfer{proto::kXferToHost, get_header, header.view_buffer()}
	));
	...
	FRG_CO_TRY(co_await transfer(
	    address, 0, proto::ControlTransfer{proto::kXferToHost, get_descriptor, descriptor}
	));
...
Controller::useConfiguration(int address, int configuration) {
	...
	FRG_CO_TRY(co_await transfer(
	    address,
	    0,
	    proto::ControlTransfer{proto::kXferToDevice, set_config, arch::dma_buffer_view{}}
	));
	co_return {};
}

async::result<frg::expected<proto::UsbError>> Controller::useInterface(
	...
	if (*valueByIndex != configValue) {
		printf(
		    "ehci: useConfiguration(%u, %u) called, but that configuration has bConfigurationValue "
		    "= %u???\n",
		    configIndex,
		    configValue,
		    *valueByIndex
...
bool Controller::QueueEntity::getReclaim() { return head->flags.load() & qh_flags::reclaimHead; }
...
Controller::transfer(int address, int pipe, proto::ControlTransfer info) {
	...
	auto transaction = _buildControl(info.flags, info.setup, info.buffer, endpoint->maxPacketSize);
...
async::result<frg::expected<proto::UsbError, size_t>>
Controller::transfer(int address, proto::PipeType type, int pipe, proto::InterruptTransfer info) {
	...
	auto transaction = _buildInterruptOrBulk(
	    info.flags, info.buffer, endpoint->maxPacketSize, info.lazyNotification
	);
	auto future = transaction->promise.get_future();
	_linkTransaction(endpoint->queueEntity, transaction);
	co_return *(co_await future.get());
...
async::result<frg::expected<proto::UsbError, size_t>>
Controller::transfer(int address, proto::PipeType type, int pipe, proto::BulkTransfer info) {
	// TODO: Ensure pipe type matches transfer direction.
	auto device = &_activeDevices[address];
	EndpointSlot *endpoint;
	if (type == proto::PipeType::in) {
...
auto Controller::_buildControl(
	...
	for (size_t i = 0; i < num_data; i++) {
		...
		transfers[i + 1].status.store(
		    td_status::active(true) | td_status::pidCode(dir == proto::kXferToDevice ? 0 : 1) |
	...
	transfers[num_data + 1].status.store(
	    td_status::active(true) | td_status::pidCode(dir == proto::kXferToDevice ? 1 : 0) |
...
async::result<frg::expected<proto::UsbError, size_t>> Controller::_directTransfer(
    proto::ControlTransfer info, QueueEntity *queue, size_t max_packet_size
) {
	auto transaction = _buildControl(info.flags, info.setup, info.buffer, max_packet_size);
	auto future = transaction->promise.get_future();
...
async::result<frg::expected<proto::UsbError, proto::DeviceSpeed>> Controller::resetPort(int number
) {
	auto offset = _space.load(cap_regs::caplength);
...
void Controller::_dump(QueueEntity *entity) {
	...
	std::cout << "    pingError: " << (int)(entity->head->status.load() & qh_status::pingError)
	          << std::endl;
	std::cout << "    splitXState: " << (int)(entity->head->status.load() & qh_status::splitXState)
	          << std::endl;
	std::cout << "    missedFrame: " << (int)(entity->head->status.load() & qh_status::missedFrame)
	          << std::endl;
	...
	std::cout << "    interruptOnComplete: "
	          << (int)(entity->head->status.load() & qh_status::interruptOnComplete) << std::endl;
	std::cout << "    totalBytes: " << (int)(entity->head->status.load() & qh_status::totalBytes)
	          << std::endl;
	std::cout << "    dataToggle: " << (int)(entity->head->status.load() & qh_status::dataToggle)
	          << std::endl;
	...
	for (size_t i = 0; i < active->transfers.size(); i++) {
		...
		std::cout << "    pingError: " << (int)(transfer.status.load() & td_status::pingError)
		          << std::endl;
		std::cout << "    splitXState: " << (int)(transfer.status.load() & td_status::splitXState)
		          << std::endl;
		std::cout << "    missedFrame: " << (int)(transfer.status.load() & td_status::missedFrame)
		          << std::endl;
		std::cout << "    transactionError: "
		          << (int)(transfer.status.load() & td_status::transactionError) << std::endl;
		std::cout << "    babbleDetected: "
		          << (int)(transfer.status.load() & td_status::babbleDetected) << std::endl;
		std::cout << "    dataBufferError: "
		          << (int)(transfer.status.load() & td_status::dataBufferError) << std::endl;
		std::cout << "    halted: " << (int)(transfer.status.load() & td_status::halted)
		          << std::endl;
		std::cout << "    pidCode: " << (int)(transfer.status.load() & td_status::pidCode)
		          << std::endl;
		std::cout << "    errorCounter: " << (int)(transfer.status.load() & td_status::errorCounter)
		          << std::endl;
		std::cout << "    cPage: " << (int)(transfer.status.load() & td_status::cPage) << std::endl;
		std::cout << "    interruptOnComplete: "
		          << (int)(transfer.status.load() & td_status::interruptOnComplete) << std::endl;
		std::cout << "    totalBytes: " << (int)(transfer.status.load() & td_status::totalBytes)
		          << std::endl;
		std::cout << "    dataToggle: " << (int)(transfer.status.load() & td_status::dataToggle)
		          << std::endl;
	}
}
...
Controller::RootHub::RootHub(Controller *controller) : Hub{nullptr, 0}, _controller{controller} {
	for (int i = 0; i < _controller->_numPorts; i++) {
		_ports.push_back(std::make_unique<Port>());
	}
}

size_t Controller::RootHub::numPorts() { return _ports.size(); }
...
async::detached bindController(mbus_ng::Entity entity) {
	...
	auto ehciEntity =
	    (co_await mbus_ng::Instance::global().createEntity("ehci-controller", descriptor)).unwrap();

	auto controller = std::make_shared<Controller>(
	    std::move(device), std::move(ehciEntity), std::move(mapping), std::move(bar), std::move(irq)
	);
