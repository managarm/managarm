...
bool IpcQueue::validSize(size_t size) { return sizeof(ElementStruct) + size <= _chunkSize; }
	...
	while (true) {
		...
		while (true) {
			...
			co_await getGlobalFutexRealm()->wait(
			    _memory->getImmediateFutex(hfOffset), _currentIndex | kHeadWaiters
			);
		}
		...
		while (true) {
			...
			if (progress + length <= _chunkSize) {
				...
				_memory->writeImmediate(
				    chunkOffset + elementOffset, &element, sizeof(ElementStruct)
				);

				size_t sgOffset = sizeof(ElementStruct);
				for (auto sgSource = node->_source; sgSource; sgSource = sgSource->link) {
					_memory->writeImmediate(
					    chunkOffset + elementOffset + sgOffset, sgSource->pointer, sgSource->size
					);
			...
			auto progressFutexWord =
			    __atomic_exchange_n(&chunkHead->progressFutex, newProgressWord, __ATOMIC_RELEASE);
