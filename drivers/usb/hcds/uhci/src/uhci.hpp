...
enum class Packet : uint8_t { in = 0x69, out = 0xE1, setup = 0x2D };
...
struct FrameListPointer {
	...
	FrameListPointer(uint32_t pointer, bool is_queue) : _bits{pointer | (is_queue << QhSelectBit)} {
		assert(pointer % 16 == 0);
	}
