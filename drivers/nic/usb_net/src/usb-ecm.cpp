...
async::result<size_t> UsbEcmNic::receive(arch::dma_buffer_view frame) {
	while (true) {
		auto res = co_await data_in_.transfer(
		    protocols::usb::BulkTransfer{protocols::usb::kXferToHost, frame}
		);
		assert(res);
...
async::result<void> UsbEcmNic::send(const arch::dma_buffer_view payload) {
	auto res = co_await data_out_.transfer(
	    protocols::usb::BulkTransfer{protocols::usb::kXferToDevice, payload}
	);
	assert(res);
	co_return;
}
