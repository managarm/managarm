...
static const struct { const char *name; } rtl_chip_infos[] = {
	// PCI Devices
...
void RealtekNic::determineMacRevision() {
	...
	static const struct rtl_mac_info { uint16_t mask; uint16_t val; MacRevision ver; } mac_info[] = {
		// 8125B family.
	...
	std::cout << "drivers/rtl8168: Card name: " << rtl_chip_infos[_revision].name << std::endl;
}
...
async::result<bool> RealtekNic::startCard() {
	...
	if(_revision <= MacRevision::MacVer06) {
		...
		assert(!"Not Implemented"); // rtl_hw_start_8125
	} else {
...
async::result<void> RealtekNic::init() {
	...
	std::cout << "drivers/rtl8168: starting driver" << std::endl;
...
async::result<void> RealtekNic::receive(arch::dma_buffer_view frame) {
