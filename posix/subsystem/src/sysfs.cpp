...
void AttributeFile::handleClose() { _cancelServe.cancel(); }
...
AttributeFile::writeAll(Process *, const void *data, size_t length) {
	...
	auto node = static_cast<AttributeNode *>(associatedLink()->getTarget().get());
	auto err = co_await node->_attr->store(
	    node->_object, std::string{reinterpret_cast<const char *>(data), length}
	);
...
helix::BorrowedDescriptor AttributeFile::getPassthroughLane() { return _passthrough; }
...
VfsType AttributeNode::getType() { return VfsType::regular; }
...
VfsType SymlinkNode::getType() { return VfsType::symlink; }
...
std::optional<std::string> Object::getClassPath() { return std::nullopt; }
...
void Object::addObject() {
	if (_parent) {
		...
		if (class_path.has_value())
			parent_dir = static_cast<DirectoryNode *>(
			    parent_dir->directMkdir(class_path.value())->getTarget().get()
			);
