...
async::result<void> MasterFile::ioctl(Process *, managarm::fs::CntRequest req,
	...
	if(req.command() == TIOCGPTN) {
		...
		if(&_channel->cts != process->pgPointer()->getSession()->getControllingTerminal()) {
			...
			resp.set_pid(_channel->cts.getSession()->getForegroundGroup()->getHull()->getPid());
			resp.set_error(managarm::fs::Errors::SUCCESS);
		}
		...
		std::cout << "\e[31mposix: PTS master TIOCSPGRP\e[39m" << std::endl;
		managarm::fs::SvrResponse resp;
		...
		if(!group) {
			...
			Error ret = _channel->cts.getSession()->setForegroundGroup(group.get());
