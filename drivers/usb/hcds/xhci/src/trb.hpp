...
constexpr RawTrb enableSlot(uint8_t slotType) {
	return RawTrb{
	    ...
	    0,
	    0,
	    (uint32_t{slotType} << 16) | (static_cast<uint32_t>(TrbType::enableSlotCommand) << 10)
	};
}
...
constexpr RawTrb addressDevice(uint8_t slotId, uintptr_t inputCtx) {
	...
	return RawTrb{
	    ...
	    static_cast<uint32_t>(inputCtx >> 32),
	    0,
	    (uint32_t{slotId} << 24) | (static_cast<uint32_t>(TrbType::addressDeviceCommand) << 10)
	};
}
...
constexpr RawTrb configureEndpoint(uint8_t slotId, uintptr_t inputCtx) {
	...
	return RawTrb{
	    ...
	    static_cast<uint32_t>(inputCtx >> 32),
	    0,
	    (uint32_t{slotId} << 24) | (static_cast<uint32_t>(TrbType::configureEndpointCommand) << 10)
	};
}
...
constexpr RawTrb evaluateContext(uint8_t slotId, uintptr_t inputCtx) {
	...
	return RawTrb{
	    ...
	    static_cast<uint32_t>(inputCtx >> 32),
	    0,
	    (uint32_t{slotId} << 24) | (static_cast<uint32_t>(TrbType::evalContextCommand) << 10)
	};
}
...
constexpr RawTrb setTransferRingDequeue(uint8_t slotId, uint8_t endpointId, uintptr_t dequeue) {
	return RawTrb{
	    ...
	    static_cast<uint32_t>(dequeue >> 32),
	    0,
	    (uint32_t{slotId} << 24) | (uint32_t{endpointId} << 16) |
...
constexpr RawTrb
dataStage(uintptr_t address, size_t size, bool chain, uint32_t tdSize, bool dataIn) {
	if (tdSize > 31)
		tdSize = 31;
	return RawTrb{
...
constexpr RawTrb statusStage(bool dataIn) {
	return RawTrb{
	    0, 0, 0, ((dataIn ? 1 : 0) << 16) | (static_cast<uint32_t>(TrbType::statusStage) << 10)
	};
}
...
constexpr RawTrb normal(uintptr_t address, size_t size, bool chain, uint32_t tdSize) {
	...
	return RawTrb{
	    ...
	    static_cast<uint32_t>(address >> 32),
	    static_cast<uint32_t>(size) | (tdSize << 17),
	    (1 << 2) | ((chain ? 1 : 0) << 4) | (static_cast<uint32_t>(TrbType::normal) << 10)
	};
}
...
inline void
buildTransferChain(size_t maxPacketSize, FU use, arch::dma_buffer_view view, FB build, Ts... ts) {
...
template <typename FU>
inline void buildControlChain(
    FU use,
    protocols::usb::SetupPacket setup,
