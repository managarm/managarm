...
async::result<std::shared_ptr<nic::Link>>
setupVirtioDevice(mbus_ng::Entity &base_entity, protocols::hw::Device hwDevice) {
...
async::result<protocols::svrctl::Error> doBindUsb(mbus_ng::Entity baseEntity) {
	...
	for (size_t configuration = 0; configuration < dev_desc->numConfigs; configuration++) {
		    ...
		    [&](int type, size_t, void *descriptor, const auto &info) {
			    if (type == protocols::usb::descriptor_type::cs_interface) {
				    ...
				    case CdcSubType::AbstractControl: {
					    auto hdr =
					        reinterpret_cast<protocols::usb::CdcAbstractControl *>(descriptor);
				    ...
				    case CdcSubType::EthernetNetworking: {
					    auto hdr =
					        reinterpret_cast<protocols::usb::CdcEthernetNetworking *>(descriptor);
				    ...
				    default: {
					    std::cout << std::format(
					                     "netserver: unhandled Function Descriptor SubType {}",
					                     uint8_t(desc->subtype)
					                 )
					              << std::endl;
					    break;
				    }
				    ...
				    default:
					    std::cout << std::format(
					        "netserver: unknown CDC subclass 0x{:x}\n", desc->interfaceSubClass
					    );
					    return;
				    }
				    ...
				    if (info.interfaceNumber && usb_info.data_if &&
				        info.interfaceNumber == usb_info.data_if &&
				    ...
				    } else if (info.interfaceNumber &&
				               info.endpointType == protocols::usb::EndpointType::interrupt &&
		...
		if (usb_info.valid && usb_info.control_if && usb_info.data_if) {
			...
			usb_info.chosen_configuration =
			    reinterpret_cast<protocols::usb::ConfigDescriptor *>(raw_descs.data())->configValue;
			matched_usb_info = std::move(usb_info);
			break;
		}
	}

	if (!matched_usb_info || matched_usb_info->subclass == protocols::usb::cdc_subclass::reserved ||
	    !matched_usb_info->valid || !matched_usb_info->chosen_configuration ||
	    !matched_usb_info->control_if || !matched_usb_info->data_if) {
		std::cout << std::format(
		    "netserver: skipping device {:04x}:{:04x} with mbus ID {}\n",
		    dev_desc->idVendor,
		    dev_desc->idProduct,
		    baseEntity.id()
		);
	...
	std::cout << std::format(
	    "netserver: binding device {:04x}:{:04x} with mbus ID {}\n",
	    dev_desc->idVendor,
	    dev_desc->idProduct,
	    baseEntity.id()
	);
	...
	auto device =
	    co_await nic::usb_net::makeShared(baseEntity.id(), std::move(dev), mac, *matched_usb_info);
...
async::detached serve(helix::UniqueLane lane) {
	while (true) {
		...
		auto sendError = [&conversation](managarm::fs::Errors err) -> async::result<void> {
			managarm::fs::SvrResponse resp;
			resp.set_error(err);
			auto buff = resp.SerializeAsString();
			auto [send] = co_await helix_ng::exchangeMsgs(
			    conversation, helix_ng::sendBuffer(buff.data(), buff.size())
			);
		...
		if (preamble.id() == managarm::fs::CntRequest::message_id) {
			...
			if (req.req_type() == managarm::fs::CntReqType::CREATE_SOCKET) {
				...
				if (req.domain() == AF_INET) {
					auto err = ip4().serveSocket(
					    std::move(local_lane), req.type(), req.protocol(), req.flags()
					);
					...
					async::detach(
					    servePassthrough(std::move(local_lane), nl_socket, &nl::NetlinkSocket::ops)
					);
				} else if (req.domain() == AF_PACKET) {
					auto err = raw().serveSocket(
					    std::move(local_lane), req.type(), req.protocol(), req.flags()
					);
					if (err != managarm::fs::Errors::SUCCESS) {
						co_await sendError(err);
				...
				std::cout << "netserver: received unknown request type: " << (int32_t)req.req_type()
				          << std::endl;
			...
			std::cout << "netserver: received unknown message: " << preamble.id() << std::endl;
			auto [dismiss] = co_await helix_ng::exchangeMsgs(conversation, helix_ng::dismiss());
			HEL_CHECK(dismiss.error());
		}
	}
...
async::detached advertise() {
	mbus_ng::Properties descriptor{{"class", mbus_ng::StringItem{"netserver"}}};

	auto entity =
	    (co_await mbus_ng::Instance::global().createEntity("netserver", descriptor)).unwrap();
