...
enum class ExtractMode { none, eventOnly, specificItem };
...
int main(int argc, char **argv) {
	...
	frg::span<const char> buffer{
	    reinterpret_cast<const char *>(ptr), static_cast<size_t>(st.st_size)
	};
	...
	auto extractRecord = [&]() -> bool {
		...
		case bragi::message_id<managarm::ostrace::EventRecord>: {
			auto maybeRecord =
			    bragi::parse_head_tail<managarm::ostrace::EventRecord>(head_span, tail_span);
			if (!maybeRecord) {
		...
		case bragi::message_id<managarm::ostrace::AnnounceItemRecord>: {
			auto maybeRecord =
			    bragi::parse_head_tail<managarm::ostrace::AnnounceItemRecord>(head_span, tail_span);
			assert(maybeRecord);
			auto &record = maybeRecord.value();
