...
namespace Transfer {
	...
	inline async::result<void> buildTransferChain(std::vector<RawTrb> &trbs, size_t maxPacketSize, arch::dma_space &space, arch::dma_buffer_view view, bool setIoc, auto build) {
	...
	inline async::result<std::vector<RawTrb>> buildNormalChain(arch::dma_space &space, arch::dma_buffer_view view, size_t maxPacketSize) {
		std::vector<RawTrb> trbs;
		co_await buildTransferChain(trbs, maxPacketSize, space, view, true, normal);
		co_return trbs;
	}

	inline async::result<std::vector<RawTrb>> buildControlChain(proto::SetupPacket setup, arch::dma_space &space, arch::dma_buffer_view view, bool dataIn, size_t maxPacketSize) {
		std::vector<RawTrb> trbs;
		...
		co_await buildTransferChain(trbs, maxPacketSize, space, view, false, build);
