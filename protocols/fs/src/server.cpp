...
async::detached handlePassthrough(
	...
	if (req.req_type() == managarm::fs::CntReqType::SEEK_ABS) {
		...
		auto res = co_await file_ops->read(
		    file.get(), extract_creds.credentials(), data.data(), req.size()
		);
		...
		auto res = co_await file_ops->pread(
		    file.get(), req.offset(), extract_creds.credentials(), data.data(), req.size()
		);
		...
		auto res = co_await file_ops->write(
		    file.get(), extract_creds.credentials(), buffer.data(), recv_buffer.actualLength()
		);
		...
		auto resultOrError = co_await file_ops->pollWait(
		    file.get(), req.sequence(), req.event_mask(), async::cancellation_token{}
		);
		if (!resultOrError) {
			managarm::fs::SvrResponse resp;
		...
		auto [extract_creds, recv_addr] = co_await helix_ng::exchangeMsgs(
		    conversation, helix_ng::extractCredentials(), helix_ng::recvInline()
		);
		HEL_CHECK(extract_creds.error());
		...
		auto error = co_await file_ops->bind(
		    file.get(), extract_creds.credentials(), recv_addr.data(), recv_addr.length()
		);
		recv_addr.reset();
		...
		auto actual_length = co_await file_ops->sockname(file.get(), addr.data(), req.size());
		...
		auto [send_resp, send_data] = co_await helix_ng::exchangeMsgs(
		    conversation,
		    helix_ng::sendBuffer(ser.data(), ser.size()),
		    helix_ng::sendBuffer(addr.data(), std::min(size_t(req.size()), actual_length))
		);
		HEL_CHECK(send_resp.error());
		HEL_CHECK(send_data.error());
...
async::result<void> servePassthrough(
	...
	while (true) {
		...
		if (preamble.id() == managarm::fs::CntRequest::message_id) {
			...
			auto [recv_msg] = co_await helix_ng::exchangeMsgs(conversation, helix_ng::recvInline());
			...
			co_await file_ops->ioctl(
			    file.get(), msg_preamble.id(), std::move(recv_msg), std::move(conversation)
			);
			...
			if (req->optlen()) {
				...
				auto [recv_buf] = co_await helix_ng::exchangeMsgs(
				    conversation, helix_ng::recvBuffer(optbuf.data(), optbuf.size())
				);
			...
			auto ret =
			    co_await file_ops->setSocketOption(file.get(), req->layer(), req->number(), optbuf);
			if (!ret) {
...
serveNode(helix::UniqueLane lane, std::shared_ptr<void> node, const NodeOperations *node_ops) {
	while (true) {
		...
		if (req.req_type() == managarm::fs::CntReqType::NODE_GET_STATS) {
			...
			auto req =
			    bragi::parse_head_tail<managarm::fs::NodeTraverseLinksRequest>(recv_req, tail);
			recv_req.reset();
			...
			auto result = co_await node_ops->traverseLinks(
			    node, std::deque(req->path_segments().begin(), req->path_segments().end())
			);
			...
			for (auto &[node, _] : nodes) {
				...
				auto [push_node] = co_await helix_ng::exchangeMsgs(
				    local_push, helix_ng::pushDescriptor(remote_lane)
				);
			...
			co_await node_ops->utimensat(
			    node, req.atime_sec(), req.atime_nsec(), req.mtime_sec(), req.mtime_nsec()
			);
