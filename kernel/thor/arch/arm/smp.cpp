...
bool bootSecondary(DeviceTreeNode *node) {
	...
	KernelPageSpace::global().mapSingle4k(
	    VirtualAddr(codeVirtPtr), codePhysPtr, page_access::write, CachingMode::uncached
	);
	...
	lowMapping.mapSingle4k(
	    codePhysPtr, codePhysPtr, false, page_access::execute, CachingMode::null
	);
	...
	case EnableMethod::spintable: {
		...
		KernelPageSpace::global().mapSingle4k(
		    VirtualAddr(virtPtr), page, page_access::write, CachingMode::uncached
		);
...
static initgraph::Task initAPs{
    ...
    [] {
	    ...
	    getDeviceTreeRoot()->forEach([&](DeviceTreeNode *node) -> bool {
		    if (node->isCompatible<4>(
		            {"arm,cortex-a72", "arm,cortex-a53", "arm,arm-v8", "arm,armv8"}
		        )) {
			    bootSecondary(node);
		    }
