...
BrcmStbPcie::BrcmStbPcie(DeviceTreeNode *node, uint16_t seg, uint8_t busStart, uint8_t busEnd)
	...
	for (size_t i = 0; i < size; i += 0x1000) {
		KernelPageSpace::global().mapSingle4k(
		    VirtualAddr(ptr) + i, addr + i, page_access::write, CachingMode::mmioNonPosted
		);
	}
...
void BrcmStbPcie::init_() {
	...
	regSpace_.store(
	    reg::priv1LinkCap, regSpace_.load(reg::priv1LinkCap) / priv1::linkCap(0b11)
	); // L1 & L0s
	...
	infoLogger() << "thor: Link is up, speed " << lnksta::linkSpeedString(ls & lnksta::linkSpeed)
	             << ", x" << (ls & lnksta::negotiatedLinkWidth) << frg::endlog;

	regSpace_.store(reg::vendorReg1, regSpace_.load(reg::vendorReg1) / vendorReg1::endianMode(0));

	regSpace_.store(reg::hardDebug, regSpace_.load(reg::hardDebug) / hardDebug::clkreqEnable(true));
}

void BrcmStbPcie::reset_() {
	regSpace_.store(reg::bridgeCtl, regSpace_.load(reg::bridgeCtl) / bridgeCtl::swInit(true));

	KernelFiber::asyncBlockCurrent(generalTimerEngine()->sleepFor(200'000));

	regSpace_.store(reg::bridgeCtl, regSpace_.load(reg::bridgeCtl) / bridgeCtl::swInit(false));

	KernelFiber::asyncBlockCurrent(generalTimerEngine()->sleepFor(200'000));

	regSpace_.store(
	    reg::hardDebug, regSpace_.load(reg::hardDebug) / hardDebug::serdesDisable(false)
	);
...
void BrcmStbPcie::enable_() {
	regSpace_.store(reg::bridgeCtl, regSpace_.load(reg::bridgeCtl) / bridgeCtl::reset(false));
...
void BrcmStbPcie::setOutboundWindow_(int n, uint64_t cpuAddr, uint64_t pcieAddr, size_t size) {
	...
	regSpace_.store(
	    baseLimit, regSpace_.load(baseLimit) / base(cpuAddr / 0x100000) / limit(limitMB)
	);
	...
	regSpace_.store(baseHi, regSpace_.load(baseHi) / hiMask(baseMB >> hiShift));
	regSpace_.store(limitHi, regSpace_.load(limitHi) / hiMask(limitMB >> hiShift));
}

uint32_t BrcmStbPcie::mdioRead_(uint8_t port, uint8_t reg) {
	regSpace_.store(reg::mdioAddr, mdio::pktPort(port) | mdio::pktReg(reg) | mdio::pktCmd(1));
...
void BrcmStbPcie::mdioWrite_(uint8_t port, uint8_t reg, uint16_t val) {
	regSpace_.store(reg::mdioAddr, mdio::pktPort(port) | mdio::pktReg(reg) | mdio::pktCmd(0));

	regSpace_.load(reg::mdioAddr);

	regSpace_.store(reg::mdioWrData, mdio::dataDone(1) | mdio::data(val));
...
arch::mem_space
BrcmStbPcie::configSpaceFor_(uint32_t seg, uint32_t bus, uint32_t slot, uint32_t function) {
	assert(seg == seg_);
	...
	regSpace_.store(
	    reg::cfgIndex, cfgIndex::bus(bus) | cfgIndex::slot(slot) | cfgIndex::function(function)
	);
...
void BrcmStbPcie::writeConfigHalf(
    uint32_t seg, uint32_t bus, uint32_t slot, uint32_t function, uint16_t offset, uint16_t value
) {
	if (bus == busStart_ && (slot || function))
		return;
...
void BrcmStbPcie::writeConfigWord(
    uint32_t seg, uint32_t bus, uint32_t slot, uint32_t function, uint16_t offset, uint32_t value
) {
	if (bus == busStart_ && (slot || function))
		return;
