...
struct WriteRequest {
	WriteRequest(const void *buffer, size_t length) : buffer(buffer), length(length), progress(0) {}
...
async::detached serveTerminal(helix::UniqueLane lane) {
	...
	while (true) {
		...
		if (req.req_type() == managarm::fs::CntReqType::DEV_OPEN) {
			...
			async::detach(
			    protocols::fs::servePassthrough(std::move(local_lane), nullptr, &fileOperations)
			);
...
async::detached runTerminal() {
	...
	auto entity = (co_await mbus_ng::Instance::global().createEntity("uart0", descriptor)).unwrap();
