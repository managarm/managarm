...
bool init() {
	...
	common::x86::wrmsr(
	    0xC0000080, common::x86::rdmsr(0xC0000080) | (1 << 12)
	); // Enable IA32_EFER.SVME
...
HelVmexitReason Vcpu::run() {
	...
	while (true) {
		...
		asm volatile("vmsave %%rax"
		             :
		             : "a"(host_additional_save_region)
		             : "memory"
		); // Use vmsave / vmload to save additional state that would otherwise have to be wrmsr'ed
