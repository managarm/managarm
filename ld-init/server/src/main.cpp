...
struct Object {
	...
	: phdrPointer(0), phdrEntrySize(0), phdrCount(0),
		tlsSegmentSize(0), tlsAlignment(0), tlsImageSize(0), tlsImagePtr(0),
		entry(0), dynamic(0), segments(*allocator),
		hasPhdrImage(false) { }

	void *imagePtr;
	uintptr_t phdrPointer, phdrEntrySize, phdrCount;
	uintptr_t tlsSegmentSize, tlsAlignment;
	uintptr_t tlsImageSize, tlsImagePtr;
	uintptr_t entry;
	uintptr_t dynamic;
...
Object *readObject(frigg::StringView path) {
	...
	for(int i = 0; i < ehdr->e_phnum; i++) {
		...
		if(phdr->p_type == PT_PHDR) {
			...
			object->tlsImagePtr = phdr->p_vaddr;
...
void sendObject(HelHandle pipe, int64_t request_id,
	...
	response.set_tls_segment_size(object->tlsSegmentSize);
	response.set_tls_alignment(object->tlsAlignment);
	response.set_tls_image_size(object->tlsImageSize);
	response.set_tls_image_ptr(base_address + object->tlsImagePtr);
