...
struct MountView : std::enable_shared_from_this<MountView> {
	...
	dev_t mountDeviceNumber() const {
		if(deviceLink_.second && deviceLink_.second->getTarget()->getType() == VfsType::blockDevice)
			return makedev(deviceLink_.second->getTarget()->readDevice().first, deviceLink_.second->getTarget()->readDevice().second);

		return makedev(0, mountId_);
	}
