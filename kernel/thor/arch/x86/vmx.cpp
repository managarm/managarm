...
inline uint64_t vmread(uint64_t encoding) {
	uint64_t tmp;
	uint8_t ret;
	asm volatile("vmread %[encoding], %[value];"
	             "setna %[ret];"
...
extern "C" uintptr_t vmxDoVmExit[]; // Actually a function that cannot be called in the conventional
                                    // sense, just need the address
...
Vmcs::Vmcs(smarter::shared_ptr<EptSpace> ept) : space(ept) {
	...
	*(uint32_t *)regionAccessor.get() = static_cast<uint32_t>(vmxRevision);
	...
	vmwrite(
	    PROC_BASED_VM_EXEC_CONTROLS, cpu | VMEXIT_ON_HLT | VMEXIT_ON_PIO | SECONDARY_CONTROLS_ON
	);
	vmwrite(PROC_BASED_VM_EXEC_CONTROLS2, EPT_ENABLE | UNRESTRICTED_GUEST | VMEXIT_ON_DESCRIPTOR);
	...
	uint64_t trAddr = ((uint64_t)entry4 << 32) | (entry1 | ((entry2) << 16) | ((entry3) << 24));
	...
	if (getGlobalCpuFeatures()->haveXsave) {
		...
		assert(
		    reinterpret_cast<PhysicalAddr>(hostFstate) != static_cast<PhysicalAddr>(-1) && "OOM"
		);
		...
		assert(
		    reinterpret_cast<PhysicalAddr>(guestFstate) != static_cast<PhysicalAddr>(-1) && "OOM"
		);
	...
	while (1) {
		...
		if (reason == VMEXIT_HLT) {
			...
			auto faultOutcome = Thread::asyncBlockCurrent(
			    space->handleFault(address, flags, getCurrentThread()->mainWorkQueue()->take())
			);
			if (!faultOutcome) {
...
#define SET_SEGMENT(segment, segment_capital)                                                      \
	...
	{                                                                                              \
		uint32_t attrib = regs->segment.type | (regs->segment.s << 4) | (regs->segment.dpl << 5) | \
		                  (regs->segment.present << 7) | (regs->segment.avl << 12) |               \
		                  (regs->segment.l << 13) | (regs->segment.db << 14) |                     \
		                  (regs->segment.g << 15);                                                 \
...
Vmcs::~Vmcs() { physicalAllocator->free((size_t)region, kPageSize); }
} // namespace thor::vmx
