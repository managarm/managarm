...
struct NullFile final : File {
  private:
	async::result<frg::expected<Error, size_t>> readSome(Process *, void *, size_t) override {
		co_return 0;
	}
...
struct NullDevice final : UnixDevice {
	NullDevice() : UnixDevice(VfsType::charDevice) { assignId({1, 3}); }
...
std::shared_ptr<UnixDevice> createNullDevice() { return std::make_shared<NullDevice>(); }
