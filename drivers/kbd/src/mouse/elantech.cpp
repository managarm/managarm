...
ElantechTouchpadDevice::probe(Controller::Port *port) {
	...
	if (magic[0] == 0x3C && magic[1] == 0x03 && (magic[2] == 0xC8 || magic[2] == 0x00)) {
		...
		uint32_t fw =
		    uint32_t{fw_version[0]} << 16 | uint32_t{fw_version[1]} << 8 | uint32_t{fw_version[2]};
		uint8_t hwVersion = 0;
...
async::result<void> ElantechTouchpadDevice::run() {
	...
	if (!res0) {
		std::println("ps2-hid: Failed to reset Elantech touchpad");
		co_return;
	}

	auto caps = co_await submitQuery(Query::Capabilities);
	if (!caps) {
		std::println("ps2-hid: Failed to query Elantech touchpad capabilities");
		co_return;
	}

	auto samples = co_await submitQuery(Query::Sample);
	if (!samples) {
		std::println("ps2-hid: Failed to query Elantech touchpad samples");
		co_return;
	}

	auto resolution = co_await submitQuery(Query::Resolution);
	if (!samples) {
		std::println("ps2-hid: Failed to query Elantech touchpad resolution");
		co_return;
	}
	...
	if (setAbsoluteErr.has_value()) {
		std::println("ps2-hid: Failed to set Elantech touchpad absolute mode");
		co_return;
	}
	...
	evDev_->enableEvent(EV_ABS, ABS_MT_TOOL_TYPE);
	...
	if (!enabled) {
		std::println("ps2-hid: Failed to enable Elantech touchpad");
		co_return;
	}
