...
PL011::PL011(uintptr_t base, uint64_t clock) : base_{base}, space_{base}, clock_{clock} {}

void PL011::init(uint64_t baud) {
	disable();

	uint64_t int_part = clock_ / (16 * baud);

	// 3 decimal places of precision should be enough :^)
	uint64_t frac_part = (((clock_ * 1000) / (16 * baud) - (int_part * 1000)) * 64 + 500) / 1000;
	...
	// 8n1, fifo enabled
	space_.store(
	    pl011_reg::line_control, pl011_line_control::word_len(3) | pl011_line_control::fifo_en(true)
	);
	space_.store(
	    pl011_reg::control,
...
void PL011::disable() { space_.store(pl011_reg::control, pl011_control::uart_en(false)); }

void PL011::write(char c) {
	while (space_.load(pl011_reg::status) & pl011_status::tx_full)
		;

	space_.store(pl011_reg::data, c);
}
