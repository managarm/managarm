...
Inode::Inode(FileSystem &fs, uint32_t number) : fs(fs), number(number), isReady(false) {}
...
Inode::findEntry(std::string name) {
	...
	while (offset < fileSize()) {
		...
		auto disk_entry =
		    reinterpret_cast<DiskDirEntry *>(reinterpret_cast<char *>(fileMapping.get()) + offset);
		assert(disk_entry->recordLength);

		if (disk_entry->inode && name.length() == disk_entry->nameLength &&
...
Inode::link(std::string name, int64_t ino, blockfs::FileType type) {
	...
	auto appendDirEntry = [&](size_t offset,
	                          size_t length) -> async::result<std::optional<DirEntry>> {
		auto diskEntry =
		    reinterpret_cast<DiskDirEntry *>(reinterpret_cast<char *>(fileMapping.get()) + offset);
		...
		auto syncInode = co_await helix_ng::synchronizeSpace(
		    helix::BorrowedDescriptor{kHelNullHandle}, target->diskMapping.get(), fs.inodeSize
		);
	...
	while (offset < fileSize()) {
		...
		if (disk_entry->inode && name.length() == disk_entry->nameLength &&
			...
			if (target->fileType == kTypeDirectory) {
				...
				auto &&target_submit = helix::submitLockMemoryView(
				    helix::BorrowedDescriptor(target->frontalMemory),
				    &target_lock_memory,
				    0,
				    target_map_size,
				    helix::Dispatcher::global()
				...
				while (target_offset < target->fileSize()) {
					...
					if (target_disk_entry->inode && target_disk_entry->nameLength == 2 &&
					    target_disk_entry->name[0] == '.' && target_disk_entry->name[1] == '.') {
						// ".."
					} else if (target_disk_entry->inode && target_disk_entry->nameLength == 1 &&
...
async::result<std::optional<DirEntry>> Inode::mkdir(std::string name) {
	...
	HEL_CHECK(helResizeMemory(dirNode->backingMemory, (fs.blockSize + 0xFFF) & ~size_t(0xFFF)));
	dirNode->fileMapping = helix::Mapping{
	    helix::BorrowedDescriptor{dirNode->frontalMemory},
	    0,
	    fs.blockSize,
	    kHelMapProtRead | kHelMapProtWrite | kHelMapDontRequireBacking
	};
	...
	auto syncInode = co_await helix_ng::synchronizeSpace(
	    helix::BorrowedDescriptor{kHelNullHandle}, dirNode->diskMapping.get(), fs.inodeSize
	);
	HEL_CHECK(syncInode.error());
	...
	syncInode = co_await helix_ng::synchronizeSpace(
	    helix::BorrowedDescriptor{kHelNullHandle}, diskMapping.get(), fs.inodeSize
	);
	HEL_CHECK(syncInode.error());

	// Synchronize the data blocks
	syncInode = co_await helix_ng::synchronizeSpace(
	    helix::BorrowedDescriptor{kHelNullHandle}, dirNode->fileMapping.get(), dirNode->fileSize()
	);
	HEL_CHECK(syncInode.error());
...
async::result<std::optional<DirEntry>> Inode::symlink(std::string name, std::string target) {
	...
	auto syncInode = co_await helix_ng::synchronizeSpace(
	    helix::BorrowedDescriptor{kHelNullHandle}, newNode->diskMapping.get(), fs.inodeSize
	);
	HEL_CHECK(syncInode.error());
...
Inode::utimensat(uint64_t atime_sec, uint64_t atime_nsec, uint64_t mtime_sec, uint64_t mtime_nsec) {
	std::cout << "\e[31m"
	             "ext2fs: utimensat() only supports setting atime and mtime to current time"
	             "\e[39m"
	          << std::endl;
	...
	if (atime_sec != UTIME_NOW || atime_nsec != UTIME_NOW || mtime_sec != UTIME_NOW ||
		...
		std::cout << "\e[31m"
		             "ext2fs: utimensat() unsupported mode called (not UTIME_NOW for all fields)"
		             "\e[39m"
		          << std::endl;
		co_return protocols::fs::Error::none;
	}
...
async::result<void> FileSystem::init() {
	...
	blockGroupDescriptorBuffer.resize(
	    (numBlockGroups * sizeof(DiskGroupDesc) + 511) & ~size_t(511)
	);
	...
	co_await device->readSectors(
	    (bgdt_offset >> blockShift) * sectorsPerBlock,
	    blockGroupDescriptorBuffer.data(),
	    blockGroupDescriptorBuffer.size() / 512
	);
	...
	HEL_CHECK(helCreateManagedMemory(
	    numBlockGroups << blockPagesShift, 0, &block_bitmap_backing, &block_bitmap_frontal
	));
	HEL_CHECK(helCreateManagedMemory(
	    numBlockGroups << blockPagesShift, 0, &inode_bitmap_backing, &inode_bitmap_frontal
	));
	...
	HEL_CHECK(helCreateManagedMemory(
	    inodesPerGroup * inodeSize * numBlockGroups, 0, &inode_table_backing, &inode_table_frontal
	));
...
async::detached FileSystem::manageBlockBitmap(helix::UniqueDescriptor memory) {
	while (true) {
		...
		auto &&submit_manage =
		    helix::submitManageMemory(memory, &manage, helix::Dispatcher::global());
		...
		assert(
		    !(manage.offset() & ((1 << blockPagesShift) - 1)) &&
		    "TODO: propery support multi-page blocks"
		);
		assert(
		    manage.length() == (1 << blockPagesShift) && "TODO: propery support multi-page blocks"
		);

		if (manage.type() == kHelManageInitialize) {
			helix::Mapping bitmap_map{
			    memory, static_cast<ptrdiff_t>(manage.offset()), manage.length()
			};
			co_await device->readSectors(
			    block * sectorsPerBlock, bitmap_map.get(), sectorsPerBlock
			);
			HEL_CHECK(helUpdateMemory(
			    memory.getHandle(), kHelManageInitialize, manage.offset(), manage.length()
			));
		} else {
			...
			HEL_CHECK(helUpdateMemory(
			    memory.getHandle(), kHelManageWriteback, manage.offset(), manage.length()
			));
		}
	}
}
...
async::detached FileSystem::manageInodeTable(helix::UniqueDescriptor memory) {
	while (true) {
		...
		if (manage.type() == kHelManageInitialize) {
			helix::Mapping table_map{
			    memory, static_cast<ptrdiff_t>(manage.offset()), manage.length()
			};
			co_await device->readSectors(
			    block * sectorsPerBlock + bg_offset / 512, table_map.get(), manage.length() / 512
			);
			HEL_CHECK(helUpdateMemory(
			    memory.getHandle(), kHelManageInitialize, manage.offset(), manage.length()
			));
...
auto FileSystem::accessRoot() -> std::shared_ptr<Inode> { return accessInode(EXT2_ROOT_INO); }
...
async::result<void>
FileSystem::write(Inode *inode, uint64_t offset, const void *buffer, size_t length) {
	...
	if (offset + length > inode->fileSize()) {
		HEL_CHECK(helResizeMemory(inode->backingMemory, (offset + length + 0xFFF) & ~size_t(0xFFF))
		);
		inode->setFileSize(offset + length);
		auto syncInode = co_await helix_ng::synchronizeSpace(
		    helix::BorrowedDescriptor{kHelNullHandle}, inode->diskMapping.get(), inodeSize
		);
		HEL_CHECK(syncInode.error());
	}
	...
	*/
	...
	auto writeMemory = co_await helix_ng::writeMemory(
	    helix::BorrowedDescriptor(inode->frontalMemory), offset, length, buffer
	);
...
async::detached FileSystem::initiateInode(std::shared_ptr<Inode> inode) {
	...
	inode->diskMapping = helix::Mapping{
	    inodeTable,
	    inode_address,
	    inodeSize,
	    kHelMapProtWrite | kHelMapProtRead | kHelMapDontRequireBacking
	...
	if (inode->fileType == kTypeDirectory) {
		...
		inode->fileMapping = helix::Mapping{
	...
	HEL_CHECK(helCreateManagedMemory(3 << blockPagesShift, 0, &backingOrder1, &frontalOrder1));
	HEL_CHECK(helCreateManagedMemory(
	    (blockSize / 4) << blockPagesShift, 0, &backingOrder2, &frontalOrder2
	));
...
async::detached FileSystem::manageFileData(std::shared_ptr<Inode> inode) {
	while (true) {
		...
		if (manage.type() == kHelManageInitialize) {
			...
			co_await inode->fs.readDataBlocks(
			    inode, manage.offset() / inode->fs.blockSize, num_blocks, file_map.get()
			);
			...
			assert(num_blocks * inode->fs.blockSize <= manage.length());
			co_await inode->fs.writeDataBlocks(
			    inode, manage.offset() / inode->fs.blockSize, num_blocks, file_map.get()
			);

			HEL_CHECK(helUpdateMemory(
			    inode->backingMemory, kHelManageWriteback, manage.offset(), manage.length()
			));
		}
	}
}

async::detached FileSystem::manageIndirect(
    std::shared_ptr<Inode> inode, int order, helix::UniqueDescriptor memory
) {
	while (true) {
		helix::ManageMemory manage;
		auto &&submit_manage =
		...
		if (order == 1) {
			...
			switch (element) {
			case 0:
				block = disk_inode->data.blocks.singleIndirect;
				break;
			case 1:
				block = disk_inode->data.blocks.doubleIndirect;
				break;
			case 2:
				block = disk_inode->data.blocks.tripleIndirect;
				break;
			default:
			...
			auto &&submit_indirect = helix::submitLockMemoryView(
			    inode->indirectOrder1,
			    &lock_indirect,
			    (1 + indirect_frame) << blockPagesShift,
			    1 << blockPagesShift,
			    helix::Dispatcher::global()
			);
			...
			helix::Mapping indirect_map{
			    inode->indirectOrder1,
			    (1 + indirect_frame) << blockPagesShift,
			    size_t{1} << blockPagesShift,
		...
		if (manage.type() == kHelManageInitialize) {
			helix::Mapping out_map{
			    memory, static_cast<ptrdiff_t>(manage.offset()), manage.length()
			};
			co_await device->readSectors(block * sectorsPerBlock, out_map.get(), sectorsPerBlock);
			HEL_CHECK(helUpdateMemory(
			    memory.getHandle(), kHelManageInitialize, manage.offset(), manage.length()
			));
		} else {
			...
			co_await device->writeSectors(block * sectorsPerBlock, out_map.get(), sectorsPerBlock);
			HEL_CHECK(helUpdateMemory(
			    memory.getHandle(), kHelManageWriteback, manage.offset(), manage.length()
			));
		}
...
async::result<uint32_t> FileSystem::allocateBlock() {
	for (uint32_t bg_idx = 0; bg_idx < numBlockGroups; bg_idx++) {
		...
		auto &&submit_bitmap = helix::submitLockMemoryView(
		    blockBitmap,
		    &lock_bitmap,
...
async::result<void>
FileSystem::assignDataBlocks(Inode *inode, uint64_t block_offset, size_t num_blocks) {
	...
	while (prg < num_blocks) {
		if (block_offset + prg < i_range) {
			while (prg < num_blocks && block_offset + prg < i_range) {
			...
			while (prg < num_blocks && block_offset + prg < s_range) {
			...
			helix::Mapping double_indirect_map{
			    inode->indirectOrder1,
			    1 << blockPagesShift,
			    size_t{1} << blockPagesShift,
			    kHelMapProtRead | kHelMapProtWrite | kHelMapDontRequireBacking
			...
			while (prg < num_blocks && block_offset + prg < d_range) {
				int64_t indirect_frame = (block_offset + prg - s_range) >> (blockShift - 2);
				int64_t indirect_index =
				    (block_offset + prg - s_range) & ((1 << (blockShift - 2)) - 1);
				...
				helix::LockMemoryView lock_indirect;
				auto &&submit = helix::submitLockMemoryView(
				    inode->indirectOrder2,
				    &lock_indirect,
				    indirect_frame << blockPagesShift,
				    1 << blockPagesShift,
				    helix::Dispatcher::global()
				);
				co_await submit.async_wait();
...
async::result<void> FileSystem::readDataBlocks(
    std::shared_ptr<Inode> inode, uint64_t offset, size_t num_blocks, void *buffer
) {
	...
	while (progress < num_blocks) {
		...
		if (index >= d_range) {
			...
			issue = fuse(num_blocks - progress, disk_inode->data.blocks.direct + index, 12 - index);
		}
...
async::result<void> FileSystem::writeDataBlocks(
    std::shared_ptr<Inode> inode, uint64_t offset, size_t num_blocks, const void *buffer
) {
	// We perform "block-fusion" here i.e. we try to read/write multiple
	// consecutive blocks in a single read/writeSectors() operation.
	...
	while (progress < num_blocks) {
		...
		if (index >= d_range) {
			...
			issue = fuse(
			    indirect_index,
			    num_blocks - progress,
			    reinterpret_cast<uint32_t *>(indirect_map.get()),
			    per_indirect
			);
			...
			issue = fuse(index, num_blocks - progress, disk_inode->data.blocks.direct, 12);
		}
...
async::result<void> FileSystem::truncate(Inode *inode, size_t size) {
	HEL_CHECK(helResizeMemory(inode->backingMemory, (size + 0xFFF) & ~size_t(0xFFF)));
...
OpenFile::OpenFile(std::shared_ptr<Inode> inode) : inode(inode), offset(0) {}

async::result<std::optional<std::string>> OpenFile::readEntries() {
	co_await inode->readyJump.wait();

	if (inode->fileType != kTypeDirectory) {
		std::cout << "\e[33m"
		             "ext2fs: readEntries called on something that's not a directory\e[39m"
		          << std::endl;
	...
	while (offset < inode->fileSize()) {
		...
		auto disk_entry =
		    reinterpret_cast<DiskDirEntry *>(reinterpret_cast<char *>(file_map.get()) + offset);
