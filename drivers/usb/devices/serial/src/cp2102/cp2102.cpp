...
std::set<std::pair<size_t, size_t>> an205Table1 = {
    ...
    {16000, 16062},   {19200, 19250},   {28800, 28912},   {38400, 38601},     {51200, 51558},
    {56000, 56280},   {57600, 58053},   {64000, 64111},   {76800, 77608},     {115200, 117028},
    {128000, 129347}, {153600, 156868}, {230400, 237832}, {250000, 254234},   {256000, 273066},
    {460800, 491520}, {500000, 567138}, {576000, 670254}, {921600, UINT_MAX},
};
...
async::result<void> Cp2102::initialize() {
	...
	protocols::usb::walkConfiguration(
	    ...
	    [&](int type, size_t, void *, const auto &info) {
	...
	co_await transferControl(
	    ...
	    pool_,
	    true,
	    uint8_t(cp2102::Request::VENDOR_SPECIFIC),
	    uint16_t(cp2102::VendorRequest::GET_PARTNUM),
	    intfNumber_,
	    buf
	);
	...
	co_await transferControl(
	    hw(), pool_, false, uint8_t(cp2102::Request::SET_MHS), control, cp2102::CONFIG_INTERFACE, {}
	);
...
async::result<void> Cp2102::setConfiguration(struct termios &new_config) {
	if (new_config.c_cflag & CBAUD) {
		...
		if (termios_baud) {
			...
			co_await transferControl(
			    ...
			    pool_,
			    false,
			    uint8_t(cp2102::Request::SET_BAUDRATE),
			    0,
			    0,
			    arch::dma_buffer_view{nullptr, &baud, sizeof(baud)}
			);
		}
	}
	...
	co_await transferControl(
	    hw(), pool_, false, uint8_t(cp2102::Request::SET_LINE_CTL), bits, 0, {}
	);

	if (cfgetospeed(&new_config) != 0 && cfgetospeed(&activeSettings) != 0 &&
	...
	if (new_config.c_iflag & IXON || new_config.c_iflag & IXOFF) {
		...
		co_await transferControl(
		    ...
		    pool_,
		    false,
		    uint8_t(cp2102::Request::SET_CHARS),
		    0,
		    cp2102::CONFIG_INTERFACE,
		    arch::dma_buffer_view{nullptr, &chars, sizeof(chars)}
		);
	}
...
size_t Cp2102::sendFifoSize() { return cp2102::BULK_BUF_SIZE; }
