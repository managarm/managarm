...
handleGetFamily(nl_socket::OpenFile *f, struct nlmsghdr *nlh, struct sockaddr_nl *sa) {
	...
	if (req.has_value()) {
		for (auto val : *req) {
			...
			default:
				std::cout << std::format(
				    "posix: unhandled CTRL_CMD_GETFAMILY attr 0x{:x}\n", val.type()
				);
				break;
			}
		}
	...
	for (auto const &[id, info] : families) {
		...
		if (!info.ops.empty()) {
			b.nested_nlattr<const GenericNetlinkFamily *>(
			    CTRL_ATTR_OPS,
			...
			b.nested_nlattr<const GenericNetlinkFamily *>(
			    ...
			    [](core::netlink::NetlinkBuilder &b, auto info) {
				    for (uint32_t i = 0; i < info->mcast_groups.size(); i++) {
					    ...
					    b.nested_nlattr<const MulticastGroupInfo *>(
					        i + 1,
					        [](core::netlink::NetlinkBuilder &b, auto ctx) {
...
async::result<protocols::fs::Error>
nlctrl::sendMsg(nl_socket::OpenFile *f, core::netlink::Packet packet, struct sockaddr_nl *sa) {
	...
	case GENL_ID_CTRL: {
		...
		case CTRL_CMD_GETFAMILY: {
			co_return co_await handleGetFamily(f, nlh, sa);
		}
		default: {
			std::cout << std::format(
			    "posix: unknown nlctrl cmd=0x{:x} version={}\n", genlh->cmd, genlh->version
			);
