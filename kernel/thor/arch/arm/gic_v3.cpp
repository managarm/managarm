...
namespace redist_reg {
	...
	static constexpr uintptr_t irqGroupBase = 0x10080;
	static constexpr uintptr_t irqConfigBase = 0x10C00;
	static constexpr uintptr_t irqGroupModBase = 0x10C00;
	static constexpr uintptr_t irqSetEnableBase = 0x10100;
	static constexpr uintptr_t irqClearEnableBase = 0x10180;
	static constexpr uintptr_t irqPriorityBase = 0x10400;
}
...
void GicRedistributorV3::initOnThisCpu() {
	...
	arch::scalar_store_relaxed<uint32_t>(space_, redist_reg::irqGroupBase, ~0);
	arch::scalar_store_relaxed<uint32_t>(space_, redist_reg::irqGroupModBase, 0);
}
...
bool GicPinV3::setMode(TriggerMode trigger, Polarity polarity) {
	...
	if(irq_ < 32) {
		...
		auto v = arch::scalar_load_relaxed<uint32_t>(redist.space_, redist_reg::irqConfigBase + offset);
		v &= ~(0b11 << bitOffset);
		v |= bitValue << bitOffset;
		arch::scalar_store_relaxed(redist.space_, redist_reg::irqConfigBase + offset, v);

		auto group = arch::scalar_load_relaxed<uint32_t>(redist.space_, redist_reg::irqGroupBase + groupOffset);
		group |= 1U << groupBitOffset;
		arch::scalar_store_relaxed(redist.space_, redist_reg::irqGroupBase + groupOffset, group);

		auto groupMod = arch::scalar_load_relaxed<uint32_t>(redist.space_, redist_reg::irqGroupModBase + groupOffset);
		groupMod &= ~(1U << groupBitOffset);
		arch::scalar_store_relaxed(redist.space_, redist_reg::irqGroupModBase + groupOffset, groupMod);
	} else {
		auto v = arch::scalar_load_relaxed<uint32_t>(dist->space_, dist_reg::irqConfigBase + offset);
		v &= ~(0b11 << bitOffset);
		v |= bitValue;
		arch::scalar_store_relaxed(dist->space_, dist_reg::irqConfigBase + offset, v);

		auto group = arch::scalar_load_relaxed<uint32_t>(dist->space_, dist_reg::irqGroupBase + groupOffset);
		group |= 1U << groupBitOffset;
		arch::scalar_store_relaxed(dist->space_, dist_reg::irqGroupBase + groupOffset, group);

		auto groupMod = arch::scalar_load_relaxed<uint32_t>(dist->space_, dist_reg::irqGroupModBase + groupOffset);
		groupMod &= ~(1U << groupBitOffset);
		arch::scalar_store_relaxed(dist->space_, dist_reg::irqGroupModBase + groupOffset, groupMod);
	}
...
void GicPinV3::mask() {
	...
	if(irq_ < 32) {
		auto &redist = getRedistForThisCpu();
		arch::scalar_store_relaxed(redist.space_, redist_reg::irqClearEnableBase, 1U << bit);
	} else {
...
void GicPinV3::unmask() {
	...
	if(irq_ < 32) {
		auto &redist = getRedistForThisCpu();
		arch::scalar_store_relaxed(redist.space_, redist_reg::irqSetEnableBase, 1U << bit);
	} else {
		auto offset = irq_ / 32 * 4;
		arch::scalar_store_relaxed(dist->space_, dist_reg::irqSetEnableBase + offset, 1U << bit);
	}
}
...
void GicPinV3::setPriority_(uint8_t priority) {
	...
	if(irq_ < 32) {
		...
		auto value = arch::scalar_load_relaxed<uint32_t>(redist.space_, redist_reg::irqPriorityBase + offset);
		value &= ~(0xFF << bitOffset);
		value |= static_cast<uint32_t>(priority) << bitOffset;
		arch::scalar_store_relaxed(redist.space_, redist_reg::irqPriorityBase + offset, value);
	} else {
		auto value = arch::scalar_load_relaxed<uint32_t>(dist->space_, dist_reg::irqPriorityBase + offset);
		value &= ~(0xFF << bitOffset);
		value |= static_cast<uint32_t>(priority) << bitOffset;
		arch::scalar_store_relaxed(dist->space_, dist_reg::irqPriorityBase + offset, value);
	}
}
