...
std::optional<FormatInfo> getFormatInfo(uint32_t fourcc) {
	auto entry = std::find_if(formats.begin(), formats.end(),
		[&] (const auto &e) { return e.format == fourcc; });
...
uint8_t getFormatBpp(const FormatInfo &info, size_t plane) {
	...
	return info.char_per_block[plane] * 8 / (getFormatBlockWidth(info, plane) * getFormatBlockHeight(info, plane));
}
