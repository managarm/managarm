...
template <size_t N> struct Requires {
...
template <typename... Args> Requires(Args &&...) -> Requires<sizeof...(Args)>;

template <size_t N> struct Entails {
	template <typename... Args>
	Entails(Args &&...args)
	    requires(std::is_convertible_v<Args, Node *> && ...)
	    : array{{args...}} {}
...
template <typename... Args> Entails(Args &&...) -> Entails<sizeof...(Args)>;

struct IntoEdgesTo {
	template <typename... Args> frg::array<Edge, sizeof...(Args)> operator()(Args &&...args) const {
		return {{{args, target}...}};
	}
...
template <typename F, size_t NR = 0, size_t NE = 0> struct Task final : Node {
