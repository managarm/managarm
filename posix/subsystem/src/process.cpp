...
VmContext::mapFile(uintptr_t hint, helix::UniqueDescriptor memory,
	...
	if(copyOnWrite) {
		HelHandle handle;
		if(memory) {
			HEL_CHECK(helCopyOnWrite(_hierarchy.getHandle(), kHelNullHandle, memory.getHandle(),
					offset, alignedSize, &handle));
		}else{
			HEL_CHECK(helCopyOnWrite(_hierarchy.getHandle(), kHelNullHandle,
					getZeroMemory().getHandle(),
					offset, alignedSize, &handle));
		}
...
async::result<Error> Process::exec(std::shared_ptr<Process> process,
	...
	process->_path = std::move(execResult.executablePath);
	process->_name = std::move(execResult.processName);
	process->_posixLane = std::move(server_lane);
	process->_threadDescriptor = std::move(execResult.thread);
