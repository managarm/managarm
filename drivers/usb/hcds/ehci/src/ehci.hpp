...
struct Controller final : proto::BaseController, std::enable_shared_from_this<Controller> {
  ...
  private:
	async::result<frg::expected<proto::UsbError, size_t>>
	_directTransfer(proto::ControlTransfer info, QueueEntity *queue, size_t max_packet_size);
  ...
  public:
	async::result<frg::expected<proto::UsbError, proto::DeviceSpeed>> resetPort(int number);
