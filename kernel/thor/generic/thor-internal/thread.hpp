...
public:
	...
	template <typename Sender, typename Tag = asyncBlockCurrentNormal>
	    requires(std::is_invocable_v<Sender, async::cancellation_token>
	             && std::is_same_v<
	                 typename std::invoke_result_t<Sender, async::cancellation_token>::value_type,
	                 void>)
	...
	template <typename Sender, typename Tag = asyncBlockCurrentNormal>
	    requires(
	        (std::is_invocable_v<Sender, async::cancellation_token>
	         && !std::is_same_v<
	             typename std::invoke_result_t<Sender, async::cancellation_token>::value_type,
	             void>)
	        || !std::is_same_v<typename Sender::value_type, void>
	    )
	static auto asyncBlockCurrent(Sender s, WorkQueue *wq, Tag tag = asyncBlockCurrentNormal{}) {
		(void)tag;
		auto thisThread = getCurrentThread();
