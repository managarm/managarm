...
struct [[gnu::packed]] block_header {
	...
	std::span<key_ptr> key_ptrs() const {
		assert(this->level);
		return std::span<key_ptr>{
		    reinterpret_cast<key_ptr *>(reinterpret_cast<uintptr_t>(this) + sizeof(block_header)),
		    this->nritems
		};
	}
	...
	std::span<item> items() const {
		...
		return std::span<item>{
		    reinterpret_cast<item *>(reinterpret_cast<uintptr_t>(this) + sizeof(block_header)),
		    this->nritems
		};
	}
};
