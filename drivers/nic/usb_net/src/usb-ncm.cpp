...
async::result<void> UsbNcmNic::initialize() {
	...
	auto device_entity =
	    (co_await mbus_ng::Instance::global().createEntity("usb-ncm", descriptor)).unwrap();
	...
	auto res = co_await device_.transfer(
	    protocols::usb::ControlTransfer{protocols::usb::kXferToHost, ctrl_msg, params.view_buffer()}
	);
	assert(res);
	...
	if (ncm_hdr->bmNetworkCapabilities & regs::bmNetworkCapabilities::crcMode) {
		ctrl_msg->type =
		    protocols::usb::setup_type::byClass | protocols::usb::setup_type::targetInterface;
		...
		res = co_await device_.transfer(
		    protocols::usb::ControlTransfer{protocols::usb::kXferToDevice, ctrl_msg, {}}
		);
		assert(res);
	}
	...
	if (ncm_hdr->bmNetworkCapabilities & regs::bmNetworkCapabilities::setEthernetPacketFilter) {
...

		{
			...
			ctrl_msg->value =
			    (packet_type::promiscuous(1) | packet_type::all_multicast(1) |
			     packet_type::directed(1) | packet_type::broadcast(1) | packet_type::multicast(0))
			        .bits();
...
async::result<void> UsbNcmNic::send(const arch::dma_buffer_view payload) {
	arch::dma_buffer buf{
	    &dmaPool_, sizeof(NcmTransferHeader) + sizeof(NcmDatagramPointer) + payload.size()
	};
	auto ncmHeader = reinterpret_cast<NcmTransferHeader *>(buf.data());
