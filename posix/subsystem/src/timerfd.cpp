...
struct OpenFile : File {
	...
	async::detached arm(Timer *timer) {
		...
		if (timer->initial) {
			...
			auto &&submit = helix::submitAwaitClock(
			    &await_initial, tick + timer->initial, helix::Dispatcher::global()
			);
		...
		while (true) {
			...
			auto &&submit = helix::submitAwaitClock(
			    &await_interval, tick + timer->interval, helix::Dispatcher::global()
			);
  ...
  public:
	...
	async::result<frg::expected<Error, PollWaitResult>> pollWait(
	    Process *, uint64_t in_seq, int mask, async::cancellation_token cancellation
	) override {
...
void setTime(File *file, struct timespec initial, struct timespec interval) {
	...
	if (__builtin_mul_overflow(
	        static_cast<uint64_t>(interval.tv_sec), 1000000000, &interval_nanos
	    ) ||
