...
struct RingBuffer {
	...
	~RingBuffer() { operator delete(storage_); }
	...
	void dequeueAdvance(size_t size) { deqPtr_ += size; }
...
struct Tcp4Socket {
	...
	bind(void *object, const char *creds, const void *addrPtr, size_t addrLength) {
		(void)creds;
		...
		if (bindEp.ipAddress != INADDR_ANY && !ip4().hasIp(bindEp.ipAddress)) {
			std::cout << "netserver: IP address " << std::setw(8) << std::hex << bindEp.ipAddress
			          << std::dec << " is not available" << std::endl;
	...
	peername(void *object, void *addr_ptr, size_t max_addr_length) {
		auto self = static_cast<Tcp4Socket *>(object);
	...
	static async::result<void> ioctl(
	...
	static async::result<protocols::fs::Error>
	connect(void *object, const char *creds, const void *addrPtr, size_t addrLength) {
		(void)creds;
	...
	read(void *object, const char *creds, void *data, size_t size) {
	...
	write(void *object, const char *creds, const void *data, size_t size) {
		co_return co_await sendMsg(
		    object, creds, 0, const_cast<void *>(data), size, nullptr, 0, {}, {}
		);
	}
	...
	static async::result<protocols::fs::RecvResult> recvMsg(
		...
		if (flags & ~MSG_PEEK)
			std::cout << "\e[31m"
			             "netserver/tcp: Encountered unexpected recvMsg() flags: "
	...
	setSocketOption(void *object, int layer, int number, std::vector<char> optbuf) {
		...
		std::cout << std::format(
		    "netserver: unhandled TCP socket setsockopt layer {} number {}\n", layer, number
		);
...
async::result<void> Tcp4Socket::flushOutPackets_() {
	while (true) {
		...
		if (connectState_ == ConnectState::sendSyn) {
			...
			PseudoHeader pseudo{
			    .src = targetInfo->source, .dst = remoteEp_.ipAddress, .len = buf.size()
			};
			Checksum csum;
			...
			auto error = co_await ip4().sendFrame(
			    std::move(*targetInfo), buf.data(), buf.size(), static_cast<uint16_t>(IpProto::tcp)
			);
			...
			auto chunk = std::min({
			    bytesAvailable - flushPointer, windowPointer - flushPointer, size_t{1000}
			    // TODO: Perform path MTU discovery.
			});
...
void Tcp4Socket::handleInPacket_(TcpPacket packet) {
	...
	if (connectState_ == ConnectState::sendSyn) {
		if (localSettledSn_ == localFlushedSn_) {
			std::cout << "netserver: Rejecting packet before SYN is sent [sendSyn]" << std::endl;
			return;
		}

		if (!(packet.header.flags.load() & TcpHeader::synFlag)) {
			std::cout << "netserver: Rejecting packet without SYN [sendSyn]" << std::endl;
			return;
		} else if (!(packet.header.flags.load() & TcpHeader::ackFlag)) {
			std::cout << "netserver: Rejecting SYN packet without ACK [sendSyn]" << std::endl;
			return;
		}

		if (packet.header.ackNumber.load() != localSettledSn_ + 1) {
			std::cout << "netserver: Rejecting packet with bad ack-number [sendSyn]" << std::endl;
			return;
		}
		...
		if (packet.header.flags.load() & TcpHeader::ackFlag) {
			...
			if (ackPointer <= validWindow) {
				...
				std::cout << "netserver: Rejecting ack-number outside of valid window" << std::endl;
			}
		}
	}
}
...
bool Tcp4::unbind(TcpEndpoint e) { return binds.erase(e) != 0; }
...
void Tcp4::serveSocket(int flags, helix::UniqueLane lane) {
	...
	async::detach(servePassthrough(std::move(lane), std::move(sock), &Tcp4Socket::ops));
}
