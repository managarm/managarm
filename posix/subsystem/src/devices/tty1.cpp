...
public:
	static void serve(smarter::shared_ptr<TTY1File> file) {
	...
	TTY1File(std::shared_ptr<MountView> mount, std::shared_ptr<FsLink> link)
	: File{FileKind::unknown,  StructName::get("tty1-file"), std::move(mount), std::move(link)} { }
};

struct TTY1Device final : UnixDevice {
	TTY1Device()
	...
	open(std::shared_ptr<MountView> mount, std::shared_ptr<FsLink> link,
		...
		if(semantic_flags & ~(semanticNonBlock | semanticRead | semanticWrite)){
			std::cout << "\e[31mposix: TTY1File open() received illegal arguments:"
		...
		auto file = smarter::make_shared<TTY1File>(std::move(mount), std::move(link));
...
std::shared_ptr<UnixDevice> createTTY1Device() {
	return std::make_shared<TTY1Device>();
}
