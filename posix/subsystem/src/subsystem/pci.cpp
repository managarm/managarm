...
struct PlainfbAttribute : sysfs::Attribute {
	PlainfbAttribute(std::string name) : sysfs::Attribute{std::move(name), false} {}

	async::result<frg::expected<Error, std::string>> show(sysfs::Object *object) override;
};

struct SubsystemVendorAttribute : sysfs::Attribute {
	SubsystemVendorAttribute(std::string name) : sysfs::Attribute{std::move(name), false} {}

	async::result<frg::expected<Error, std::string>> show(sysfs::Object *object) override;
};

struct SubsystemDeviceAttribute : sysfs::Attribute {
	SubsystemDeviceAttribute(std::string name) : sysfs::Attribute{std::move(name), false} {}

	async::result<frg::expected<Error, std::string>> show(sysfs::Object *object) override;
};

struct ConfigAttribute : sysfs::Attribute {
	ConfigAttribute(std::string name) : sysfs::Attribute{std::move(name), false, 256} {}

	async::result<frg::expected<Error, std::string>> show(sysfs::Object *object) override;
};

struct ResourceAttribute : sysfs::Attribute {
	ResourceAttribute(std::string name) : sysfs::Attribute{std::move(name), false} {}
...
struct ResourceNAttribute : sysfs::Attribute {
	ResourceNAttribute(size_t num, std::shared_ptr<drvcore::Device> dev, size_t size)
	    : sysfs::Attribute{"resource" + std::to_string(num), true, size},
	      _device{std::move(dev)},
	      _barIndex{num} {}

	async::result<frg::expected<Error, std::string>> show(sysfs::Object *object) override;
	async::result<frg::expected<Error, helix::UniqueDescriptor>> accessMemory(sysfs::Object *object
	) override;
...
struct ClassAttribute : sysfs::Attribute {
	ClassAttribute(std::string name) : sysfs::Attribute{std::move(name), false} {}

	async::result<frg::expected<Error, std::string>> show(sysfs::Object *object) override;
};

struct IrqAttribute : sysfs::Attribute {
	IrqAttribute(std::string name) : sysfs::Attribute{std::move(name), false} {}
...
struct Device final : drvcore::BusDevice {
	...
	protocols::hw::Device &hwDevice() { return _hwDevice; }
...
struct RootPort final : drvcore::Device {
	RootPort(
	    std::string sysfs_name, int64_t mbus_id, std::shared_ptr<drvcore::Device> parent = nullptr
	)
...
SubsystemVendorAttribute::show(sysfs::Object *object) {
	char buffer[7]; // The format is 0x1234\0.
	auto device = static_cast<Device *>(object);
...
async::result<frg::expected<Error, std::string>>
SubsystemDeviceAttribute::show(sysfs::Object *object) {
	char buffer[7]; // The format is 0x1234\0.
	auto device = static_cast<Device *>(object);
...
ResourceNAttribute::accessMemory(sysfs::Object *object) {
	auto device = static_cast<Device *>(object);
...
async::detached bind(mbus_ng::Entity entity, mbus_ng::Properties properties) {
	...
	if (type == "pci-device" || type == "pci-bridge") {
		...
		std::cout << "POSIX: Installing PCI device " << sysfs_name << " (mbus ID: " << entity.id()
		          << ")" << std::endl;
		...
		auto device =
		    std::make_shared<Device>(sysfs_name, entity.id(), std::move(hwDevice), parentObj);
		device->pciBus =
		    std::stoi(std::get<mbus_ng::StringItem>(properties["pci-bus"]).value, nullptr, 16);
		device->pciSlot =
		    std::stoi(std::get<mbus_ng::StringItem>(properties["pci-slot"]).value, nullptr, 16);
		device->pciFunction =
		    std::stoi(std::get<mbus_ng::StringItem>(properties["pci-function"]).value, nullptr, 16);
		device->vendorId =
		    std::stoi(std::get<mbus_ng::StringItem>(properties["pci-vendor"]).value, nullptr, 16);
		device->deviceId =
		    std::stoi(std::get<mbus_ng::StringItem>(properties["pci-device"]).value, nullptr, 16);
		if (type == "pci-device") {
			device->subsystemVendorId = std::stoi(
			    std::get<mbus_ng::StringItem>(properties["pci-subsystem-vendor"]).value, nullptr, 16
			);
			device->subsystemDeviceId = std::stoi(
			    std::get<mbus_ng::StringItem>(properties["pci-subsystem-device"]).value, nullptr, 16
			);
		}
		...
		std::cout << "POSIX: Installed PCI root bus " << sysfs_name << " (mbus ID: " << entity.id()
		          << ")" << std::endl;
...
async::detached run() {
	...
	auto filter = mbus_ng::Conjunction({mbus_ng::EqualsFilter{"unix.subsystem", "pci"}});
