...
async::detached StorageDevice::run(int config_num, int intf_num) {
	...
	auto endp_in =
	    (co_await intf.getEndpoint(proto::PipeType::in, in_endp_number.value())).unwrap();
	auto endp_out =
	    (co_await intf.getEndpoint(proto::PipeType::out, out_endp_number.value())).unwrap();
	...
	while (true) {
		if (!_queue.empty()) {
			...
			(co_await endp_out.transfer(proto::BulkTransfer{
			     proto::XferFlags::kXferToDevice,
			...
			if (!req->isWrite) {
				proto::BulkTransfer data_info{
				    proto::XferFlags::kXferToHost,
			...
			(co_await endp_in.transfer(proto::BulkTransfer{
			     proto::XferFlags::kXferToHost,
			...
			if (csw.status) {
				std::cout << "block-usb: Error status 0x" << std::hex << (unsigned int)csw.status
				          << std::dec << " in CSW" << std::endl;
...
async::detached bindDevice(mbus_ng::Entity entity) {
	...
	proto::walkConfiguration(
	    ...
	    [&](int type, size_t, void *p, const auto &info) {
		    if (type == proto::descriptor_type::configuration) {
			    ...
			    if (intf_number) {
				    std::cout << "block-usb: Ignoring interface " << info.interfaceNumber.value()
				              << std::endl;
				    return;
			    }
			    if (logEnumeration)
	...
	if (intf_class.value() != protocols::usb::usb_class::mass_storage ||
	    intf_subclass.value() != 0x06 || intf_protocol.value() != 0x50)
		co_return;
