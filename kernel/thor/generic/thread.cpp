...
void Thread::blockCurrent() {
	...
	if (logRunStates)
		infoLogger() << "thor: " << (void *)thisThread.get() << " is blocked" << frg::endlog;
...
void Thread::deferCurrent() {
	...
	if (logRunStates)
		infoLogger() << "thor: " << (void *)thisThread.get() << " is deferred" << frg::endlog;
...
void Thread::deferCurrent(IrqImageAccessor image) {
	...
	if (logRunStates)
		infoLogger() << "thor: " << (void *)this_thread.get() << " is deferred" << frg::endlog;
...
void Thread::suspendCurrent(IrqImageAccessor image) {
	...
	if (logRunStates)
		infoLogger() << "thor: " << (void *)this_thread.get() << " is suspended" << frg::endlog;
...
void Thread::interruptCurrent(Interrupt interrupt, FaultImageAccessor image) {
	...
	if (logRunStates)
		infoLogger() << "thor: " << (void *)this_thread.get() << " is (synchronously) interrupted"
		             << frg::endlog;
	       ...
	       frg::unique_lock<Mutex> lock) {
		    ...
		    while (!queue.empty()) {
			    ...
			    async::execution::set_value(
			        node->receiver, frg::make_tuple(Error::success, sequence, interrupt)
			    );
		    }
...
void Thread::raiseSignals(SyscallImageAccessor image) {
	...
	if (logTransitions)
		infoLogger() << "thor: raiseSignals() in " << (void *)this_thread.get() << frg::endlog;
	assert(this_thread->_runState == kRunActive);

	if (this_thread->_pendingKill) {
		if (logRunStates)
			infoLogger() << "thor: " << (void *)this_thread.get() << " was (asynchronously) killed"
			             << frg::endlog;
		    ...
		    [](Continuation cont,
		       ...
		       frg::unique_lock<Mutex> lock) {
			    ...
			    while (!queue.empty()) {
				    auto node = queue.pop_front();
				    async::execution::set_value(
				        node->receiver, frg::make_tuple(Error::threadExited, 0, kIntrNull)
				    );
			    }
		    ...
		    [](Continuation cont,
		       ...
		       frg::unique_lock<Mutex> lock) {
			    ...
			    while (!queue.empty()) {
				    auto node = queue.pop_front();
				    async::execution::set_value(
				        node->receiver, frg::make_tuple(Error::success, sequence, kIntrRequested)
				    );
			    }
...
void Thread::killOther(smarter::borrowed_ptr<Thread> thread) { thread->_kill(); }
...
UserContext &Thread::getContext() { return _userContext; }

smarter::borrowed_ptr<Universe> Thread::getUniverse() { return _universe; }
...
void Thread::invoke() {
	...
	if (logRunStates)
		infoLogger() << "thor: "
		             << " " << _credentials[0] << " " << _credentials[1] << " " << _credentials[2]
		             << " " << _credentials[3] << " " << _credentials[4] << " " << _credentials[5]
		             << " " << _credentials[6] << " " << _credentials[7] << " " << _credentials[8]
		             << " " << _credentials[9] << " " << _credentials[10] << " " << _credentials[11]
...
void Thread::_uninvoke() { UserContext::deactivate(); }
...
void Thread::AssociatedWorkQueue::wakeup() { unblockOther(_thread->self); }
