...
async::result<protocols::fs::SeekResult> File::ptSeekAbs(void *object, int64_t offset) {
...
async::result<protocols::fs::SeekResult> File::ptSeekRel(void *object, int64_t offset) {
	auto self = static_cast<File *>(object);
...
async::result<protocols::fs::SeekResult> File::ptSeekEof(void *object, int64_t offset) {
	auto self = static_cast<File *>(object);
...
File::ptRead(void *object, const char *credentials, void *buffer, size_t length) {
	auto self = static_cast<File *>(object);
...
File::ptWrite(void *object, const char *credentials, const void *buffer, size_t length) {
	auto self = static_cast<File *>(object);
	auto process = findProcessWithCredentials(credentials);
...
File::ptAllocate(void *object, int64_t offset, size_t size) {
	auto self = static_cast<File *>(object);
...
File::ptBind(void *object, const char *credentials, const void *addr_ptr, size_t addr_length) {
	auto self = static_cast<File *>(object);
	auto process = findProcessWithCredentials(credentials);
...
async::result<protocols::fs::Error>
File::ptConnect(void *object, const char *credentials, const void *addr, size_t addr_len) {
	auto self = static_cast<File *>(object);
	auto process = findProcessWithCredentials(credentials);
...
async::result<void> File::ptIoctl(
...
async::result<frg::expected<protocols::fs::Error, size_t>>
File::ptPeername(void *object, void *addr_ptr, size_t max_addr_length) {
	auto self = static_cast<File *>(object);
...
async::result<protocols::fs::RecvResult> File::ptRecvMsg(
	...
	return self->recvMsg(process.get(), flags, data, len, addr, addr_len, max_ctrl_len);
}
...
async::result<frg::expected<protocols::fs::Error, size_t>> File::ptSendMsg(
	...
	if (flags & ~(MSG_DONTWAIT | MSG_CMSG_CLOEXEC | MSG_NOSIGNAL)) {
		std::cout << "\e[31mposix: Unknown SENDMSG flags: 0x" << std::hex << flags << std::dec
		          << "\e[39m" << std::endl;
	...
	return self->sendMsg(process.get(), flags, data, len, addr, addr_len, std::move(files), ucreds);
}

async::result<helix::BorrowedDescriptor> File::ptAccessMemory(void *object) {
	auto self = static_cast<File *>(object);
	co_return co_await self->accessMemory();
}

async::result<frg::expected<protocols::fs::Error>>
File::ptSetSocketOption(void *object, int layer, int number, std::vector<char> optbuf) {
	auto self = static_cast<File *>(object);
...
bool File::isTerminal() { return _defaultOps & defaultIsTerminal; }
...
async::result<frg::expected<Error, size_t>> File::pread(Process *, int64_t, void *, size_t) {
	std::cout << "posix \e[1;34m" << structName() << "\e[0m: Object does not implement pread()"
	          << std::endl;
...
async::result<frg::expected<Error, size_t>> File::writeAll(Process *, const void *, size_t) {
	std::cout << "posix \e[1;34m" << structName() << "\e[0m: Object does not implement writeAll()"
	          << std::endl;
...
File::recvMsg(Process *, uint32_t, void *, size_t, void *, size_t, size_t) {
	throw std::runtime_error("posix: Object has no File::recvMsg()");
}

async::result<frg::expected<protocols::fs::Error, size_t>> File::sendMsg(
    Process *,
    uint32_t,
    const void *,
    size_t,
	...
	std::cout << "posix \e[1;34m" << structName() << "\e[0m: Object does not implement sendMsg()"
	          << std::endl;
...
async::result<frg::expected<Error, off_t>> File::seek(off_t, VfsSeek) {
	if (_defaultOps & defaultPipeLikeSeek) {
		...
		std::cout << "posix \e[1;34m" << structName() << "\e[0m: Object does not implement seek()"
		          << std::endl;
...
async::result<frg::expected<Error, PollWaitResult>> File::pollWait(
    Process *process, uint64_t sequence, int mask, async::cancellation_token cancellation
) {
	while (true) {
...
async::result<int> File::getOption(int) {
	std::cout << "posix \e[1;34m" << structName() << "\e[0m: Object does not implement getOption()"
	          << std::endl;
	throw std::runtime_error("posix: Object has no File::getOption()");
}

async::result<void> File::setOption(int, int) {
	std::cout << "posix \e[1;34m" << structName() << "\e[0m: Object does not implement setOption()"
	          << std::endl;
...
async::result<protocols::fs::Error> File::connect(Process *, const void *, size_t) {
	std::cout << "posix \e[1;34m" << structName() << "\e[0m: Object does not implement connect()"
	          << std::endl;
...
async::result<size_t> File::sockname(void *, size_t) {
	std::cout << "posix \e[1;34m" << structName() << "\e[0m: Object does not implement sockname()"
	          << std::endl;
...
async::result<frg::expected<Error, std::string>> File::ttyname() {
	std::cout << "posix \e[1;34m" << structName() << "\e[0m: Object does not implement ttyname()"
	          << std::endl;
	co_return Error::notTerminal;
}
...
async::result<frg::expected<protocols::fs::Error>>
File::setSocketOption(int layer, int number, std::vector<char> optbuf) {
	(void)layer;
	(void)number;
	(void)optbuf;
