...
async::result<size_t> Device::getSize() { co_return _size * 512; }
...
async::detached Device::_processRequests() {
	while (true) {
		...
		chain.setupBuffer(
		    virtio_core::hostToDevice, arch::dma_buffer_view{nullptr, header, sizeof(VirtRequest)}
		);
		...
		for (size_t i = 0; i < request->numSectors; i++) {
			...
			if (request->write) {
				chain.setupBuffer(
				    ...
				    arch::dma_buffer_view{nullptr, (char *)request->buffer + 512 * i, 512}
				);
			} else {
				chain.setupBuffer(
		...
		if (logInitiateRetire)
			std::cout << "Submitting " << request->numSectors << " data descriptors" << std::endl;

		// Setup a descriptor for the status byte.
		chain.append(co_await _requestQueue->obtainDescriptor());
		chain.setupBuffer(
		    virtio_core::deviceToHost,
		    arch::dma_buffer_view{nullptr, &statusBuffer[chain.front().tableIndex()], 1}
		);

		// Submit the request to the device
		_requestQueue
		    ->postDescriptor(chain.front(), request, [](virtio_core::Request *base_request) {
			    auto request = static_cast<UserRequest *>(base_request);
			    if (logInitiateRetire)
				    std::cout << "Retiring " << request->numSectors << " data descriptors"
				              << std::endl;
			    request->event.raise();
		    });
