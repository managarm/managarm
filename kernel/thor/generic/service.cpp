	...
	while (true) {
		...
		if (req.req_type() == managarm::fs::CntReqType::WRITE) {
			...
			if (credsError != Error::success) {
				infoLogger() << "thor: Could not receive stdio credentials" << frg::endlog;
				co_return;
			}
...
struct OpenRegular : OpenFile {
	OpenRegular(MfsRegular *module) : module(module), offset(0) {}
	...
	while (true) {
		...
		if (req.req_type() == managarm::fs::CntReqType::READ) {
			...
			frg::unique_memory<KernelAlloc> dataBuffer{
			    *kernelAlloc, frg::min(size_t(req.size()), file->module->size() - file->offset)
			};
			...
			auto memoryError = co_await PushDescriptorSender{
			    conversation, MemoryViewDescriptor{file->module->getMemory()}
			};
			// TODO: improve error handling here.
...
struct Process {
	Process(frg::string<KernelAlloc> name, smarter::shared_ptr<Thread, ActiveHandle> thread)
		...
		posixHandle = _thread->getUniverse()->attachDescriptor(
		    universeLock, LaneDescriptor{std::move(posixStream.get<1>())}
		);

		mbusHandle =
		    _thread->getUniverse()->attachDescriptor(universeLock, LaneDescriptor{*mbusClient});
	}

	coroutine<void> setupAddressSpace() {
		auto view = smarter::allocate_shared<MemorySlice>(*kernelAlloc, fileTableMemory, 0, 0x1000);
		auto result = co_await _thread->getAddressSpace()->map(
		    std::move(view), 0, 0, 0x1000, AddressSpace::kMapPreferTop | AddressSpace::kMapProtRead
		);
		assert(result);
	...
	frg::string_view name() { return _name; }
	...
	coroutine<int> attachFile(OpenFile *file) {
		Handle handle;
		{
			...
			handle = _thread->getUniverse()->attachDescriptor(
			    universe_guard, LaneDescriptor(file->clientLane)
			);
		}
		...
		for (int fd = 0; fd < (int)openFiles.size(); ++fd) {
			...
			auto copyOutcome = co_await fileTableMemory->copyTo(
			    sizeof(Handle) * fd, &handle, sizeof(Handle), WorkQueue::generalQueue()->take()
			);
			assert(copyOutcome);
			co_return fd;
		}
...
coroutine<void> Process::runPosixRequests() {
	while (true) {
		...
		if (preamble.id() == bragi::message_id<managarm::posix::CntRequest>) {
			...
			auto req = bragi::parse_head_only<managarm::posix::CntRequest>(reqBuffer, *kernelAlloc);
			if (!req) {
			...
			if (req->request_type() != managarm::posix::CntReqType::SIG_ACTION)
				infoLogger() << "thor: Unexpected legacy POSIX request " << req->request_type()
				             << frg::endlog;
			...
			if (module->type == MfsType::directory) {
				...
				auto file = frg::construct<initrd::OpenDirectory>(
				    *kernelAlloc, static_cast<MfsDirectory *>(module)
				);
				file->clientLane = std::move(stream.get<1>());

				async::detach_with_allocator(
				    *kernelAlloc, runDirectoryRequests(file, std::move(stream.get<0>()))
				);
				...
				auto file = frg::construct<initrd::OpenRegular>(
				    *kernelAlloc, static_cast<MfsRegular *>(module)
				);
				file->clientLane = std::move(stream.get<1>());

				async::detach_with_allocator(
				    *kernelAlloc, runRegularRequests(file, std::move(stream.get<0>()))
				);
			...
			auto req =
			    bragi::parse_head_only<managarm::posix::IsTtyRequest>(reqBuffer, *kernelAlloc);
			if (!req) {
				infoLogger() << "thor: Could not parse POSIX request" << frg::endlog;
				co_return;
			}
			...
			auto req =
			    bragi::parse_head_only<managarm::posix::CloseRequest>(reqBuffer, *kernelAlloc);
			if (!req) {
				infoLogger() << "thor: Could not parse POSIX request" << frg::endlog;
				co_return;
			}
			...
			auto req =
			    bragi::parse_head_only<managarm::posix::VmMapRequest>(reqBuffer, *kernelAlloc);
			if (!req) {
				infoLogger() << "thor: Could not parse POSIX request" << frg::endlog;
				co_return;
			}
			...
			if (req->flags() & MAP_ANONYMOUS) {   // MAP_ANONYMOUS.
				if (req->flags() & MAP_PRIVATE) { // MAP_PRIVATE.
					...
					auto memory =
					    smarter::allocate_shared<AllocatedMemory>(*kernelAlloc, req->size());
			...
			if (req->flags() & MAP_PRIVATE) { // MAP_PRIVATE.
				auto cowMemory = smarter::allocate_shared<CopyOnWriteMemory>(
				    *kernelAlloc, std::move(fileMemory), req->rel_offset(), req->size()
				);
				cowMemory->selfPtr = cowMemory;
				slice = smarter::allocate_shared<MemorySlice>(
				    *kernelAlloc, std::move(cowMemory), 0, req->size()
				);
			} else {
			...
			auto mapResult = co_await space->map(
			    std::move(slice), req->address_hint(), 0, req->size(), protFlags
			);
			// TODO: improve error handling here.
			assert(mapResult);
			...
			infoLogger() << "thor: Illegal POSIX request type " << preamble.id() << frg::endlog;
			co_return;
		}
	}
}
...
coroutine<void> Process::runObserveLoop() {
	...
	while (true) {
		...
		if (interrupt == kIntrPanic) {
			...
			urgentLogger() << "thor: Panic in server " << name().data() << frg::endlog;
			launchGdbServer(_thread, _name, WorkQueue::generalQueue()->take());
			break;
		} else if (interrupt == kIntrPageFault) {
			// Do nothing and stop observing.
			// TODO: Make sure the server is destructed here.
			urgentLogger() << "thor: Fault in server " << name().data() << frg::endlog;
			launchGdbServer(_thread, _name, WorkQueue::generalQueue()->take());
			break;
			...
			auto cowMemory = smarter::allocate_shared<CopyOnWriteMemory>(
			    *kernelAlloc, std::move(fileMemory), 0, size
			);
			cowMemory->selfPtr = cowMemory;
			auto slice =
			    smarter::allocate_shared<MemorySlice>(*kernelAlloc, std::move(cowMemory), 0, size);
			...
			*_thread->_executor.result1() = 1;
			...
			panicLogger() << "thor: Unexpected observation " << (uint32_t)interrupt << frg::endlog;
		}
	}
}
} // namespace posix
...
void runService(
	...
	KernelFiber::run([name, thread, controlLane = std::move(controlLane)]() mutable {
		...
		async::detach_with_allocator(*kernelAlloc, stdio::runStdioRequests(stdioStream.get<0>()));
		...
		async::detach_with_allocator(*kernelAlloc, process->runPosixRequests());
		async::detach_with_allocator(*kernelAlloc, process->runObserveLoop());
