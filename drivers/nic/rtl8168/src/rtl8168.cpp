...
void RealtekNic::determineMacRevision() {
	...
	// This structure was adapted from the linux kernel driver. It seems to be the only sane way to
	// detect exactly which card we have: it compares various bits from the TxConfig register and
	// applies some masks over the identification registers.
	...
	uint16_t xid =
	    (_mmio.load(regs::transmit_config) & flags::transmit_config::detect_bits) & 0xFCF;
	...
	frg::to(std::cout
	) << frg::fmt("drivers/rtl8168: MAC Revision: MacVer{:02}", uint16_t(_revision))
	  << frg::endlog;
...
// some motherboards (at least some embedded ARM boards) have problems if you write the low int
// first
...
async::result<bool> RealtekNic::startCard() {
	...
	if (_revision <= MacRevision::MacVer06) {
		...
		if (_revision >= MacRevision::MacVer34 && _revision != MacRevision::MacVer37 &&
...
async::result<void> RealtekNic::init() {
	...
	//       It should be safe to treat them as rtl8168 cards, however we should ideally handle
	//       this. I could not find accurate information as to which card they exactly are, but
	//       seeing that no driver I could find handles them specially, they are likely pretty
	//       standard RTL chips which require no special cases anyway, so it is kind of irrelevant
	//       what we set here.
	switch (pci_id) {
	...
	default: {
		std::cout << "drivers/rtl8168: unknown PCI device ID " << std::hex << pci_id << std::dec
		          << std::endl;
...
//       What we should do is have a constant callback we always poll to, as this way of doing
//       things will always be prone to race conditions
...
async::detached RealtekNic::processIrqs() {
	...
	while (true) {
		...
		if (logIRQs) {
			frg::to(std::cout
			) << frg::fmt("drivers/rtl8168: IRQ received status 0x{:04x}", uint16_t(status))
			  << frg::endlog;
		}
		...
		if (status & flags::interrupt_status::tx_desc_unavailable) {
			...
			// If the TX_OK bit is also set, and we still have data to send, then ring the doorbell
			// again. This bypasses bugs found in some cards.
			if (status & flags::interrupt_status::tx_ok && !_txQueue->bufferEmpty() &&
			    restart_transmitter_on_tx_ok_and_tx_desc_unavailable) {
				ringDoorbell();
			}
		}
