...
template <typename T> void maybeFlip(T &x) {
...
struct Udp4Socket {
	...
	connect(void *obj, const char *creds, const void *addr_ptr, size_t addr_size) {
		...
		if (auto e = checkAddress(addr_ptr, addr_size, remote); e != protocols::fs::Error::none) {
			co_return e;
		}
	...
	bind(void *obj, const char *creds, const void *addr_ptr, size_t addr_size) {
		...
		if (auto e = checkAddress(addr_ptr, addr_size, local); e != protocols::fs::Error::none) {
			co_return e;
		}
	...
	static async::result<RecvResult> recvmsg(
		...
		addr.sin_addr = {convert_endian<endian::big>(element->packet->header.source)};
		...
		if (self->ipPacketInfo_) {
			...
			ctrl.write<struct in_pktinfo>({
			    ...
			    .ipi_spec_dst =
			        {.s_addr = convert_endian<endian::big>(element->packet->header.destination)},
	...
	static async::result<frg::expected<protocols::fs::Error, size_t>> sendmsg(
		...
		Checksum chk;
		PseudoHeader psh{
		    .src = convert_endian<endian::big>(ti->source), .dst = target.addr, .len = header.len
		};
		...
		auto error = co_await ip4().sendFrame(
		    std::move(*ti), buf.data(), buf.size(), static_cast<uint16_t>(IpProto::udp)
		);
	...
	static async::result<frg::expected<Error>>
	setSocketOption(void *obj, int layer, int number, std::vector<char> optbuf) {
		auto self = static_cast<Udp4Socket *>(obj);
...
void
Udp4::feedDatagram(smarter::shared_ptr<const Ip4Packet> packet, std::weak_ptr<nic::Link> link) {
	Udp udp{.link = link};
...
bool Udp4::unbind(Endpoint e) { return binds.erase(e) != 0; }
