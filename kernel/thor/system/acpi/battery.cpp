...
coroutine<void> BatteryBusObject::run() {
	...
	properties.stringProperty(
	    "power_supply.type", frg::string<KernelAlloc>(*kernelAlloc, "battery")
	);
	properties.stringProperty("power_supply.id", frg::to_allocated_string(*kernelAlloc, _id));
	properties.stringProperty(
	    "drvcore.mbus-parent", frg::to_allocated_string(*kernelAlloc, acpi_object)
	);
...
coroutine<frg::expected<Error>> BatteryBusObject::handleRequest(LaneHandle lane) {
	...
	if (preamble.id() == bragi::message_id<managarm::hw::BatteryStateRequest>) {
		auto req =
		    bragi::parse_head_only<managarm::hw::BatteryStateRequest>(reqBuffer, *kernelAlloc);
...
uacpi_status
BatteryBusObject::notification(uacpi_handle context, uacpi_namespace_node *node, uacpi_u64 value) {
	if (logBatteryNotifications) {
		auto path = uacpi_namespace_node_generate_absolute_path(node);
		infoLogger() << "thor: battery '" << path << "' received AML Notify(" << value << ")"
		             << frg::endlog;
...
void BatteryBusObject::updateBIF() {
	...
	if (ret != UACPI_STATUS_OK) {
		infoLogger() << "thor: uacpi_object_get_package(bif) error " << uacpi_status_to_string(ret)
		             << frg::endlog;
		return;
	}
	...
	if (_state.acpi_units && design_capacity && *design_capacity != BIF::DesignCapacity::UNKNOWN) {
		if (*_state.acpi_units == BIF::PowerUnit::MILLIAMPERE) {
			...
			if (_state.voltage)
				_state.design_capacity_milliwatthours =
				    (*_state.design_capacity_milliamperehours) * (*_state.voltage);
		} else if (*_state.acpi_units == BIF::PowerUnit::MILLIWATT) {
			_state.design_capacity_milliwatthours = *design_capacity;

			if (_state.voltage)
				_state.design_capacity_milliamperehours =
				    (*_state.design_capacity_milliwatthours) / (*_state.voltage);
		} else {
			panicLogger() << "thor: unhandled ACPI battery power unit " << *_state.acpi_units
			              << frg::endlog;
		}
	} else {
	...
	if (_state.acpi_units && last_full_charge_capacity
	    && *last_full_charge_capacity != BIF::LastFullChargeCapacity::UNKNOWN) {
		if (*_state.acpi_units == BIF::PowerUnit::MILLIAMPERE) {
			_state.last_full_charge_capacity_milliamperehours = *last_full_charge_capacity;

			if (_state.voltage)
				_state.last_full_charge_capacity_milliwatthours =
				    (*_state.last_full_charge_capacity_milliamperehours) * (*_state.voltage);
		} else if (*_state.acpi_units == BIF::PowerUnit::MILLIWATT) {
			_state.last_full_charge_capacity_milliwatthours = *last_full_charge_capacity;

			if (_state.voltage)
				_state.last_full_charge_capacity_milliamperehours =
				    (*_state.last_full_charge_capacity_milliwatthours) / (*_state.voltage);
		} else {
			panicLogger() << "thor: unhandled ACPI battery power unit " << *_state.acpi_units
			              << frg::endlog;
		}
...
void BatteryBusObject::updateBST() {
	...
	if (ret != UACPI_STATUS_OK) {
		infoLogger() << "thor: uacpi_object_get_package(bst) error " << uacpi_status_to_string(ret)
		             << frg::endlog;
		return;
	}
	...
	if (_state.acpi_units && remaining_capacity && remaining_capacity != BST::Capacity::UNKNOWN) {
		if (*_state.acpi_units == BIF::PowerUnit::MILLIAMPERE) {
			...
			if (_state.voltage)
				_state.remaining_capacity_milliwatthours =
				    (*_state.remaining_capacity_milliamperehours) * (*_state.voltage);
		} else if (*_state.acpi_units == BIF::PowerUnit::MILLIWATT) {
			_state.remaining_capacity_milliwatthours = *remaining_capacity;

			if (_state.voltage)
				_state.remaining_capacity_milliamperehours =
				    (*_state.remaining_capacity_milliwatthours) / (*_state.voltage);
		} else {
			panicLogger() << "thor: unhandled ACPI battery power unit " << *_state.acpi_units
			              << frg::endlog;
		}
...
void BatteryBusObject::updateState() {
	...
	if (logBatteryUpdates) {
		...
		infoLogger() << "\tState: " << (_state.charging ? "charging" : "discharging")
		             << frg::endlog;
		if (_state.battery_technology)
			infoLogger() << "\tBattery Technology: "
			             << (*_state.battery_technology == BIF::BatteryTechnology::PRIMARY
			                     ? "Primary"
			                     : "Secondary")
			             << frg::endlog;
		if (_state.design_voltage)
		...
		if (_state.remaining_capacity_milliwatthours)
			infoLogger() << "\tRemaining Capacity: " << *_state.remaining_capacity_milliwatthours
			             << " mWh" << frg::endlog;
		if (_state.remaining_capacity_milliamperehours)
			infoLogger() << "\tRemaining Capacity: " << *_state.remaining_capacity_milliamperehours
			             << " mAh" << frg::endlog;
		if (_state.design_capacity_milliwatthours)
			infoLogger() << "\tDesign Capacity: " << *_state.design_capacity_milliwatthours
			             << " mWh" << frg::endlog;
		if (_state.design_capacity_milliamperehours)
			infoLogger() << "\tDesign Capacity: " << *_state.design_capacity_milliamperehours
			             << " mAh" << frg::endlog;
		if (_state.last_full_charge_capacity_milliwatthours)
			infoLogger() << "\tLast Full Charge Capacity: "
			             << *_state.last_full_charge_capacity_milliwatthours << " mWh"
			             << frg::endlog;
		if (_state.last_full_charge_capacity_milliamperehours)
			infoLogger() << "\tLast Full Charge Capacity: "
			             << *_state.last_full_charge_capacity_milliamperehours << " mAh"
			             << frg::endlog;
	}
}
...
static initgraph::Task initBatteriesTask{
    &globalInitEngine,
    "acpi.init-batteries",
