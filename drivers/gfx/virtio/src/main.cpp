	...
	for (size_t i = 0; i < 16; i++) {
		if (info.modes[i].enabled) {
			...
			assignments.push_back(drm_core::Assignment::withInt(
			    ...
			    srcWProperty(),
			    info.modes[i].rect.width << 16
			));
			assignments.push_back(drm_core::Assignment::withInt(
			    crtc->primaryPlane()->sharedModeObject(),
			    srcHProperty(),
			    info.modes[i].rect.height << 16
			));
			assignments.push_back(drm_core::Assignment::withInt(
			    crtc->primaryPlane()->sharedModeObject(), crtcWProperty(), info.modes[i].rect.width
			));
			assignments.push_back(drm_core::Assignment::withInt(
			    crtc->primaryPlane()->sharedModeObject(), crtcHProperty(), info.modes[i].rect.height
			));

			assignments.push_back(drm_core::Assignment::withInt(connector, dpmsProperty(), 3));
			assignments.push_back(
			    drm_core::Assignment::withModeObj(connector, crtcIdProperty(), crtc)
			);

			std::vector<drm_mode_modeinfo> supported_modes;
			drm_core::addDmtModes(
			    supported_modes, info.modes[i].rect.width, info.modes[i].rect.height
			);
			std::sort(
...
std::tuple<int, int, int> GfxDevice::driverVersion() { return {0, 0, 1}; }
...
GfxDevice::createDumb(uint32_t width, uint32_t height, uint32_t bpp) {
	...
	auto bo = std::make_shared<BufferObject>(
	    this, _resourceIdAllocator.allocate(), size, helix::UniqueDescriptor(handle), width, height
	);
...
async::detached GfxDevice::Configuration::_dispatch(std::unique_ptr<drm_core::AtomicState> state) {
	...
	for (auto pair : plane_states) {
		...
		if (ps->fb != nullptr) {
			...
			co_await Cmd::setScanout(
			    ps->src_w,
			    ps->src_h,
			    static_pointer_cast<GfxDevice::Plane>(ps->plane)->scanoutId(),
			    resourceId,
			    _device
			);
...
drm_core::Plane *GfxDevice::Crtc::primaryPlane() { return _primaryPlane.get(); }

int GfxDevice::Crtc::scanoutId() { return _scanoutId; }
...
void GfxDevice::FrameBuffer::notifyDirty() { _xferAndFlush(); }
...
int GfxDevice::Plane::scanoutId() { return _scanoutId; }

// ----------------------------------------------------------------
// GfxDevice: BufferObject.
// ----------------------------------------------------------------

GfxDevice::BufferObject::~BufferObject() { _memory.release(); }
...
uint32_t GfxDevice::BufferObject::resourceId() { return _resourceId; }

async::result<void> GfxDevice::BufferObject::wait() { return async::make_result(_jump.wait()); }
...
async::result<void> doBind(mbus_ng::Entity hwEntity) {
	...
	auto transport =
	    co_await virtio_core::discover(std::move(hwDevice), virtio_core::DiscoverMode::modernOnly);
	...
	auto gfxEntity =
	    (co_await mbus_ng::Instance::global().createEntity("gfx_virtio", descriptor)).unwrap();
