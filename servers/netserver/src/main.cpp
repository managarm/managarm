...
async::result<std::shared_ptr<nic::Link>> setupIntelDevice(protocols::hw::Device hwDevice) {
	co_return nic::e1000::makeShared(std::move(hwDevice));
}
...
async::result<protocols::svrctl::Error> bindDevice(int64_t base_id) {
	...
	if(vendor_str->value == VENDOR_REDHAT) {
		...
		if(auto device_str = std::get_if<mbus::StringItem>(&properties["pci-device"]);
    		!device_str || !intel_device_ids.contains(device_str->value))
			co_return protocols::svrctl::Error::deviceNotSupported;

		device = co_await setupIntelDevice(std::move(hwDevice));
	} else {
		co_return protocols::svrctl::Error::deviceNotSupported;
	}
