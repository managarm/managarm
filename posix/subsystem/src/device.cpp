...
void UnixDeviceRegistry::install(std::shared_ptr<UnixDevice> device) {
	...
	if (!node_path.empty())
		async::detach(createDeviceNode(std::move(node_path), device->type(), device->getId()));
}
...
async::result<void> createDeviceNode(std::string path, VfsType type, DeviceId id) {
	...
	while (true) {
		...
		if (s == std::string::npos) {
			...
			if (!link)
				link =
				    std::get<std::shared_ptr<FsLink>>(co_await node->mkdir(path.substr(k, s - k)));
			k = s + 1;
			node = link->getTarget();
		}
	}
...
struct DeviceFile : File {
  private:
	...
	writeAll(Process *, const void *data, size_t length) override {
		...
		while (progress < length) {
			size_t chunk = co_await _file.writeSome(
			    reinterpret_cast<const char *>(data) + progress, length - progress
			);
			progress += chunk;
		}
		co_return length;
	...
	async::result<frg::expected<Error, PollStatusResult>> pollStatus(Process *) override {
		auto pollOverIpc = [this]() -> async::result<frg::expected<Error, PollStatusResult>> {
