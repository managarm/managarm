...
size_t KernletObject::instanceSize() { return _instanceSize; }

size_t KernletObject::numberOfBindParameters() { return _bindDefns.size(); }
...
BoundKernlet::BoundKernlet(smarter::shared_ptr<KernletObject> object) : _object{std::move(object)} {
...
void BoundKernlet::setupOffsetBinding(size_t index, uint32_t offset) {
	...
	if (logBinding)
		infoLogger() << "thor: Binding offset " << offset << " to instance offset " << defn.offset
		             << frg::endlog;
...
void BoundKernlet::setupMemoryViewBinding(size_t index, void *p) {
	...
	const auto &defn = _object->defnOfBindParameter(index);
	if (logBinding)
		infoLogger() << "thor: Binding memory view " << p << " to instance offset " << defn.offset
		             << frg::endlog;
...
smarter::shared_ptr<KernletObject> processElfDso(
    const char *buffer, const frg::vector<KernletParameterType, KernelAlloc> &bind_types
) {
	...
	for (int i = 0; i < ehdr.e_phnum; i++) {
		...
		if (phdr.p_type == PT_LOAD) {
			...
			for (size_t pg = 0; pg < misalign + phdr.p_memsz; pg += kPageSize) {
				auto va =
				    reinterpret_cast<VirtualAddr>(base + phdr.p_vaddr + pg) & ~(kPageSize - 1);
				auto physical = physicalAllocator->allocate(kPageSize);
				assert(physical != PhysicalAddr(-1) && "OOM");
				KernelPageSpace::global().mapSingle4k(va, physical, pf, CachingMode::null);
			}
	...
	auto resolveExternal = [](frg::string_view name) -> void * {
#ifdef THOR_ARCH_SUPPORTS_PIO
		uint16_t (*abi_pio_read16)(ptrdiff_t) = [](ptrdiff_t offset) -> uint16_t {
			if (logIo)
		...
		void (*abi_pio_write16)(ptrdiff_t, uint16_t) = [](ptrdiff_t offset, uint16_t value) {
			if (logIo)
		...
		uint8_t (*abi_mmio_read8)(const char *, ptrdiff_t) = [](const char *base,
		                                                        ptrdiff_t offset) -> uint8_t {
			if (logIo)
				infoLogger() << "__mmio_read8 on " << (void *)base << ", offset: " << offset
				             << frg::endlog;
		...
		uint32_t (*abi_mmio_read32)(const char *, ptrdiff_t) = [](const char *base,
		                                                          ptrdiff_t offset) -> uint32_t {
			if (logIo)
				infoLogger() << "__mmio_read32 on " << (void *)base << ", offset: " << offset
				             << frg::endlog;
		...
		void (*abi_mmio_write32)(char *, ptrdiff_t, uint32_t) =
		    [](char *base, ptrdiff_t offset, uint32_t value) {
			    if (logIo)
				    infoLogger() << "__mmio_write32 on " << (void *)base << ", offset: " << offset
				                 << frg::endlog;
		...
		void (*abi_trigger_bitset)(void *, uint32_t) = [](void *p, uint32_t bits) {
			if (logIo)
				infoLogger() << "__trigger_bitset on " << p << ", bits: " << bits << frg::endlog;
  ...
  private:
	coroutine<frg::expected<Error>> handleRequest(LaneHandle boundLane) override {
		...
		if (preamble.id() == bragi::message_id<managarm::kernlet::UploadRequest>) {
			auto maybeReq = bragi::parse_head_tail<managarm::kernlet::UploadRequest>(
			    headBuffer, tailBuffer, *kernelAlloc
			);
			...
			if (respError != Error::success)
				co_return respError;
			auto objectError =
			    co_await PushDescriptorSender{lane, KernletObjectDescriptor{std::move(kernlet)}};
