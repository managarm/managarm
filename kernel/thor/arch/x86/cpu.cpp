...
void activateTss(common::x86::Tss64 *tss) {
	common::x86::makeGdtTss64Descriptor(
	    getCpuData()->gdt, kGdtIndexTask, tss, sizeof(common::x86::Tss64)
	);
...
static constexpr uint16_t fcwInitializer = (1 << 0) |   // IM
                                           (1 << 1) |   // DM
                                           (1 << 2) |   // ZM
                                           (1 << 3) |   // OM
                                           (1 << 4) |   // UM
...
void workOnExecutor(Executor *executor) {
	...
	auto push = [&](uint64_t v) { memcpy(--nsp, &v, sizeof(uint64_t)); };
...
void UserContext::deactivate() { activateTss(&getCpuData()->tss); }
...
void UserContext::enableIoPort(uintptr_t port) { tss.ioBitmap[port / 8] &= ~(1 << (port % 8)); }
...
static initgraph::Task enumerateCpuFeaturesTask{
    ...
    [] {
	    ...
	    if (common::x86::cpuid(0x80000007)[3] & (1 << 8)) {
		    debugLogger() << "thor: CPUs support invariant TSC" << frg::endlog;
	    ...
	    if (common::x86::cpuid(0x01)[2] & (1 << 24)) {
		    debugLogger() << "thor: CPUs support TSC deadline mode" << frg::endlog;
		    globalCpuFeatures.haveTscDeadline = true;
	    } else {
		    debugLogger() << "thor: CPUs do not support TSC deadline mode!" << frg::endlog;
	    }

	    auto intelPmLeaf = common::x86::cpuid(0xA)[0];
	    if (intelPmLeaf & 0xFF) {
		    debugLogger() << "thor: CPUs support Intel performance counters" << frg::endlog;
		    globalCpuFeatures.profileFlags |= CpuFeatures::profileIntelSupported;
	    }
	    auto amdPmLeaf = common::x86::cpuid(0x8000'0001)[2];
	    if (amdPmLeaf & (1 << 23)) {
		    debugLogger() << "thor: CPUs support AMD performance counters" << frg::endlog;
	    ...
	    if (vmxSupported) {
		    debugLogger() << "thor: CPUs support VMX" << frg::endlog;
	    ...
	    if (svmSupported) {
		    debugLogger() << "thor: CPUs support SVM" << frg::endlog;
...
static initgraph::Task initBootProcessorTask{
    ...
    [] {
	    ...
	    debugLogger() << "Booting on CPU #" << staticBootCpuContext->localApicId << frg::endlog;
