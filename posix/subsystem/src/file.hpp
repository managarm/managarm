...
// TODO: Rename this enum as is not part of the VFS.
enum class VfsSeek { null, absolute, relative, eof };

template <typename T> using FutureMaybe = async::result<T>;

template <typename T> using expected = async::result<std::variant<Error, T>>;
...
struct File : private smarter::crtp_counter<File, DisposeFileHandle> {
  ...
  public:
	...
	static async::result<frg::expected<protocols::fs::Error>> ptTruncate(void *object, size_t size);
	...
	static async::result<size_t> ptSockname(void *object, void *addr_ptr, size_t max_addr_length);

	static async::result<void> ptIoctl(
	...
	static async::result<frg::expected<protocols::fs::Error>>
	ptSetSocketOption(void *obj, int layer, int number, std::vector<char> optbuf);
  ...
  public:
	...
	virtual async::result<protocols::fs::RecvResult> recvMsg(
	    Process *process,
	    uint32_t flags,
	    void *data,
	    size_t max_length,
	...
	virtual async::result<frg::expected<protocols::fs::Error, size_t>> sendMsg(
	    Process *process,
	    uint32_t flags,
	    const void *data,
	    size_t max_length,
	...
	virtual async::result<frg::expected<Error, PollWaitResult>>
	...
	virtual async::result<void> ioctl(
	    Process *process,
	    uint32_t id,
	...
	virtual async::result<frg::expected<protocols::fs::Error>>
	setSocketOption(int layer, int number, std::vector<char> optbuf);
...
struct DummyFile final : File {
  public:
	...
	DummyFile(
	    std::shared_ptr<MountView> mount, std::shared_ptr<FsLink> link, DefaultOps default_ops = 0
	)
...
struct PassthroughFile : File {
	PassthroughFile(helix::UniqueLane lane)
	    : File{StructName::get("passthrough")},
	      _file{std::move(lane)} {}
