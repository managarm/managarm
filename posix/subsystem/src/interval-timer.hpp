...
struct IntervalTimer {
	...
	void getTime(timeval &initial, timeval &interval);
	void reset();

	// Called when the timer expires. The success argument indicates whether the timer
	// expired (true) or if the wait was cancelled (false). The return value indicates
	// whether the timer should be cancelled (false) or should be continued (true).
