...
enum class TimerState { none, queued, elapsed, retired };
...
struct PrecisionTimerEngine final : private AlarmSink {
  ...
  public:
	...
	template <typename R> struct SleepOperation;

	struct [[nodiscard]] SleepSender {
		using value_type = void;

		template <typename R> friend SleepOperation<R> connect(SleepSender sender, R receiver) {
...
inline void PrecisionTimerNode::CancelFunctor::operator()() { node_->_engine->cancelTimer(node_); }
