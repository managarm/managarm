...
struct OpenFile : File {
  public:
	...
	OpenFile(uint64_t mask, bool nonBlock)
	    : File{StructName::get("signalfd"), nullptr, SpecialLink::makeSpecialLink(VfsType::regular, 0777)},
	      _mask{mask},
	      _nonBlock{nonBlock} {}
	...
	async::result<frg::expected<Error, PollWaitResult>> pollWait(
	    Process *process, uint64_t inSeq, int pollMask, async::cancellation_token cancellation
	) override {
		(void)pollMask; // TODO: utilize mask.
		auto result = co_await process->signalContext()->pollSignal(inSeq, _mask, cancellation);
		co_return PollWaitResult{std::get<0>(result), (std::get<1>(result) & _mask) ? EPOLLIN : 0};
	}
	...
	async::result<frg::expected<Error, PollStatusResult>> pollStatus(Process *process) override {
		...
		co_return PollStatusResult{
		    std::get<0>(result), (std::get<1>(result) & _mask) ? EPOLLIN : 0
		};
	}
