...
template <typename T> struct coroutine_continuation {
...
template <> struct coroutine_continuation<void> {
	virtual void resume() = 0;
...
template <typename T, typename R> struct coroutine_operation;

template <typename T> struct coroutine {
	template <typename T_, typename R> friend struct coroutine_operation;
			...
			struct awaiter {
				...
				void await_suspend(std::coroutine_handle<void>) noexcept {
					auto cfp = promise_->cfp_.exchange(
					    coroutine_cfp::past_suspend, std::memory_order_release
					);
...
struct coroutine_operation final : private coroutine_continuation<T> {
  ...
  private:
	void resume() override { async::execution::set_value_noinline(receiver_, std::move(value())); }
...
template <typename T, typename R> coroutine_operation<T, R> connect(coroutine<T> s, R receiver) {
	return {std::move(s), std::move(receiver)};
};

template <typename T> async::sender_awaiter<coroutine<T>, T> operator co_await(coroutine<T> s) {
	return {std::move(s)};
}
...
struct detached_coroutine_promise {
	...
	void operator delete(void *p, size_t size) { return thor::kernelAlloc->deallocate(p, size); }
