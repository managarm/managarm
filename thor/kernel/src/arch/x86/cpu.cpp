...
void secondaryMain(StatusBlock *status_block) {
	...
	__atomic_store_n(&status_block->stage, 3, __ATOMIC_RELEASE);
...
void bootSecondary(unsigned int apic_id) {
	...
	PageAccessor image_accessor{generalWindow, pma};
	memcpy(image_accessor.get(), _binary_thor_kernel_obj_arch_x86_trampoline_bin_start,
			image_size);
	...
	PageAccessor status_accessor{generalWindow, pma + kPageSize};
	auto status_block = reinterpret_cast<StatusBlock *>(status_accessor.get());

	status_block->stage = 0;
	...
	__atomic_store_n(&status_block->stage, 2, __ATOMIC_RELEASE);
	
	// Wait until the AP exits the boot code.
	while(__atomic_load_n(&status_block->stage, __ATOMIC_ACQUIRE) < 3) {
		frigg::pause();
	}
