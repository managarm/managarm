...
struct ReaderFile : File {
  public:
	...
	ReaderFile(
	    std::shared_ptr<MountView> mount, std::shared_ptr<FsLink> link, bool nonBlock = false
	)
	...
	async::result<frg::expected<Error, PollWaitResult>> pollWait(
	    Process *, uint64_t pastSeq, int mask, async::cancellation_token cancellation
	) override {
		...
		if (id == managarm::fs::GenericIoctlRequest::message_id) {
			...
			case FIONREAD: {
				size_t count = std::accumulate(
				    _channel->packetQueue.cbegin(),
				    _channel->packetQueue.cend(),
				    0,
				    [](size_t sum, const Packet &p) { return sum + (p.buffer.size() - p.offset); }
				);
			...
			std::cout << "\e[31m"
			             "fifo: Unknown ioctl() message with ID "
			          << id << "\e[39m" << std::endl;
