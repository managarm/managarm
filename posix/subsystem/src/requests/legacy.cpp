...
HandleRequest::operator()(managarm::posix::CntRequest &&req,
	...
	if(req.request_type() == managarm::posix::CntReqType::WAIT) {
		...
		auto removePendingSignal = [&](int signo) {
			if (self->delayedSignal && self->delayedSignal->signalNumber == signo) {
			...
			while (auto item = self->tryFetchSignal(1 << (signo - 1)))
				delete item;
		};
		...
		logRequest(logRequests, self, "EPOLL_ADD", "epollfd={} fd={}", req.fd(), req.newfd());
		...
		logRequest(logRequests, self, "EPOLL_MODIFY");
		...
		logRequest(logRequests, self, "EPOLL_DELETE");
