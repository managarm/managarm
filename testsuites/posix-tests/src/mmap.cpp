...
void *offsetBy(void *ptr, ptrdiff_t n) {
	return reinterpret_cast<void *>(reinterpret_cast<uintptr_t>(ptr) + n);
}

sigjmp_buf restoreEnv;

void signalHandler(int, siginfo_t *, void *) { siglongjmp(restoreEnv, 1); }
...
DEFINE_TEST(mprotect_check_whether_split_mappings_get_protected_correctly, ([] {
	            void *mem =
	                mmap(NULL, 0x6000, PROT_READ | PROT_EXEC, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
	            assert_errno("mmap", mem != MAP_FAILED);
	            ...
	            runChecks([&] { assert(ensureWritable(mem)); });
            }))
