...
async::result<void> FileSystem::init() {
	...
	HEL_CHECK(helCreateManagedMemory(
	    superblock_.total_bytes, kHelManagedReadahead, &treeBackingMemory, &treeFrontalMemory
	));
	manageTree();
...
FileSystem::getBtreeNode(LogicalAddress start) {
	...
	co_return std::make_pair(std::move(nodeMapping), std::move(lock));
}

async::result<std::optional<std::span<std::byte>>>
FileSystem::find(LogicalAddress start, key k, BtreePtr &stack) {
	auto [nodeMapping, lock] = co_await getBtreeNode(start);
	block_header *header = reinterpret_cast<block_header *>(nodeMapping.get());

	if (header->level != 0) {
		auto ub = std::ranges::upper_bound(header->key_ptrs(), k, {}, &key_ptr::k);
		if (ub == header->key_ptrs().begin())
...
FileSystem::upperBound(LogicalAddress start, key k, BtreePtr &stack) {
	...
	if (header->level != 0) {
		...
		if (first || ub == header->key_ptrs().end())
			co_return first;
...
FileSystem::firstKey(LogicalAddress root, BtreePtr &stack) {
	auto [nodeMapping, lock] = co_await getBtreeNode(root);
	block_header *header = reinterpret_cast<block_header *>(nodeMapping.get());

	if (header->level != 0) {
		stack.emplace_back(root, header->key_ptrs().at(0).k, std::move(nodeMapping));
		co_return co_await firstKey(header->key_ptrs().at(0).addr, stack);
	} else {
...
async::result<std::optional<std::span<std::byte>>> FileSystem::nextKey(BtreePtr &stack) {
	{
		...
		block_header *header = reinterpret_cast<block_header *>(layer.mapping.get());
		assert(header->level == 0);

		std::span<item> items{
		    reinterpret_cast<item *>(reinterpret_cast<std::byte *>(layer.mapping.get()) + sizeof(block_header)), header->nritems
		};
	...
	while (true) {
		...
		std::span<key_ptr> items{
		    reinterpret_cast<key_ptr *>(reinterpret_cast<std::byte *>(layer.mapping.get()) + sizeof(block_header)), header->nritems
		};
