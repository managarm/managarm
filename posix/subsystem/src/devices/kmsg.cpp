  ...
  private:
	...
	readSome(Process *, void *data, size_t length) override {
		...
		int ret_len = snprintf(
		    reinterpret_cast<char *>(data), length, "%s", reinterpret_cast<char *>(buffer.data())
		);
	...
	helix::BorrowedDescriptor getPassthroughLane() override { return passthrough_; }

	void handleClose() override { lane_ = helix::UniqueLane{}; }
  ...
  public:
	static void serve(smarter::shared_ptr<KmsgFile> file) {
		...
		async::detach(protocols::fs::servePassthrough(
		    std::move(lane), file, &fileOperations, file->cancelServe_
		));
	}
...
struct KmsgDevice final : UnixDevice {
	KmsgDevice() : UnixDevice(VfsType::charDevice) { assignId({1, 11}); }

	std::string nodePath() override { return "kmsg"; }

	async::result<frg::expected<Error, smarter::shared_ptr<File, FileHandle>>> open(
	    std::shared_ptr<MountView> mount, std::shared_ptr<FsLink> link, SemanticFlags flags
	) override {
		...
		auto file = smarter::make_shared<KmsgFile>(
		    std::move(mount), std::move(link), (co_await entity.getRemoteLane()).unwrap(), nonblock
		);
...
std::shared_ptr<UnixDevice> createKmsgDevice() { return std::make_shared<KmsgDevice>(); }
