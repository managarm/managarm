...
std::shared_ptr<MountView> MountView::getParent() const { return _parent; }

std::shared_ptr<FsLink> MountView::getAnchor() const { return _anchor; }

std::shared_ptr<FsLink> MountView::getOrigin() const { return _origin; }

async::result<void>
MountView::mount(std::shared_ptr<FsLink> anchor, std::shared_ptr<FsLink> origin) {
	if (anchor) {
...
async::result<void> populateRootView() {
	...
	std::vector<std::pair<std::shared_ptr<FsNode>, std::string>> stack;
	...
	while (!stack.empty()) {
		...
		while (true) {
			...
			if (resp.file_type() == managarm::fs::FileType::DIRECTORY) {
				// TODO: Check for errors from mkdir().
				auto link =
				    std::get<std::shared_ptr<FsLink>>(co_await item.first->mkdir(resp.path()));
...
struct Path {
	...
	bool isRelative() { return _relative; }

	bool trailingSlash() { return _trailingSlash; }

	bool empty() { return _components.empty(); }
...
async::result<frg::expected<protocols::fs::Error, void>> PathResolver::resolve(ResolveFlags flags) {
	...
	while (true) {
		if (_components.empty() || ((flags & resolvePrefix) && _components.size() == 1))
			break;
		...
		if (name == ".") {
			...
			if (_currentPath == _rootPath) {
				...
				assert(
				    owner && "VFS resolution crosses root directory of non-root"
			...
			if (_currentPath.second->getTarget()->hasTraverseLinks()) {
				...
				if (!result) {
					assert(
					    ...
					    result.error() == Error::noSuchFile || result.error() == Error::notDirectory
					);
					_currentPath = ViewPath{_currentPath.first, nullptr};
					if (result.error() == Error::illegalOperationTarget) {
						std::cout << "\e[33mposix: Illegal operation target in "
						             "PathResolver::resolve\e[39m"
						          << std::endl;
				...
				if (auto mount = _currentPath.first->getMount(child); mount) {
					if (debugResolve)
						std::cout << "posix " << sn << ":     VFS path is a mount point"
						          << std::endl;
				...
				if (next.second->getTarget()->getType() == VfsType::symlink &&
					...
					auto symlinkResult =
					    co_await next.second->getTarget()->readSymlink(next.second.get(), _process);
					...
					if (!link.isRelative())
						...
						_currentPath =
						    ViewPath{_currentPath.first, next.second->getOwner()->treeLink()};
...
ViewPath rootPath() { return ViewPath{rootView, rootView->getOrigin()}; }

async::result<frg::expected<protocols::fs::Error, ViewPath>>
resolve(ViewPath root, ViewPath workdir, std::string name, Process *process, ResolveFlags flags) {
...
async::result<frg::expected<Error, smarter::shared_ptr<File, FileHandle>>> open(
	...
	auto resolveResult = co_await resolve(
	    std::move(root), std::move(workdir), std::move(name), process, resolve_flags
	);
	if (!resolveResult) {
		assert(
