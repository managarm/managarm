...
struct HashTreeHeader {
	uint16_t limit;
	uint16_t count;
	uint32_t block;
	HashTreeEntry entries[];
};

struct HashTreeRoot {
	uint32_t dotInode;
	uint16_t dotRecordLength;
	uint8_t dotNameLength;
	uint8_t dotFileType;
	char dotName[4];
	uint32_t dotdotInode;
	uint16_t dotdotRecordLength;
	uint8_t dotdotNameLength;
	uint8_t dotdotFileType;
	char dotdotName[4];
	uint32_t reserved;
	uint8_t hashVersion;
	uint8_t infoLength;
	uint8_t indirectLevels;
	uint8_t unusedFlags;
	HashTreeHeader hdr;
};
...
enum {
	EXT4_DIR_HASH_LEGACY = 0,
	EXT4_DIR_HASH_HALF_MD4 = 1,
	EXT4_DIR_HASH_TEA = 2,
	EXT4_DIR_HASH_LEGACY_UNSIGNED = 3,
	EXT4_DIR_HASH_HALF_MD4_UNSIGNED = 4,
	EXT4_DIR_HASH_TEA_UNSIGNED = 5,
	EXT4_DIR_HASH_SIPHASH = 6
};
