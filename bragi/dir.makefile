...
$c_TARGETS := all-$c clean-$c install-$c $($c_BINDIR)/libbragi_mbus.a $($c_BINDIR)

.PHONY: all-$c clean-$c gen-$c install-$c

all-$c: $($c_BINDIR)/libbragi_mbus.a

clean-$c:
	rm -f $($d_BINDIR)/libbragi_mbus.a $($d_OBJECT_PATHS) $($d_OBJECT_PATHS:%.o=%.d)
...
install-$c:
	...
	install $($d_BINDIR)/libbragi_mbus.a $(SYSROOT_PATH)/usr/lib
...
$($c_BINDIR)/libbragi_mbus.a: $($c_OBJECT_PATHS) | $($c_BINDIR)
