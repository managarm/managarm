...
struct UnixDevice {
	UnixDevice(VfsType type) : _type{type}, _id{0, 0} {}

  protected:
	~UnixDevice() = default;

  public:
	VfsType type() { return _type; }

	void assignId(DeviceId id) { _id = id; }

	DeviceId getId() { return _id; }
...
struct UnixDeviceRegistry {
  ...
  private:
	struct Compare {
		...
		bool operator()(std::shared_ptr<UnixDevice> a, DeviceId b) const { return a->getId() < b; }
		bool operator()(DeviceId a, std::shared_ptr<UnixDevice> b) const { return a < b->getId(); }
