...
void toDrmModeInfo(const NvKmsKapiDisplayMode *displayMode, drm_mode_modeinfo *mi) {
	...
	if(strlen(displayMode->name)) {
		memcpy(mi->name, displayMode->name,
			std::min(sizeof(mi->name), sizeof(displayMode->name)));

		mi->name[sizeof(mi->name) - 1] = '\0';
	} else {
		snprintf(mi->name, sizeof(mi->name), "%dx%d%s",
			mi->hdisplay, mi->vdisplay, (displayMode->timings.flags.interlaced) ? "i" : "");
	}
}
